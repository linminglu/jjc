...
import com.gf.bjpk10.util.BjPk10Util;
...
import com.gf.sfpk10.util.SfPk10Util;
import com.ram.model.User;
import com.ram.service.user.IUserService;

public class SfPk10ServiceImpl extends BaseService implements ISfPk10Service {
	private ISfPk10DAO gfSfPk10DAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setSfPk10DAO(ISfPk10DAO gfSfPk10DAO) {
//		gfSfPk10DAO = gfSfPk10DAO;
//		super.dao = gfSfPk10DAO;
//	}

	public void setGfSfPk10DAO(ISfPk10DAO gfSfPk10DAO) {
		this.gfSfPk10DAO = gfSfPk10DAO;
		super.dao = gfSfPk10DAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_SFPK10;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfSfPk10DAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfSfPk10DAO.updateObjectList(saveList, null);
			flag = "success";
		} else {
			flag = "inited";
		}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfSfPk10GaSession session = new GfSfPk10GaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfSfPk10GaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfSfPk10GaSession>();
...
//			
//			for (int i = 0; i < SfPk10Constants.SF_PK10_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*SfPk10Constants.SF_PK10_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfSfPk10GaSession k10Session = new GfSfPk10GaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,SfPk10Constants.SF_PK10_TIME_INTERVAL));
//				k10Session.setOpenStatus(SfPk10Constants.SF_PK10_OPEN_STATUS_INIT);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfSfPk10GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfSfPk10DAO.countObjects(hq);
	...
	public List<GfSfPk10GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfSfPk10GaSession> sessionlist=new ArrayList<GfSfPk10GaSession>();
		GfSfPk10GaSession currentSession=gfSfPk10DAO.getCurrentSession();
		GfSfPk10GaSession tempsession =gfSfPk10DAO.getPreviousSessionBySessionNo((Long.parseLong(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfSfPk10GaSession) gfSfPk10DAO.getObject(GfSfPk10GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfSfPk10GaSession session =gfSfPk10DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(SfPk10Constants.SF_PK10_OPEN_STATUS_INIT) || openStatus1.equals(SfPk10Constants.SF_PK10_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(SfPk10Constants.SF_PK10_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfSfPk10DAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_SFPK10);					
					if(sessionInfo != null){
						...
						gfSfPk10DAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfSfPk10GaSession> updateAndOpenResult() {
//		List<GfSfPk10GaSession> sessionlist=new ArrayList<GfSfPk10GaSession>();
//		GfSfPk10GaSession currentSession=gfSfPk10DAO.getCurrentSession();
//		GfSfPk10GaSession tempsession =gfSfPk10DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
//		}else{
//			tempsession=(GfSfPk10GaSession) gfSfPk10DAO.getObject(GfSfPk10GaSession.class, (currentSession.getSessionId()-1));
//		}
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfsfpk10");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[SfPk10 start fetch result xml data...]________________---------SfPk10");
//						GameHelpUtil.log("sfpk10", "___[SfPk10 start fetch result xml data...]________________---------SfPk10");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[SfPk10 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/SfPk10.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("sfpk10", "SfPk10ServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("sfpk10", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("sfpk10", "SfPk10ServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateSfPk10SessionOpenResultMethod(GfSfPk10GaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_SFPK10, Constants.PUB_STATUS_OPEN);
			...
			para.add(Constants.GAME_TYPE_GF_SFPK10);
			if(orderNum !=null){
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							BigDecimal point = det.getPointMultiple();//积分倍率
							
							BigDecimal winMoney = SfPk10Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_SFPK10);
			...
			gfSfPk10DAO.updateObjectList(spList, null);
			gfSfPk10DAO.updateObjectList(spDeList, null);
			gfSfPk10DAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("sfpk10", "SfPk10ServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_SFPK10);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_SFPK10);
		...
		gfSfPk10DAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfSfPk10DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_SFPK10,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_SFPK10,remark);
	}
	...
	public GfSfPk10GaSession getCurrentSession(){
		return gfSfPk10DAO.getCurrentSession();
	}
	public GfSfPk10GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfSfPk10DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	public void saveUserBetInfo(String room,Map<Integer,Integer> betMap,List<GaBetOption> list,GfSfPk10GaSession gaPick11Session,User user,BigDecimal betAll){
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_SFPK10);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfSfPk10DAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_SFPK10, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfSfPk10DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfSfPk10GaSession GfSfPk10GaSession = gfSfPk10DAO.getCurrentSession(); // 当前期数信息
		if(GfSfPk10GaSession != null){
			String curNo = GfSfPk10GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Long.parseLong(curNo) - 1)); //上期期号
			GfSfPk10GaSession SfPk10PreSession = getPreviousSessionBySessionNo(preNo);
			if(SfPk10PreSession==null){
				SfPk10PreSession=(GfSfPk10GaSession) gfSfPk10DAO.getObject(GfSfPk10GaSession.class, GfSfPk10GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfSfPk10GaTrend gdk10 ");
			List<Object> trendList = gfSfPk10DAO.findObjects(hq); //上期冷热排行榜
			List<GfSfPk10GaTrend> saveList = new ArrayList<GfSfPk10GaTrend>(); // 本期冷热排行榜

			String results = SfPk10PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfSfPk10GaTrend trend = (GfSfPk10GaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfSfPk10GaSession session,String openResult){

		boolean flag=false;
		session.setOpenResult(openResult);
		gfSfPk10DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	
	public boolean saveAndOpenResult(GfSfPk10GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateSfPk10SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("sfpk10", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfSfPk10DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfSfPk10DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfSfPk10DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfSfPk10GaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_SFPK10);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfSfPk10GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_SFPK10);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_SFPK10));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfSfPk10DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_SFPK10) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_SFPK10,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_SFPK10,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfSfPk10DAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_SfPk10, betmoney, regId);
	 ...
	 * @param list 投注信息
	 * @param seMap 投注明细
	 * @param isAddNo 是否追号 --代购
	 * @param isWinStop 是否中奖后停止---代购
	 * @param betmoney 投注金额
	 * @param betNum 购买份数---合买
	 * @param currentSession 场次
	 * @return
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfSfPk10GaSession currentSession) {
		//保存对象
		List<Object> saveList = new ArrayList<Object>();
		//批次号，是否属于同一追号
		String batchNum = GameHelpUtil.getBatchNum();
		//投注与明细关联
		...
		String sessionNo="";//期号
		int multiple;//倍数
		for(Map.Entry<String, Integer> entry:seMap.entrySet()){
			sessionNo = entry.getKey();multiple = entry.getValue();
			log.fatal("_______________________________"+sessionNo+","+multiple+","+currentSession.getSessionNo());
			GfSfPk10GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= SfPk10DAO.findObjects(hql);
...
//			sp.setIsGuarantee(DcbConstants.NOT_GUARANTEE);//不保底
			...
			sp.setSessionNo(sessionNo);
			sp.setMultiple(multiple);
			sp.setBetType(GameConstants.PROCUREMENT_SERVICE);//代购
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				for(int i=0;i<opList.size();i++){
					GaBetOption betOp = (GaBetOption) opList.get(i);
					if(playType == Integer.parseInt(betOp.getPlayType())){
						betRate = betOp.getBetRate().toString();
						title= betOp.getTitle();
						point=betOp.getPointMultiple();
					}
				}
				GaBetSponsorDetail de = new GaBetSponsorDetail();
				de.setBetRate(new BigDecimal(betRate));
				de.setJointId(sp.getJointId());
				...
				int betCount = SfPk10Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		//user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, betmoney, null,Constants.GAME_TYPE_GF_SFPK10,remark);
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_SFPK10,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfSfPk10DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_SfPk10,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_SfPk10);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_SfPk10);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfSfPk10GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("sfpk10", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfSfPk10GaSession session=SfPk10DAO.getCurrentSession();
//		if(session!=null){
//			GfSfPk10GaSession tempsession =SfPk10DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfSfPk10GaOmit omit=gfSfPk10DAO.getSfPk10OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfSfPk10GaOmit preomit=gfSfPk10DAO.getSfPk10OmitBySessionNo((Long.parseLong(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfSfPk10GaSession se = (GfSfPk10GaSession) gfSfPk10DAO.getObject(GfSfPk10GaSession.class, tempsession.getSessionId()-1);
							preomit=gfSfPk10DAO.getSfPk10OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfSfPk10GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfSfPk10GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("sfpk10", "SfPk10ServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfSfPk10GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("sfpk10", "SfPk10ServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("sfpk10", "SfPk10ServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("sfpk10", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("sfpk10", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("sfpk10", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_SFPK10,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_SFPK10,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_SFPK10,remark);
	}
	...
	public GfSfPk10GaSession getFirstSessionByDate(String date) {
		return gfSfPk10DAO.getFirstSessionByDate(date);
	}
	public GfSfPk10GaSession getEndSessionByDate(String date){
		return gfSfPk10DAO.getEndSessionByDate(date);
	}
	public List<GfSfPk10GaOmit> findGfSfPk10GaOmitList(String string,
			List<Object> para, int num){
		return gfSfPk10DAO.findGfSfPk10GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfSfPk10GaSession tempsession) {
//		GfSfPk10GaSession session=SfPk10DAO.getCurrentSession();
//		if(session!=null){
//			GfSfPk10GaSession tempsession =SfPk10DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_SFPK10);
...
//			}
	 ...
	 * @return
	 */
	public String getTodaySessionNo(GfSfPk10GaSession beforeSession,int index,String sessionNo1){
	...
	public String updateInitTodaySession() {
		...
		if(!isTodaySessionInit){
			...
			for (int i = 0; i < SfPk10Constants.SF_PK10_MAX_PART; i++) {
				//计算出当前场次的时间
				Date startTime=DateTimeUtil.getDateTimeOfSeconds(startDate,SfPk10Constants.SF_PK10_TIME_INTERVAL*i);
				Date endTime=DateTimeUtil.getDateTimeOfSeconds(startTime,SfPk10Constants.SF_PK10_TIME_INTERVAL);
	...
	@Override
	public boolean saveRevokePrize(GfSfPk10GaSession session) {

		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_SFPK10,session.getSessionNo());
		if(result){
