 ...
 * 定时去官网抓取开奖数据
 */
public class FetchFiveResult extends QuartzJobBean{
...
//    	
//    	if(!Constants.getTimerOpen("wfc.fetchResult.gf")) return;
//    	
//		GfFiveGaSession fiveSessaion=gfFiveService.updateAndOpenResult();
//		
//		String flag = gfFiveService.updateFiveSessionOpenResultMethod(fiveSessaion, fiveSessaion.getOpenResult(),null);
//		if(ParamUtils.chkString(flag)){
//			fiveSessaion.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这期开奖计算出错
//		}else{
//			gfFiveService.updateDayBetCount(fiveSessaion);
//		}
//		gfFiveService.updateFetchAndOpenOmit(fiveSessaion);
//
//		gfFiveService.saveObject(fiveSessaion, null);
//    }
  ...
  //##定时接口-----------------------------------------------------------------------------------------
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_FC);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_FC);//抓取间隔毫秒
	int delayOpenTime = GameHelpUtil.getDelayOpenTime(Constants.GAME_TYPE_GF_FC);//延迟开奖秒数(系统开) 
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_FC)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_FC);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_FC);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfFiveGaSession curSession = gfFiveService.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfFiveGaSession openSession = (GfFiveGaSession)gfFiveService.getObject(GfFiveGaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfFiveGaSession> list = (List<GfFiveGaSession>) gfFiveService.updateAndOpenResult(sessionNoMap);
		        			
		        			Collections.sort(list,new Comparator<GfFiveGaSession>(){
		        				public int compare(GfFiveGaSession o1, GfFiveGaSession o2){
		        			...
		        			for(GfFiveGaSession session:list){
		        				
		        				gfFiveService.updateFetchAndOpenTrendResult(session);

		        				String flag = gfFiveService.updateFiveSessionOpenResultMethod(session, session.getOpenResult(),null);
		        				
		        				if(ParamUtils.chkString(flag)){
		        					session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
		        					savelist.add(session);
		        				}else{
		        					gfFiveService.updateDayBetCount(session);
		        				}
		        				gfFiveService.updateFetchAndOpenOmit(session);
		        			}
		        			gfFiveService.updateObjectList(savelist, null);
