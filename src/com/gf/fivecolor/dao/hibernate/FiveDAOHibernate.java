package com.gf.fivecolor.dao.hibernate;
...
implements IFiveDAO {
	...
	public GfFiveGaSession getCurrentSession() {
		...
		List<GfFiveGaSession> list = getSession()
				.createQuery(
						"from GfFiveGaSession se where se.startTime<=? and se.endTime>? and (se.openStatus=? or se.openStatus=?) order by se.sessionId")
		...
		if (list != null && list.size() > 0) {
			return (GfFiveGaSession) list.get(0);
		}
		return null;
	}
	...
	public GfFiveGaSession getPreviousSessionBySessionNo(String sessionNo) {
		String  hql=" from GfFiveGaSession bj3 where bj3.sessionNo=? ";
	...
	public List<GfFiveGaTrend> findGaTrendList() {
		String hql = " from GfFiveGaTrend trend order by trend.trendId desc";
		List<GfFiveGaTrend> list;
	...
	public PaginationSupport findFcGaSessionList(String hqsl,List<Object> para,int pageNum,int pageSize) {
		Query query = makeQuerySQL( " from GfFiveGaSession ho where 1=1 "+hqsl, para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfFiveGaSession ho where 1=1 "+hqsl, para);
...
//	@Override
//	public List<GfFiveGaSession> getCurrentSession(int start, int end) {
...
//		}
//		
//		List<GfFiveGaSession> list = getSession().createQuery("from GfFiveGaSession bj3s where bj3s.startTime>? and "
...
//		Query query = makeQuerySQL( " select new com.bj3.model.dto.FcBetDTO(se.sessionNo,ho.totalPoint,ho.winCash,se.startTime,se.endTime,ho.totalPoint-ho.winCash) from FcGaBet ho,GfFiveGaSession se where 1=1 "
//				+ " and se.sessionId = ho.sessionId "+hql+" group by ho.sessionId order by ho.sessionId desc ", para);
//		query.setFirstResult(pageNum);
//		query.setMaxResults(pageSize);
//		List queList = query.list();
//		Query count = makeQuerySQL("select count(ho.betId) from FcGaBet ho, GfFiveGaSession se where 1=1" +
...
//	}
	...
	public List<WinCoDTO> findGaWinCountList() {
	    Query query = getSession().createQuery(" select new com.gf.fivecolor.model.dto.WinCoDTO(co,u) from GaWinCount co,User u where 1=1 and co.userId = u.userId order by co.totalMoney desc")
	    		.setMaxResults(10);
	...
	public GfFiveGaOmit getFcGaOmitBySessionNo(String sessionNo){
		String hql = " from GfFiveGaOmit ho where ho.sessionNo=? ";
		List<GfFiveGaOmit> list;
	...
	public List<GfFiveGaOmit> findGfFiveGaOmitList(String hqls, List<Object> para,
			int num) {
		String  hql=" from GfFiveGaOmit ho where 1=1  ";
		Query query =makeQuerySQL(hql+hqls,para);
		query.setMaxResults(num);
		List<GfFiveGaOmit>list=query.list();
		return list;
	}
	...
	public PaginationSupport findGfFiveGaSessionList(String hqls,
		...
		Query query = makeQuerySQL( " from GfFiveGaSession ho where 1=1 "+hqls,para);
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfFiveGaSession ho where 1=1 "+hqls, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
	...
	public PaginationSupport findGameBetAndWinList(String hqls,
			List<Object> para, int startIndex, int pageSize) {
		Query query = makeQuerySQL( " select new com.apps.model.dto.AgentDTO(se.sessionNo,se.startTime,se.endTime,sum(ho.money),sum(ho.winCash),sum(ho.money)-sum(ho.winCash)) from GaBetSponsor ho,GfFiveGaSession se where 1=1 "
				+ " and ho.gameType ="+ Constants.GAME_TYPE_GF_FC
				+ " and se.sessionId = ho.sessionId "+hqls+" group by ho.sessionId order by ho.sessionId desc ", para);
		
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GaBetSponsor ho,GfFiveGaSession se where 1=1" 
				+ " and ho.gameType ="+ Constants.GAME_TYPE_GF_FC
