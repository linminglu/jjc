...
public class FiveUtil {
	...
	public static int getTotalBetNum(String playType,String betBall){
		int totalBet =0;
		if(FiveConstants.PLAY_TYPE_ZERO.equals(playType)){//大小单双
		...
		}else if(FiveConstants.PLAY_TYPE_ONE.equals(playType)){//一星
			char[] unit = betBall.toCharArray();
			int unitNum =unit.length;
			totalBet = totalBet + unitNum;
		}else if(FiveConstants.PLAY_TYPE_TWO.equals(playType)){//二星组选
			String[] grou = betBall.split(",");
			int unitNum =FiveUtil.getTotal(grou.length,2);
			totalBet = totalBet + unitNum;
		}else if(FiveConstants.PLAY_TYPE_THREE.equals(playType)){//二星直选
			char[] tents = betBall.split(",")[0].toCharArray();
			char[] unit = betBall.split(",")[1].toCharArray();
			int tentsNum = tents.length;
			int unitNum =unit.length;
			totalBet = totalBet + tentsNum*unitNum;
		}else if(FiveConstants.PLAY_TYPE_FOUR.equals(playType)){//三星组六
			String[] grou = betBall.split(",");
			int unitNum =FiveUtil.getTotal(grou.length,3);
			totalBet = totalBet + unitNum;
		}else if(FiveConstants.PLAY_TYPE_FIVE.equals(playType)){//三星组三
			String[] grou = betBall.split(",");
			int unitNum =FiveUtil.getTotal(grou.length,2);
			totalBet = totalBet + unitNum*2;
		}else if(FiveConstants.PLAY_TYPE_SIX.equals(playType)){//三星直选
			char[] tents = betBall.split(",")[0].toCharArray();
			char[] unit = betBall.split(",")[1].toCharArray();
		...
		}else if(FiveConstants.PLAY_TYPE_SEVEN.equals(playType)){//五星直选
			char[] tents = betBall.split(",")[0].toCharArray();
			char[] unit = betBall.split(",")[1].toCharArray();
			char[] percent = betBall.split(",")[2].toCharArray();
		...
		}else if(FiveConstants.PLAY_TYPE_EIGHT.equals(playType)){//三星组三
			char[] tents = betBall.split(",")[0].toCharArray();
			char[] unit = betBall.split(",")[1].toCharArray();
			char[] percent = betBall.split(",")[2].toCharArray();
			char[]  thousand = betBall.split(",")[3].toCharArray();
...
//		  System.out.println("数据长度 " + list.size());
	  ...
	  */
		public static BigDecimal judgeWinMoney(String playType, String betBall,
			...
			if(FiveConstants.PLAY_TYPE_ZERO.equals(playType)){
				...
				return winMoney;
			}else if(FiveConstants.PLAY_TYPE_ONE.equals(playType)){
				char ball;//单个球
				...
				return winMoney;
			}else if(FiveConstants.PLAY_TYPE_TWO.equals(playType)){
				...
				return winMoney;
			}else if(FiveConstants.PLAY_TYPE_THREE.equals(playType)){
				if(betBall.split(",")[0].contains(results[3])&&betBall.split(",")[1].contains(results[4])){
					winMoney = winMoney.add(betRate);
				}
				return winMoney;
			}else if(FiveConstants.PLAY_TYPE_FOUR.equals(playType)){
...
//					}
				...
				return winMoney;
			}else if(FiveConstants.PLAY_TYPE_FIVE.equals(playType)){
...
//					}
				...
				return winMoney;
			}else if(FiveConstants.PLAY_TYPE_EIGHT.equals(playType)){
				if(betBall.split(",")[0].contains(results[0])&&betBall.split(",")[1].contains(results[1])&&betBall.split(",")[2].contains(results[2])
						&&betBall.split(",")[3].contains(results[3])&&betBall.split(",")[4].contains(results[4])){
					//五星通选一等奖
					winMoney = winMoney.add(betRate);
