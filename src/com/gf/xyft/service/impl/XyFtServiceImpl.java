...
import com.gf.xyft.util.XyFtUtil;
import com.ram.model.User;
import com.ram.service.user.IUserService;

public class XyFtServiceImpl extends BaseService implements IXyFtService {
	private IXyFtDAO gfXyFtDAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setXyFtDAO(IXyFtDAO gfXyFtDAO) {
//		gfXyFtDAO = gfXyFtDAO;
//		super.dao = gfXyFtDAO;
//	}

	public void setGfXyFtDAO(IXyFtDAO gfXyFtDAO) {
		this.gfXyFtDAO = gfXyFtDAO;
		super.dao = gfXyFtDAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_XYFT;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfXyFtDAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfXyFtDAO.updateObjectList(saveList, null);
			flag = "success";
			log.info("___[today["+today+"] init completed]__________________________"+code);
			GameHelpUtil.log("xyft", "___[today["+today+"] init completed]__________________________"+code);
		} else {
			flag = "inited";
			log.info("___[today["+today+"] has been inited]__________________________"+code);
			GameHelpUtil.log("xyft", "___[today["+today+"] has been inited]__________________________"+code);
		}
		return flag;
	}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfXyFtGaSession session = new GfXyFtGaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfXyFtGaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfXyFtGaSession>();
////			log.info("___[start today]__________________________");
//			
//			String startTimeStr = today + XyFtConstants.XY_FT_START_TIME_STR;//开始时间串
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
////			log.info("___[startTimeStr="+startTimeStr+"]");
//			
//			for (int i = 0; i < XyFtConstants.XY_FT_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*XyFtConstants.XY_FT_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfXyFtGaSession k10Session = new GfXyFtGaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,XyFtConstants.XY_FT_TIME_INTERVAL));
//				k10Session.setOpenStatus(XyFtConstants.XY_FT_OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				XyFtDAO.saveObject(k10Session);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfXyFtGaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfXyFtDAO.countObjects(hq);
	...
	public List<GfXyFtGaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfXyFtGaSession> sessionlist=new ArrayList<GfXyFtGaSession>();
		GfXyFtGaSession currentSession=gfXyFtDAO.getCurrentSession();
		GfXyFtGaSession tempsession =gfXyFtDAO.getPreviousSessionBySessionNo((Long.parseLong(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfXyFtGaSession) gfXyFtDAO.getObject(GfXyFtGaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfXyFtGaSession session =gfXyFtDAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(XyFtConstants.XY_FT_OPEN_STATUS_INIT) || openStatus1.equals(XyFtConstants.XY_FT_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(XyFtConstants.XY_FT_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfXyFtDAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_XYFT);					
					if(sessionInfo != null){
						...
						gfXyFtDAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfXyFtGaSession> updateAndOpenResult() {
//		List<GfXyFtGaSession> sessionlist=new ArrayList<GfXyFtGaSession>();
//		GfXyFtGaSession currentSession=gfXyFtDAO.getCurrentSession();
//		GfXyFtGaSession tempsession =gfXyFtDAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
//		}else{
//			tempsession=(GfXyFtGaSession) gfXyFtDAO.getObject(GfXyFtGaSession.class, (currentSession.getSessionId()-1));
//		}
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfxyft");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[XyFt start fetch result xml data...]________________---------XyFt");
//						GameHelpUtil.log("xyft", "___[XyFt start fetch result xml data...]________________---------XyFt");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[XyFt start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/XyFt.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("xyft", "XyFtServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("xyft", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("xyft", "XyFtServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateXyFtSessionOpenResultMethod(GfXyFtGaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_XYFT, Constants.PUB_STATUS_OPEN);
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							BigDecimal winMoney = XyFtUtil.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_XYFT);
			...
			gfXyFtDAO.updateObjectList(spList, null);
			gfXyFtDAO.updateObjectList(spDeList, null);
			gfXyFtDAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("xyft", "XyFtServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_XYFT);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_XYFT);
		...
		gfXyFtDAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfXyFtDAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_XYFT,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_XYFT,remark);
	}
	...
	public GfXyFtGaSession getCurrentSession(){
		return gfXyFtDAO.getCurrentSession();
	}
	public GfXyFtGaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfXyFtDAO.getPreviousSessionBySessionNo(sessionNo);
	}
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_XYFT);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfXyFtDAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_XYFT, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfXyFtDAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfXyFtGaSession GfXyFtGaSession = gfXyFtDAO.getCurrentSession(); // 当前期数信息
		if(GfXyFtGaSession != null){
			String curNo = GfXyFtGaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Long.parseLong(curNo) - 1)); //上期期号
			GfXyFtGaSession XyFtPreSession = getPreviousSessionBySessionNo(preNo);
			if(XyFtPreSession==null){
				XyFtPreSession=(GfXyFtGaSession) gfXyFtDAO.getObject(GfXyFtGaSession.class, GfXyFtGaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfXyFtGaTrend gdk10 ");
			List<Object> trendList = gfXyFtDAO.findObjects(hq); //上期冷热排行榜
			List<GfXyFtGaTrend> saveList = new ArrayList<GfXyFtGaTrend>(); // 本期冷热排行榜

			String results = XyFtPreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfXyFtGaTrend trend = (GfXyFtGaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfXyFtGaSession session,String openResult){
		...
		boolean flag=false;
		session.setOpenResult(openResult);
		gfXyFtDAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	...
	public boolean saveAndOpenResult(GfXyFtGaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateXyFtSessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("xyft", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfXyFtDAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfXyFtDTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfXyFtDAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfXyFtGaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_XYFT);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfXyFtGaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_XYFT);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_XYFT));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfXyFtDAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_XYFT) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_XYFT,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_XYFT,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfXyFtDAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_XyFt, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfXyFtGaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= XyFtDAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = XyFtUtil.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		//user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, betmoney, null,Constants.GAME_TYPE_GF_XYFT,remark);
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_XYFT,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfXyFtDAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_XyFt,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_XyFt);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_XyFt);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfXyFtGaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("xyft", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfXyFtGaSession session=XyFtDAO.getCurrentSession();
//		if(session!=null){
//			GfXyFtGaSession tempsession =XyFtDAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfXyFtGaOmit omit=gfXyFtDAO.getXyFtOmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfXyFtGaOmit preomit=gfXyFtDAO.getXyFtOmitBySessionNo((Long.parseLong(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfXyFtGaSession se = (GfXyFtGaSession) gfXyFtDAO.getObject(GfXyFtGaSession.class, tempsession.getSessionId()-1);
							preomit=gfXyFtDAO.getXyFtOmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfXyFtGaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfXyFtGaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("xyft", "XyFtServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfXyFtGaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("xyft", "XyFtServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("xyft", "XyFtServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("xyft", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("xyft", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("xyft", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_XYFT,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_XYFT,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_XYFT,remark);
	}
	...
	public GfXyFtGaSession getFirstSessionByDate(String date) {
		return gfXyFtDAO.getFirstSessionByDate(date);
	}
	public GfXyFtGaSession getEndSessionByDate(String date){
		return gfXyFtDAO.getEndSessionByDate(date);
	}
	public List<GfXyFtGaOmit> findGfXyFtGaOmitList(String string,
			List<Object> para, int num){
		return gfXyFtDAO.findGfXyFtGaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfXyFtGaSession tempsession) {
//		GfXyFtGaSession session=XyFtDAO.getCurrentSession();
//		if(session!=null){
//			GfXyFtGaSession tempsession =XyFtDAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_XYFT);
...
//			}
	 ...
	 * @return
	 */
	public String getTodaySessionNo(GfXyFtGaSession beforeSession,int index,String sessionNo1){
		if(ParamUtils.chkString(sessionNo1.trim())){
			return  (Long.parseLong(sessionNo1)+ index)+"";
		}else{
			return  (Long.parseLong(beforeSession.getSessionNo())+ index)+"";
		}	
	}
...
//			log.info("___[startTimeStr="+startTimeStr+"]");
			...
			HQUtils hq = new HQUtils("from GfXyFtGaSession gks where gks.startTime>? and gks.startTime<? order by gks.sessionId desc ");
			...
			hq.addPars(todayStart);
			hq.addPars(todayEnd);
			List<Object> list=gfXyFtDAO.findObjects(hq);
			GfXyFtGaSession beforeSession=null;
			if(list!=null&&list.size()>0){
				beforeSession=(GfXyFtGaSession) list.get(0);
			}
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean checkSessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfXyFtGaSession ho where ho.startTime>? and ho.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfXyFtDAO.countObjects(hq);
		return ParamUtils.chkInteger(count)?true:false;
	...
	public String updateInitSession(int days) {
		...
		if(!isSessionInit){
			...
			String startDateStr = theDay + XyFtConstants.XY_FT_START_TIME_STR;//开始时间串
			...
			Date endTime = null;
			for (int i = 0; i < XyFtConstants.XY_FT_MAX_PART; i++) {
				startTime = DateTimeUtil.getDateTimeOfSeconds(startDate,i*XyFtConstants.XY_FT_TIME_INTERVAL);
				endTime = DateTimeUtil.getDateTimeOfSeconds(startDate,(i+1)*XyFtConstants.XY_FT_TIME_INTERVAL);
				...
				session.setOpenStatus(XyFtConstants.XY_FT_OPEN_STATUS_INIT);
				saveList.add(session);
			}
			gfXyFtDAO.updateObjectList(saveList, null);
			flag = "success";
	...
	public boolean saveRevokePrize(GfXyFtGaSession session) {
		...
		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_XYFT,session.getSessionNo());
		if(result){
