package com.gf.ssc.xjssc.dao.hibernate;
...
implements IXjSscDAO {
	...
	public GfXjSscGaSession getCurrentSession() {
		...
		List<GfXjSscGaSession> list = getSession().createQuery("from GfXjSscGaSession gks where gks.startTime<=? and gks.endTime>? and (gks.openStatus=? or gks.openStatus=?) order by gks.sessionId")
		.setTimestamp(0, now)
		.setTimestamp(1, now)
		...
		if(list!=null && list.size()>0){
			return (GfXjSscGaSession)list.get(0);
		}
		return null;
	}
	...
	public GfXjSscGaSession getPreviousSessionBySessionNo(String sessionNo) {
		String  hql=" from GfXjSscGaSession bj3 where bj3.sessionNo=? ";
	...
	public List<GfXjSscGaTrend> findGaTrendList() {
		String hql = " from GfXjSscGaTrend trend order by trend.trendId desc";
		List<GfXjSscGaTrend> list;
	...
	public PaginationSupport findGaSessionList(String hqsl,List<Object> para,int pageNum,int pageSize) {
		Query query = makeQuerySQL( " from GfXjSscGaSession ho where 1=1 "+hqsl, para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfXjSscGaSession ho where 1=1 "+hqsl, para);
...
//	@Override
//	public List<GfXjSscGaSession> getCurrentSession(int start, int end) {
...
//		}
//		
//		List<GfXjSscGaSession> list = getSession().createQuery("from GfXjSscGaSession bj3s where bj3s.startTime>? and "
...
//		Query query = makeQuerySQL( " select new com.bj3.model.dto.FcBetDTO(se.sessionNo,ho.totalPoint,ho.winCash,se.startTime,se.endTime,ho.totalPoint-ho.winCash) from FcGaBet ho,GfXjSscGaSession se where 1=1 "
//				+ " and se.sessionId = ho.sessionId "+hql+" group by ho.sessionId order by ho.sessionId desc ", para);
//		query.setFirstResult(pageNum);
//		query.setMaxResults(pageSize);
//		List queList = query.list();
//		Query count = makeQuerySQL("select count(ho.betId) from FcGaBet ho, GfXjSscGaSession se where 1=1" +
...
//	}
	...
	public List<WinCoDTO> findGaWinCountList() {
	    Query query = getSession().createQuery(" select new com.gf.ssc.xjssc.model.dto.WinCoDTO(co,u) from GaWinCount co,User u where 1=1 and co.userId = u.userId order by co.totalMoney desc")
	    		.setMaxResults(10);
	...
	public GfXjSscGaOmit getFcGaOmitBySessionNo(String sessionNo){
		String hql = " from GfXjSscGaOmit ho where ho.sessionNo=? ";
		List<GfXjSscGaOmit> list;
	...
	@Override
	public List<GfXjSscGaOmit> findGfXjSscGaOmitList(String hqls, List<Object> para,
			int num) {
		String  hql=" from GfXjSscGaOmit ho where 1=1  ";
		Query query =makeQuerySQL(hql+hqls,para);
		query.setMaxResults(num);
		List<GfXjSscGaOmit>list=query.list();
		return list;
	}
	...
	public PaginationSupport findGameBetAndWinList(String hqls,
		...
		Query query = makeQuerySQL( " select new com.apps.model.dto.AgentDTO(se.sessionNo,se.startTime,se.endTime,sum(ho.money),sum(ho.winCash),sum(ho.money)-sum(ho.winCash)) from GaBetSponsor ho,GfXjSscGaSession se where 1=1 "
				+ " and ho.gameType ="+ Constants.GAME_TYPE_GF_XJSSC
				+ " and se.sessionId = ho.sessionId "+hqls+" group by ho.sessionId order by ho.sessionId desc ", para);
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GaBetSponsor ho,GfXjSscGaSession se where 1=1" 
				+ " and ho.gameType ="+ Constants.GAME_TYPE_GF_XJSSC
	...
	public PaginationSupport findGfXjSscGaSessionList(String hql,
			List<Object> para, int startIndex, int pageSize) {
		Query query = makeQuerySQL( " from GfXjSscGaSession ho where 1=1 "+hql,para);
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfXjSscGaSession ho where 1=1 "+hql, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
