package com.gf.ssc.jxssc.dao.hibernate;
...
public class JxSscDAOHibernate extends AbstractBaseDAOHibernate 
implements IJxSscDAO {
	...
	public GfJxSscGaSession getCurrentSession() {
		...
		List<GfJxSscGaSession> list = getSession().createQuery("from GfJxSscGaSession gks where gks.startTime<=? and gks.endTime>? and (gks.openStatus=? or gks.openStatus=?) order by gks.sessionId")
		.setTimestamp(0, now)
		.setTimestamp(1, now)
		...
		if(list!=null && list.size()>0){
			return (GfJxSscGaSession)list.get(0);
		}
		return null;
	}
	...
	public GfJxSscGaSession getPreviousSessionBySessionNo(String sessionNo) {
		String  hql=" from GfJxSscGaSession bj3 where bj3.sessionNo=? ";
	...
	public List<GfJxSscGaTrend> findGaTrendList() {
		String hql = " from GfJxSscGaTrend trend order by trend.trendId desc";
		List<GfJxSscGaTrend> list;
	...
	public PaginationSupport findGaSessionList(String hqsl,List<Object> para,int pageNum,int pageSize) {
		Query query = makeQuerySQL( " from GfJxSscGaSession ho where 1=1 "+hqsl, para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfJxSscGaSession ho where 1=1 "+hqsl, para);
...
//	@Override
//	public List<GfJxSscGaSession> getCurrentSession(int start, int end) {
...
//		}
//		
//		List<GfJxSscGaSession> list = getSession().createQuery("from GfJxSscGaSession bj3s where bj3s.startTime>? and "
...
//		Query query = makeQuerySQL( " select new com.bj3.model.dto.FcBetDTO(se.sessionNo,ho.totalPoint,ho.winCash,se.startTime,se.endTime,ho.totalPoint-ho.winCash) from FcGaBet ho,GfJxSscGaSession se where 1=1 "
//				+ " and se.sessionId = ho.sessionId "+hql+" group by ho.sessionId order by ho.sessionId desc ", para);
//		query.setFirstResult(pageNum);
//		query.setMaxResults(pageSize);
//		List queList = query.list();
//		Query count = makeQuerySQL("select count(ho.betId) from FcGaBet ho, GfJxSscGaSession se where 1=1" +
...
//	}
	...
	public List<WinCoDTO> findGaWinCountList() {
	    Query query = getSession().createQuery(" select new com.gf.ssc.jxssc.model.dto.WinCoDTO(co,u) from GaWinCount co,User u where 1=1 and co.userId = u.userId order by co.totalMoney desc")
	    		.setMaxResults(10);
	...
	public GfJxSscGaOmit getFcGaOmitBySessionNo(String sessionNo){
		String hql = " from GfJxSscGaOmit ho where ho.sessionNo=? ";
		List<GfJxSscGaOmit> list;
	...
	@Override
	public List<GfJxSscGaOmit> findGfJxSscGaOmitList(String hqls, List<Object> para,
			int num) {
		String  hql=" from GfJxSscGaOmit ho where 1=1  ";
		Query query =makeQuerySQL(hql+hqls,para);
		query.setMaxResults(num);
		List<GfJxSscGaOmit>list=query.list();
		return list;
	}
	...
	public PaginationSupport findGameBetAndWinList(String hqls,
		...
		Query query = makeQuerySQL( " select new com.apps.model.dto.AgentDTO(se.sessionNo,se.startTime,se.endTime,sum(ho.money),sum(ho.winCash),sum(ho.money)-sum(ho.winCash)) from GaBetSponsor ho,GfJxSscGaSession se where 1=1 "
				+ " and se.sessionId = ho.sessionId "+hqls+" group by ho.sessionId order by ho.sessionId desc ", para);
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.jointId) from GaBetSponsor ho,GfJxSscGaSession se where 1=1" +
				" and  se.sessionId = ho.sessionId "+hqls, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
	...
	public PaginationSupport findGfJxSscGaSessionList(String hql,
			List<Object> para, int startIndex, int pageSize) {
		Query query = makeQuerySQL( " from GfJxSscGaSession ho where 1=1 "+hql,para);
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfJxSscGaSession ho where 1=1 "+hql, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
