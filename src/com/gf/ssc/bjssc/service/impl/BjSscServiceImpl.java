...
import com.gf.ssc.bjssc.BjSscConstants;
import com.gf.ssc.bjssc.dao.IBjSscDAO;
...
public class BjSscServiceImpl  extends BaseService implements IBjSscService {
	private IBjSscDAO gfBjSscDAO;
	private IUserService userService;
	private IGaService gaService;
	
	public void setGfBjSscDAO(IBjSscDAO gfBjSscDAO) {
		this.gfBjSscDAO = gfBjSscDAO;
		super.dao = gfBjSscDAO;
...
//		List<GfBjSscGaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfBjSscGaSession>();
//			String startTimeStr = today + BjSscConstants.BJ_SSC_START_TIME_STR;//开始时间串
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
//			for (int i = 0; i < BjSscConstants.BJ_SSC_MAX_PART_NIGHT; i++) {//经观察得知
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*BjSscConstants.BJ_SSC_TIME_INTERVAL_NIGHT * 60 * 1000;
...
//				GfBjSscGaSession k10Session = new GfBjSscGaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,BjSscConstants.BJ_SSC_TIME_INTERVAL_NIGHT));
//				k10Session.setOpenStatus(GameConstants.OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				gfBjSscDAO.saveObject(k10Session);
//				saveList.add(k10Session);
//			}
//			startTimeStr = today + BjSscConstants.BJ_SSC_START_TIME_STR_DAY;//开始时间串
//			startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			for (int i = BjSscConstants.BJ_SSC_MAX_PART_NIGHT; i < BjSscConstants.BJ_SSC_MAX_PART_DAY; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + (i-BjSscConstants.BJ_SSC_MAX_PART_NIGHT)*BjSscConstants.BJ_SSC_TIME_INTERVAL_DAY * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				//String openResult = GameConstants.getRandomK10Result();//随机生成5个中奖数字[1-21]
//				String sessionNo = this.getTodaySessionNo(curSessionDate, i+1);//期号
...
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,BjSscConstants.BJ_SSC_TIME_INTERVAL_DAY));
//				k10Session.setOpenStatus(GameConstants.OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				gfBjSscDAO.saveObject(k10Session);
//				saveList.add(k10Session);
...
//			startTimeStr = today + BjSscConstants.BJ_SSC_START_TIME_STR_DAY;//开始时间串
//			startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			for (int i = BjSscConstants.BJ_SSC_MAX_PART_DAY; i < BjSscConstants.BJ_SSC_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + (i-BjSscConstants.BJ_SSC_MAX_PART_DAY)*BjSscConstants.BJ_SSC_TIME_INTERVAL_NIGHT * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				//String openResult = GameConstants.getRandomK10Result();//随机生成5个中奖数字[1-21]
//				String sessionNo = this.getTodaySessionNo(curSessionDate, i+1);//期号
...
//				saveList.add(k10Session);
//			}
//			gfBjSscDAO.updateObjectList(saveList, null);
//			flag = "success";
//		} else {
//			flag = "inited";
//		}
...
//	}
	...
	public boolean checkSessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfBjSscGaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfBjSscDAO.countObjects(hq);
	...
	public String updateInitSession(int days) {
		...
		if(!isSessionInit){
			...
			String startDateStr = theDay + BjSscConstants.BJ_SSC_START_TIME_STR;//开始时间串
			Date startDate = DateTimeUtil.strToDateMul(startDateStr);//开始时间

			String sessionNo = null;
			Date startTime = null;
			Date endTime = null;
			for (int i = 0; i < BjSscConstants.BJ_SSC_MAX_PART_NIGHT; i++) {
				startTime = DateTimeUtil.getDateTimeOfSeconds(startDate,i*BjSscConstants.BJ_SSC_TIME_INTERVAL);
				endTime = DateTimeUtil.getDateTimeOfSeconds(startDate,(i+1)*BjSscConstants.BJ_SSC_TIME_INTERVAL);
				sessionNo = this.getSessionNo(startDate, i+1);//期号
				
				GfBjSscGaSession session = new GfBjSscGaSession();
				session.setSessionNo(sessionNo);
				session.setStartTime(startTime);
				session.setEndTime(endTime);
				session.setOpenStatus(BjSscConstants.BJ_SSC_OPEN_STATUS_INIT);
				saveList.add(session);
			}
			
			startDateStr = theDay + BjSscConstants.BJ_SSC_START_TIME_STR_DAY;//白天开始时间串
			startDate = DateTimeUtil.strToDateMul(startDateStr);//白天开始时间
			for (int i = BjSscConstants.BJ_SSC_MAX_PART_NIGHT; i < BjSscConstants.BJ_SSC_MAX_PART; i++) {
				startTime = DateTimeUtil.getDateTimeOfSeconds(startDate,(i-BjSscConstants.BJ_SSC_MAX_PART_NIGHT)*BjSscConstants.BJ_SSC_TIME_INTERVAL);
				endTime = DateTimeUtil.getDateTimeOfSeconds(startDate,(i-BjSscConstants.BJ_SSC_MAX_PART_NIGHT+1)*BjSscConstants.BJ_SSC_TIME_INTERVAL);
				sessionNo = this.getSessionNo(startDate, i+1);//期号
	 ...
	 * @return
	 */
	public String updateBjSscSessionOpenResultMethod(GfBjSscGaSession fcsession,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(fcsession.getSessionNo(), 
					Constants.GAME_TYPE_GF_BJSSC, Constants.PUB_STATUS_OPEN);
				...
				for(int i = 0; i < list.size(); i++){
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							BigDecimal winMoney = BjSscUtil.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);//设为无效
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(part != null && part.size()>0){
							...
							if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
								...
								para2.add(Constants.GAME_TYPE_GF_BJSSC);
			...
			gfBjSscDAO.updateObjectList(spList, null);
			gfBjSscDAO.updateObjectList(spDeList, null);
			gfBjSscDAO.updateObjectList(paList, null);
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_BJSSC);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_BJSSC);
		...
		gfBjSscDAO.updateObjectList(gaWinCoList, null);
	}
...
// 	    	ManageFile.writeTextToFile("重庆时时彩paList》》》》》》》》》"+DateTimeUtil.DateToStringAll(new Date())+"  [sessionNO]"+paList.size()+"\n", Constants.getWebRootPath()+"/gamelogo/bjsscjilu.txt", true);
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfBjSscDAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_BJSSC,remark,
	...
	public GfBjSscGaSession getCurrentSession() {
		return gfBjSscDAO.getCurrentSession();
	}
	
	public GfBjSscGaSession getPreviousSessionBySessionNo(String sessionNo) {
		return gfBjSscDAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	@Override
	public PaginationSupport findGaSessionList(String hql,
			List<Object> para, int pageNum, int pageSize) {
		return gfBjSscDAO.findGaSessionList(hql,para,pageNum,pageSize);
	}
		 
	 public void updateFetchAndOpenTrendResult(GfBjSscGaSession session){
//			String lastSessionNo=(Integer.parseInt(gfBjSscDAO.getCurrentSession().getSessionNo())-1)+"";
//			GfBjSscGaSession session =gfBjSscDAO.getPreviousSessionBySessionNo(lastSessionNo);
...
//			}
			if(session.getOpenStatus().equals(Constants.OPEN_STATUS_OPENED)){
				List<GfBjSscGaTrend> list=gfBjSscDAO.findGaTrendList();
				List<GfBjSscGaTrend> savelist=new ArrayList<GfBjSscGaTrend>();
				Map<String,Boolean> map=getTrendResult(session.getOpenResult());
				if(!map.isEmpty()){
					for(int i=0;i<list.size();i++){
						GfBjSscGaTrend trend=list.get(i);
...
//					session=null;
		...
		public List<GfBjSscGaTrend> findFcTrendList() {
			return gfBjSscDAO.findGaTrendList();
		}
		
		@Override
		public User saveSponsorBet(GfBjSscGaSession se,int multiple,User user, List<String> list,
			...
			batchNum = System.currentTimeMillis() +BjSscUtil.getTwo();
			String orderNum = "";

			//方案总金额
			BigDecimal money = new BigDecimal(betNum *2*multiple);
			List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_BJSSC);
			...
			sp.setGameType(Constants.GAME_TYPE_GF_BJSSC);
			sp.setGameName(BjSscConstants.GAME_NAME);
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfBjSscDAO.saveObjectDB(sp);
			orderNum = se.getSessionNo() + "BJSSC" +sp.getJointId();
			sp.setOrderNum(orderNum);
			gfBjSscDAO.saveObject(sp);
			...
			if(Constants.GUARANTEE.equals(isGuaranteed)){//保底
				...
				user=userService.saveTradeDetail(user, user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, guaranteeMoney, sp.getJointId(), Constants.GAME_TYPE_GF_BJSSC, remark.toString());
			}
			StringBuilder remark = new StringBuilder();
			remark.append("订单:").append(sp.getOrderNum()).append(",购买彩票 扣款 ")
			    .append(lotteryMoney).append("元");
			user = userService.saveTradeDetail(user, user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, lotteryMoney, pa.getRid(), Constants.GAME_TYPE_GF_BJSSC, remark.toString());
		...
		public User saveProcurementServiceBet(User user,
			...
			for (String key : seMap.keySet()) {
				...
				GfBjSscGaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
				String orderNum = "";
...
//				sp.setIsGuarantee(FcConstants.NOT_GUARANTEE);//不保底
				...
				sp = (GaBetSponsor) gfBjSscDAO.saveObjectDB(sp);
				orderNum = sessionNo + "BJSSC" +sp.getJointId();
				sp.setOrderNum(orderNum);
			    saveList.add(sp);
			  ...
			  //关联构造
				...
				for(String op : list){
					...
					int betCount = BjSscUtil.getTotalBetNum(playType+"",betBall);//投注注数
				...
				if(isFirst){
					...
					tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
							...
							Constants.GAME_TYPE_GF_BJSSC,remark.toString(), sessionNo, user.getUserType(),user.getLoginName());
			...
			gfBjSscDAO.updateObjectList(rlList, null);
		...
		public List<WinCoDTO> findGaWinCountList() {
			return gfBjSscDAO.findGaWinCountList();
		}
		
		@Override
		public void updateFetchAndOpenOmit(GfBjSscGaSession tempsession){
//			GfBjSscGaSession session=gfBjSscDAO.getCurrentSession();
//			if(session!=null){
//				GfBjSscGaSession tempsession =gfBjSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
				if(tempsession!=null){
					if(ParamUtils.chkString(tempsession.getOpenResult())){
						...
						GfBjSscGaOmit omit=gfBjSscDAO.getFcGaOmitBySessionNo(tempsession.getSessionNo());
						if(omit==null){
							GfBjSscGaOmit preomit=gfBjSscDAO.getFcGaOmitBySessionNo((Integer.parseInt(tempsession.getSessionNo())-1)+"");
							if(preomit == null){
								GfBjSscGaSession se = (GfBjSscGaSession) gfBjSscDAO.getObject(GfBjSscGaSession.class, tempsession.getSessionId()-1);
								preomit=gfBjSscDAO.getFcGaOmitBySessionNo(se.getSessionNo());
							}
							if(preomit!=null){
								omit=new GfBjSscGaOmit();
								...
								Field f =null;
								try {
									f= GfBjSscGaOmit.class.getDeclaredField("ge"+Integer.parseInt(array[4]));
									f.setAccessible(true);  
									f.set(omit, 0);
									f= GfBjSscGaOmit.class.getDeclaredField("shi"+Integer.parseInt(array[3]));
									f.setAccessible(true);  
									f.set(omit, 0);
									f= GfBjSscGaOmit.class.getDeclaredField("bai"+Integer.parseInt(array[2]));
									f.setAccessible(true);  
									f.set(omit, 0);
									f= GfBjSscGaOmit.class.getDeclaredField("qian"+Integer.parseInt(array[1]));
									f.setAccessible(true);  
									f.set(omit, 0);
									f= GfBjSscGaOmit.class.getDeclaredField("wan"+Integer.parseInt(array[0]));
									f.setAccessible(true);  
									f.set(omit, 0);
									for(int i=0;i<array.length;i++){
										f= GfBjSscGaOmit.class.getDeclaredField("zu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
									}
								} catch (Exception e) {
...
//			}
		...
		public List<GfBjSscGaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
			List<GfBjSscGaSession> sessionlist = new ArrayList<GfBjSscGaSession>();
			GfBjSscGaSession crrrentsession =gfBjSscDAO.getCurrentSession();
			//待开奖场次
			GfBjSscGaSession tempsession =gfBjSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(crrrentsession.getSessionNo())-1)+"");	
			if(tempsession==null){
				tempsession=(GfBjSscGaSession) gfBjSscDAO.getObject(GfBjSscGaSession.class, (crrrentsession.getSessionId()-1));
			}
			final String  lastSessionNo=tempsession.getSessionNo();//待开奖场次
			
			if(!sessionNoMap.isEmpty()){
				for(String key:sessionNoMap.keySet()){
					GfBjSscGaSession session =gfBjSscDAO.getPreviousSessionBySessionNo(key);
					if(session!=null){
				...
				if(sessionInfo!=null){
					...
					gfBjSscDAO.updateObject(sessionInfo, null);
...
//		@Override
//		public List<GfBjSscGaSession> updateAndOpenResult() {
//			List<GfBjSscGaSession> sessionlist = new ArrayList<GfBjSscGaSession>();
//			GfBjSscGaSession crrrentsession =gfBjSscDAO.getCurrentSession();
...
//
//		               		//从这里 ---------------------------------------------------------------------------
//	            		    GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfbjssc");
//	            		    String officalURL ="";
...
//	            		    }
//		               	    try{
////		               	    	ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(DateTimeUtil.getNowSQLDate())+"___[bjssc start fetch result xml data...]________________", Constants.getWebRootPath()+"/gamelogo/bjssc.txt", true);
//		               	    }catch(Exception e){
//		               	    	e.printStackTrace();
//		               	    }
...
//		               		//到这里 ---------------------------------------------------------------------------		
//	            		    sleep(3000);
////	            		    ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(DateTimeUtil.getNowSQLDate())+resultXML, Constants.getWebRootPath()+"/gamelogo/bjssc.txt", true);
...
//						sessionInfo.setGameType(Constants.GAME_TYPE_GF_BJSSC);
//					}
//					sessionInfo.setLatestSessionNo(crrrentsession.getSessionNo());
...
//		}
		...
		public List<GfBjSscGaOmit> findGfBjSscGaOmitList(String hql,
				List<Object> para, int num) {
			return gfBjSscDAO.findGfBjSscGaOmitList(hql,para,num);

		}

		public PaginationSupport findGfBjSscGaSessionList(String hql,
				List<Object> para, int startIndex, int pageSize) {
			return gfBjSscDAO.findGfBjSscGaSessionList(hql,para,startIndex,pageSize);
		}			
		...
		public PaginationSupport findGameBetAndWinList(String hqls,
				List<Object> para, int startIndex, int pageSize) {
			return  gfBjSscDAO.findGameBetAndWinList(hqls,para,startIndex,pageSize);
		}
		
		@Override
		public String saveAndOpenResult(GfBjSscGaSession session,
				String openResult) {
			String buffer="";
			String flag = "";
			if(ParamUtils.chkString(openResult)){
				...
				session.setOpenResult(openResult);		
				flag = updateBjSscSessionOpenResultMethod(session, session.getOpenResult(),null);
				if(!ParamUtils.chkString(flag)){      
					session.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
					session.setOpenStatus(Constants.OPEN_STATUS_OPENED);
					gfBjSscDAO.updateObject(session, null);
...
//					log.info("___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		...
		@Override
		public void updateDayBetCount(GfBjSscGaSession tempsession) {
//			GfBjSscGaSession session=gfBjSscDAO.getCurrentSession();
//			if(session!=null){
//				GfBjSscGaSession tempsession =gfBjSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
				if(tempsession!=null){
					...
					//如果上期已开奖
						...
						spPara.add(Constants.GAME_TYPE_GF_BJSSC);
...
//			}					
		...
		private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
			userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_BJSSC,remark);
		}
		
		
		private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
			userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_BJSSC,remark);
		}
		
		
		private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
				String remark) {//退回参与用户金额
			userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_BJSSC,remark);
		}
		@Override
		public boolean saveRevokePrize(GfBjSscGaSession session) {

			boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_BJSSC,session.getSessionNo());
			if(result){
