package com.gf.ssc.bjssc.dao.hibernate;
...
implements IBjSscDAO {
	...
	public GfBjSscGaSession getCurrentSession() {
		...
		List<GfBjSscGaSession> list = getSession().createQuery("from GfBjSscGaSession gks where gks.startTime<=? and gks.endTime>? and (gks.openStatus=? or gks.openStatus=?) order by gks.sessionId")
		.setTimestamp(0, now)
		.setTimestamp(1, now)
		...
		if(list!=null && list.size()>0){
			return (GfBjSscGaSession)list.get(0);
		}
		return null;
	}
	...
	public GfBjSscGaSession getPreviousSessionBySessionNo(String sessionNo) {
		String  hql=" from GfBjSscGaSession bj3 where bj3.sessionNo=? ";
	...
	public List<GfBjSscGaTrend> findGaTrendList() {
		String hql = " from GfBjSscGaTrend trend order by trend.trendId desc";
		List<GfBjSscGaTrend> list;
	...
	public PaginationSupport findGaSessionList(String hqsl,List<Object> para,int pageNum,int pageSize) {
		Query query = makeQuerySQL( " from GfBjSscGaSession ho where 1=1 "+hqsl, para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfBjSscGaSession ho where 1=1 "+hqsl, para);
...
//	@Override
//	public List<GfBjSscGaSession> getCurrentSession(int start, int end) {
...
//		}
//		
//		List<GfBjSscGaSession> list = getSession().createQuery("from GfBjSscGaSession bj3s where bj3s.startTime>? and "
...
//		Query query = makeQuerySQL( " select new com.bj3.model.dto.FcBetDTO(se.sessionNo,ho.totalPoint,ho.winCash,se.startTime,se.endTime,ho.totalPoint-ho.winCash) from FcGaBet ho,GfBjSscGaSession se where 1=1 "
//				+ " and se.sessionId = ho.sessionId "+hql+" group by ho.sessionId order by ho.sessionId desc ", para);
//		query.setFirstResult(pageNum);
//		query.setMaxResults(pageSize);
//		List queList = query.list();
//		Query count = makeQuerySQL("select count(ho.betId) from FcGaBet ho, GfBjSscGaSession se where 1=1" +
...
//	}
	...
	public List<WinCoDTO> findGaWinCountList() {
	    Query query = getSession().createQuery(" select new com.gf.ssc.bjssc.model.dto.WinCoDTO(co,u) from GaWinCount co,User u where 1=1 and co.userId = u.userId order by co.totalMoney desc")
	    		.setMaxResults(10);
	...
	public GfBjSscGaOmit getFcGaOmitBySessionNo(String sessionNo){
		String hql = " from GfBjSscGaOmit ho where ho.sessionNo=? ";
		List<GfBjSscGaOmit> list;
	...
	@Override
	public List<GfBjSscGaOmit> findGfBjSscGaOmitList(String hqls, List<Object> para,
			int num) {
		String  hql=" from GfBjSscGaOmit ho where 1=1  ";
		Query query =makeQuerySQL(hql+hqls,para);
		query.setMaxResults(num);
		List<GfBjSscGaOmit>list=query.list();
		return list;
	}
	...
	public PaginationSupport findGameBetAndWinList(String hqls,
		...
		Query query = makeQuerySQL( " select new com.apps.model.dto.AgentDTO(se.sessionNo,se.startTime,se.endTime,sum(ho.money),sum(ho.winCash),sum(ho.money)-sum(ho.winCash)) from GaBetSponsor ho,GfBjSscGaSession se where 1=1 "
				+ " and ho.gameType ="+ Constants.GAME_TYPE_GF_BJSSC
				+ " and se.sessionId = ho.sessionId "+hqls+" group by ho.sessionId order by ho.sessionId desc ", para);
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GaBetSponsor ho,GfBjSscGaSession se where 1=1" 
				+ " and ho.gameType ="+ Constants.GAME_TYPE_GF_BJSSC
	...
	public PaginationSupport findGfBjSscGaSessionList(String hql,
			List<Object> para, int startIndex, int pageSize) {
		Query query = makeQuerySQL( " from GfBjSscGaSession ho where 1=1 "+hql,para);
		query.setFirstResult(startIndex);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GfBjSscGaSession ho where 1=1 "+hql, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
