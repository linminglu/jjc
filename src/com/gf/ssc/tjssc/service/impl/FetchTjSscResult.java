...
//    	
//    	if(!Constants.getTimerOpen("tjssc.fetchResult.gf")) return;
//    	
//    	log.info("-------重庆时时彩 ---开始开奖---"+DateTimeUtil.DateToStringAll(new Date()));
//    	
//		List<GfTjSscGaSession> list=gfTjSscService.updateAndOpenResult();
//
//		Collections.sort(list,new Comparator<GfTjSscGaSession>(){
//			public int compare(GfTjSscGaSession o1, GfTjSscGaSession o2){
...
//		});  
//
//		List<GfTjSscGaSession> savelist=new ArrayList<GfTjSscGaSession>();
//		for(GfTjSscGaSession session:list){
//			ManageFile.writeTextToFile("[sessionNo]"+session.getSessionNo()+"\n", Constants.getWebRootPath()+"/gamelogo/tjsscjilu.txt", true);
////			List<Object> para = new ArrayList<Object>();
//			
//			gfTjSscService.updateFetchAndOpenTrendResult(session);
//
//			String flag = gfTjSscService.updateTjSscSessionOpenResultMethod(session, session.getOpenResult(),null);
//			
//			if(ParamUtils.chkString(flag)){
//				session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
//				savelist.add(session);
//			}else{
//				gfTjSscService.updateDayBetCount(session);
//			}
//			gfTjSscService.updateFetchAndOpenOmit(session);
//		}
//		gfTjSscService.updateObjectList(savelist, null);
...
//    }
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_TJSSC);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_TJSSC);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_TJSSC)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_TJSSC);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_TJSSC);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfTjSscGaSession curSession = gfTjSscService.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfTjSscGaSession openSession = (GfTjSscGaSession)gfTjSscService.getObject(GfTjSscGaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfTjSscGaSession> list = (List<GfTjSscGaSession>) gfTjSscService.updateAndOpenResult(sessionNoMap);
		        			Collections.sort(list,new Comparator<GfTjSscGaSession>(){
		        				public int compare(GfTjSscGaSession o1, GfTjSscGaSession o2){
		        			    	if(o1.getSessionId() < o2.getSessionId()){return -1;}  
		        			    	if(o1.getSessionId() == o2.getSessionId()){return 0;}  
