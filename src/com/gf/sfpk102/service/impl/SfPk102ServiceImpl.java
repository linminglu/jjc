...
import com.gf.sfpk102.util.SfPk102Util;
import com.ram.model.User;
import com.ram.service.user.IUserService;

public class SfPk102ServiceImpl extends BaseService implements ISfPk102Service {
	private ISfPk102DAO gfSfPk102DAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setSfPk102DAO(ISfPk102DAO gfSfPk102DAO) {
//		gfSfPk102DAO = gfSfPk102DAO;
//		super.dao = gfSfPk102DAO;
//	}

	public void setGfSfPk102DAO(ISfPk102DAO gfSfPk102DAO) {
		this.gfSfPk102DAO = gfSfPk102DAO;
		super.dao = gfSfPk102DAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_SFPK102;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfSfPk102DAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfSfPk102DAO.updateObjectList(saveList, null);
			flag = "success";
			log.info("___[today["+today+"] init completed]__________________________"+code);
			GameHelpUtil.log("sfpk102", "___[today["+today+"] init completed]__________________________"+code);
		} else {
			flag = "inited";
			log.info("___[today["+today+"] has been inited]__________________________"+code);
			GameHelpUtil.log("sfpk102", "___[today["+today+"] has been inited]__________________________"+code);
		}
		return flag;
	}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfSfPk102GaSession session = new GfSfPk102GaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfSfPk102GaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfSfPk102GaSession>();
////			log.info("___[start today]__________________________");
//			
//			String startTimeStr = today + SfPk102Constants.SF_PK102_START_TIME_STR;//开始时间串
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
...
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*SfPk102Constants.SF_PK102_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfSfPk102GaSession k10Session = new GfSfPk102GaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,SfPk102Constants.SF_PK102_TIME_INTERVAL));
//				k10Session.setOpenStatus(SfPk102Constants.SF_PK102_OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				SfPk102DAO.saveObject(k10Session);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfSfPk102GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfSfPk102DAO.countObjects(hq);
	...
	public List<GfSfPk102GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfSfPk102GaSession> sessionlist=new ArrayList<GfSfPk102GaSession>();
		GfSfPk102GaSession currentSession=gfSfPk102DAO.getCurrentSession();
		GfSfPk102GaSession tempsession =gfSfPk102DAO.getPreviousSessionBySessionNo((Long.parseLong(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfSfPk102GaSession) gfSfPk102DAO.getObject(GfSfPk102GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfSfPk102GaSession session =gfSfPk102DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(SfPk102Constants.SF_PK102_OPEN_STATUS_INIT) || openStatus1.equals(SfPk102Constants.SF_PK102_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(SfPk102Constants.SF_PK102_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfSfPk102DAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_SFPK102);					
					if(sessionInfo != null){
						...
						gfSfPk102DAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfSfPk102GaSession> updateAndOpenResult() {
//		List<GfSfPk102GaSession> sessionlist=new ArrayList<GfSfPk102GaSession>();
//		GfSfPk102GaSession currentSession=gfSfPk102DAO.getCurrentSession();
//		GfSfPk102GaSession tempsession =gfSfPk102DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
//		}else{
//			tempsession=(GfSfPk102GaSession) gfSfPk102DAO.getObject(GfSfPk102GaSession.class, (currentSession.getSessionId()-1));
//		}
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfsfpk102");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[SfPk102 start fetch result xml data...]________________---------SfPk102");
//						GameHelpUtil.log("sfpk102", "___[SfPk102 start fetch result xml data...]________________---------SfPk102");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[SfPk102 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/SfPk102.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("sfpk102", "SfPk102ServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("sfpk102", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("sfpk102", "SfPk102ServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateSfPk102SessionOpenResultMethod(GfSfPk102GaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_SFPK102, Constants.PUB_STATUS_OPEN);
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							BigDecimal winMoney = SfPk102Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_SFPK102);
			...
			gfSfPk102DAO.updateObjectList(spList, null);
			gfSfPk102DAO.updateObjectList(spDeList, null);
			gfSfPk102DAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("sfpk102", "SfPk102ServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_SFPK102);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_SFPK102);
		...
		gfSfPk102DAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfSfPk102DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_SFPK102,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_SFPK102,remark);
	}
	...
	public GfSfPk102GaSession getCurrentSession(){
		return gfSfPk102DAO.getCurrentSession();
	}
	public GfSfPk102GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfSfPk102DAO.getPreviousSessionBySessionNo(sessionNo);
	}
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_SFPK102);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfSfPk102DAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_SFPK102, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfSfPk102DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfSfPk102GaSession GfSfPk102GaSession = gfSfPk102DAO.getCurrentSession(); // 当前期数信息
		if(GfSfPk102GaSession != null){
			String curNo = GfSfPk102GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Long.parseLong(curNo) - 1)); //上期期号
			GfSfPk102GaSession SfPk102PreSession = getPreviousSessionBySessionNo(preNo);
			if(SfPk102PreSession==null){
				SfPk102PreSession=(GfSfPk102GaSession) gfSfPk102DAO.getObject(GfSfPk102GaSession.class, GfSfPk102GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfSfPk102GaTrend gdk10 ");
			List<Object> trendList = gfSfPk102DAO.findObjects(hq); //上期冷热排行榜
			List<GfSfPk102GaTrend> saveList = new ArrayList<GfSfPk102GaTrend>(); // 本期冷热排行榜

			String results = SfPk102PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfSfPk102GaTrend trend = (GfSfPk102GaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfSfPk102GaSession session,String openResult){
		...
		boolean flag=false;
		session.setOpenResult(openResult);
		gfSfPk102DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	...
	public boolean saveAndOpenResult(GfSfPk102GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateSfPk102SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("sfpk102", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfSfPk102DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfSfPk102DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfSfPk102DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfSfPk102GaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_SFPK102);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfSfPk102GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_SFPK102);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_SFPK102));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfSfPk102DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_SFPK102) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_SFPK102,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_SFPK102,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfSfPk102DAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_SfPk102, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfSfPk102GaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= SfPk102DAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = SfPk102Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		//user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, betmoney, null,Constants.GAME_TYPE_GF_SFPK102,remark);
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_SFPK102,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfSfPk102DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_SfPk102,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_SfPk102);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_SfPk102);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfSfPk102GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("sfpk102", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfSfPk102GaSession session=SfPk102DAO.getCurrentSession();
//		if(session!=null){
//			GfSfPk102GaSession tempsession =SfPk102DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfSfPk102GaOmit omit=gfSfPk102DAO.getSfPk102OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfSfPk102GaOmit preomit=gfSfPk102DAO.getSfPk102OmitBySessionNo((Long.parseLong(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfSfPk102GaSession se = (GfSfPk102GaSession) gfSfPk102DAO.getObject(GfSfPk102GaSession.class, tempsession.getSessionId()-1);
							preomit=gfSfPk102DAO.getSfPk102OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfSfPk102GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfSfPk102GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("sfpk102", "SfPk102ServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfSfPk102GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("sfpk102", "SfPk102ServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							gfSfPk102DAO.saveObjectDB(omit);
						}else{
							Map<String,String> map=new HashMap<String,String>();
							for(int i=1;i<=11;i++){
								map.put(""+i, i+"");
							...
							try {
								...
								GameHelpUtil.log("sfpk102", "SfPk102ServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("sfpk102", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("sfpk102", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("sfpk102", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_SFPK102,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_SFPK102,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_SFPK102,remark);
	}
	...
	public GfSfPk102GaSession getFirstSessionByDate(String date) {
		return gfSfPk102DAO.getFirstSessionByDate(date);
	}
	public GfSfPk102GaSession getEndSessionByDate(String date){
		return gfSfPk102DAO.getEndSessionByDate(date);
	}
	public List<GfSfPk102GaOmit> findGfSfPk102GaOmitList(String string,
			List<Object> para, int num){
		return gfSfPk102DAO.findGfSfPk102GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfSfPk102GaSession tempsession) {
//		GfSfPk102GaSession session=SfPk102DAO.getCurrentSession();
//		if(session!=null){
//			GfSfPk102GaSession tempsession =SfPk102DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_SFPK102);
...
//			}
	 ...
	 * @return
	 */
	public String getTodaySessionNo(GfSfPk102GaSession beforeSession,int index,String sessionNo1){
	...
	public String updateInitTodaySession() {
		...
		if(!isTodaySessionInit){
			...
			for (int i = 0; i < SfPk102Constants.SF_PK102_MAX_PART; i++) {
				//计算出当前场次的时间
				Date startTime=DateTimeUtil.getDateTimeOfSeconds(startDate,SfPk102Constants.SF_PK102_TIME_INTERVAL*i);
				Date endTime=DateTimeUtil.getDateTimeOfSeconds(startTime,SfPk102Constants.SF_PK102_TIME_INTERVAL);
	...
	public boolean saveRevokePrize(GfSfPk102GaSession session) {
		...
		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_SFPK102,session.getSessionNo());
		if(result){
