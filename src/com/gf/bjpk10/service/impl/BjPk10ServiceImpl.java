...
public class BjPk10ServiceImpl extends BaseService implements IBjPk10Service {
	private IBjPk10DAO gfBjPk10DAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setBjPk10DAO(IBjPk10DAO gfBjPk10DAO) {
//		gfBjPk10DAO = gfBjPk10DAO;
//		super.dao = gfBjPk10DAO;
//	}

	public void setGfBjPk10DAO(IBjPk10DAO gfBjPk10DAO) {
		this.gfBjPk10DAO = gfBjPk10DAO;
		super.dao = gfBjPk10DAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_BJPK10;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfBjPk10DAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfBjPk10DAO.updateObjectList(saveList, null);
			flag = "success";
			log.info("___[today["+today+"] init completed]__________________________"+code);
			GameHelpUtil.log("bjpk10", "___[today["+today+"] init completed]__________________________"+code);
		} else {
			flag = "inited";
			log.info("___[today["+today+"] has been inited]__________________________"+code);
			GameHelpUtil.log("bjpk10", "___[today["+today+"] has been inited]__________________________"+code);
		}
		return flag;
	}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfBjPk10GaSession session = new GfBjPk10GaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfBjPk10GaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfBjPk10GaSession>();
...
//			
//			for (int i = 0; i < BjPk10Constants.BJ_PK10_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*BjPk10Constants.BJ_PK10_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfBjPk10GaSession k10Session = new GfBjPk10GaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,BjPk10Constants.BJ_PK10_TIME_INTERVAL));
//				k10Session.setOpenStatus(BjPk10Constants.BJ_PK10_OPEN_STATUS_INIT);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfBjPk10GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfBjPk10DAO.countObjects(hq);
	...
	public String updateFetchAndOpenResult_(Map<String, SessionItem> sessionNoMap) {
		if(sessionNoMap==null || sessionNoMap.isEmpty()) return "fail::no open sessionNo";
		//当前场次及开奖场次处理
		GfBjPk10GaSession curtSession = gfBjPk10DAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		GfBjPk10GaSession lastSession = (GfBjPk10GaSession)gfBjPk10DAO.getObject(GfBjPk10GaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<GfBjPk10GaSession> openedList = new ArrayList<GfBjPk10GaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				GfBjPk10GaSession session = gfBjPk10DAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = false;//updateBjPk10SessionOpenResultMethod(session,result,null);
						if(flag){
							...
							gfBjPk10DAO.saveObject(session);
							openedList.add(session);
						}
						GameHelpUtil.log(gameCode,"End ... ["+sNo+"],status="+session.getOpenStatus()+",result="+result);
					}
					// 把资金明细里投注记录状态值改为有效
					userService.updateUserTradeDetailStatus(session.getSessionNo(), 
							Constants.GAME_TYPE_GF_BJPK10, Constants.PUB_STATUS_OPEN);
				}else{
			...
			//更新彩种场次状态
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_BJPK10);
			if(sessionInfo!=null){
				...
				gfBjPk10DAO.saveObject(sessionInfo);
				CacheUtil.updateGameList();
				GameHelpUtil.log(gameCode,"gsi last ... "+lastSessionNo+","+lastItem.getResult());
			}
			
			//更新走势 上面已成功开奖的场次
			long startTrending = System.currentTimeMillis();
			for(GfBjPk10GaSession session:openedList){
	...
	public List<GfBjPk10GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfBjPk10GaSession> sessionlist=new ArrayList<GfBjPk10GaSession>();
		GfBjPk10GaSession currentSession=gfBjPk10DAO.getCurrentSession();
		GfBjPk10GaSession tempsession =gfBjPk10DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfBjPk10GaSession) gfBjPk10DAO.getObject(GfBjPk10GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfBjPk10GaSession session =gfBjPk10DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(BjPk10Constants.BJ_PK10_OPEN_STATUS_INIT) || openStatus1.equals(BjPk10Constants.BJ_PK10_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(BjPk10Constants.BJ_PK10_OPEN_STATUS_OPENED);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_BJPK10);					
					if(sessionInfo != null){
						SessionItem lastItem = (SessionItem)sessionNoMap.get(lastSessionNo);//上一期
						if(lastItem!=null){
							sessionInfo.setOpenResult(lastItem.getResult());
						...
						gfBjPk10DAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfBjPk10GaSession> updateAndOpenResult() {
//		List<GfBjPk10GaSession> sessionlist=new ArrayList<GfBjPk10GaSession>();
//		GfBjPk10GaSession currentSession=gfBjPk10DAO.getCurrentSession();
//		GfBjPk10GaSession tempsession =gfBjPk10DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfbjpk10");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[BjPk10 start fetch result xml data...]________________---------BjPk10");
//						GameHelpUtil.log("bjpk10", "___[BjPk10 start fetch result xml data...]________________---------BjPk10");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[BjPk10 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/BjPk10.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("bjpk10", "BjPk10ServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("bjpk10", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("bjpk10", "BjPk10ServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateBjPk10SessionOpenResultMethod(GfBjPk10GaSession session,String result,String orderNum){
		try{
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							BigDecimal winMoney = BjPk10Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_BJPK10);
			...
			gfBjPk10DAO.updateObjectList(spList, null);
			gfBjPk10DAO.updateObjectList(spDeList, null);
			gfBjPk10DAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("bjpk10", "BjPk10ServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_BJPK10);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_BJPK10);
		...
		gfBjPk10DAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfBjPk10DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_BJPK10,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_BJPK10,remark);
	}
	...
	public GfBjPk10GaSession getCurrentSession(){
		return gfBjPk10DAO.getCurrentSession();
	}
	public GfBjPk10GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfBjPk10DAO.getPreviousSessionBySessionNo(sessionNo);
	}
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_BJPK10);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfBjPk10DAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_BJPK10, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfBjPk10DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfBjPk10GaSession GfBjPk10GaSession = gfBjPk10DAO.getCurrentSession(); // 当前期数信息
		if(GfBjPk10GaSession != null){
			String curNo = GfBjPk10GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
			GfBjPk10GaSession BjPk10PreSession = getPreviousSessionBySessionNo(preNo);
			if(BjPk10PreSession==null){
				BjPk10PreSession=(GfBjPk10GaSession) gfBjPk10DAO.getObject(GfBjPk10GaSession.class, GfBjPk10GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfBjPk10GaTrend gdk10 ");
			List<Object> trendList = gfBjPk10DAO.findObjects(hq); //上期冷热排行榜
			List<GfBjPk10GaTrend> saveList = new ArrayList<GfBjPk10GaTrend>(); // 本期冷热排行榜

			String results = BjPk10PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfBjPk10GaTrend trend = (GfBjPk10GaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfBjPk10GaSession session,String openResult){
		...
		boolean flag=false;
		session.setOpenResult(openResult);
		gfBjPk10DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	...
	public boolean saveAndOpenResult(GfBjPk10GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateBjPk10SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("bjpk10", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfBjPk10DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfBjPk10DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfBjPk10DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfBjPk10GaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_BJPK10);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfBjPk10GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_BJPK10);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_BJPK10));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfBjPk10DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_BJPK10) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_BJPK10,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_BJPK10,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfBjPk10DAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_BjPk10, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfBjPk10GaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= BjPk10DAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = BjPk10Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		//user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, betmoney, null,Constants.GAME_TYPE_GF_JSFT,remark);
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_BJPK10,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfBjPk10DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_BjPk10,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_BjPk10);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_BjPk10);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfBjPk10GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("bjpk10", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfBjPk10GaSession session=BjPk10DAO.getCurrentSession();
//		if(session!=null){
//			GfBjPk10GaSession tempsession =BjPk10DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfBjPk10GaOmit omit=gfBjPk10DAO.getBjPk10OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfBjPk10GaOmit preomit=gfBjPk10DAO.getBjPk10OmitBySessionNo((Integer.parseInt(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfBjPk10GaSession se = (GfBjPk10GaSession) gfBjPk10DAO.getObject(GfBjPk10GaSession.class, tempsession.getSessionId()-1);
							preomit=gfBjPk10DAO.getBjPk10OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfBjPk10GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfBjPk10GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("bjpk10", "BjPk10ServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfBjPk10GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("bjpk10", "BjPk10ServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("bjpk10", "BjPk10ServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("bjpk10", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("bjpk10", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("bjpk10", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_BJPK10,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_BJPK10,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_BJPK10,remark);
	}
	...
	public GfBjPk10GaSession getFirstSessionByDate(String date) {
		return gfBjPk10DAO.getFirstSessionByDate(date);
	}
	public GfBjPk10GaSession getEndSessionByDate(String date){
		return gfBjPk10DAO.getEndSessionByDate(date);
	}
	public List<GfBjPk10GaOmit> findGfBjPk10GaOmitList(String string,
			List<Object> para, int num){
		return gfBjPk10DAO.findGfBjPk10GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfBjPk10GaSession tempsession) {
//		GfBjPk10GaSession session=BjPk10DAO.getCurrentSession();
//		if(session!=null){
//			GfBjPk10GaSession tempsession =BjPk10DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_BJPK10);
...
//			}
	 ...
	 * @return
	 */
	public String getTodaySessionNo(GfBjPk10GaSession beforeSession,int index,String sessionNo1){
...
//			log.info("___[startTimeStr="+startTimeStr+"]");
			...
			HQUtils hq = new HQUtils("from GfBjPk10GaSession gks where gks.startTime>? and gks.startTime<? order by gks.sessionId desc ");
			...
			hq.addPars(todayStart);
			hq.addPars(todayEnd);
			List<Object> list=gfBjPk10DAO.findObjects(hq);
			GfBjPk10GaSession beforeSession=null;
			if(list!=null&&list.size()>0){
				beforeSession=(GfBjPk10GaSession) list.get(0);
			}
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean saveRevokePrize(GfBjPk10GaSession session) {
		...
		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_BJPK10,session.getSessionNo());
		if(result){
