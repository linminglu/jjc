...
//	public String updateTomorrowSession();
	 ...
	 * @return
	 */
	public List<GfJsFtDTO> findGaBetDetailById(String string, List<Object> para);
	/**
	 * 保存合买投注
	 * @param u 用户
	 * @param list 投注号码列表
	...
	public User saveSponsorBet(User u,List<String> list,Map<String,Integer>seMap,int num,int buyNum,String isGuaranteed,int gaNum,BigDecimal betMon,
			BigDecimal needMoney,int betNum,GfJsFtGaSession currentSession,BigDecimal buyMoney);
	/**
	 * 保存代购投注
	 * @param user
	 * @param list
	...
	public User saveProcurementServiceBet(User user, List<String> list,
			Map<String, Integer> seMap, String isAddNo, String isWinStop,BigDecimal betMon,int betNum,GfJsFtGaSession currentSession);
	/**
	 * 更新开奖结果
	 */
	public String updateJsFtSessionOpenResultMethod(
			GfJsFtGaSession session, String openResult,String orderNum);
	/**
	 * 根据日期查找日期中的第一期
	 * @param string
	 * @return
	 */
	public GfJsFtGaSession getFirstSessionByDate(String string);
	/**
	 * 根据日期查找日期中的最后期
	 * @param string
	 * @return
	 */
	public GfJsFtGaSession getEndSessionByDate(String string);
	/**
	 * 查询遗漏表数据  num是最大查询数量
	 */
	...
	public void updateFetchAndOpenOmit(GfJsFtGaSession session);
	 ...
	 * 统计每日盈亏、每期盈亏。
	 */
	public void updateDayBetCount(GfJsFtGaSession session);
