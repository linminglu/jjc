...
public class JsFtServiceImpl extends BaseService implements IJsFtService {
	private IJsFtDAO gfJsFtDAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setJsFtDAO(IJsFtDAO gfJsFtDAO) {
//		gfJsFtDAO = gfJsFtDAO;
//		super.dao = gfJsFtDAO;
//	}

	public void setGfJsFtDAO(IJsFtDAO gfJsFtDAO) {
		this.gfJsFtDAO = gfJsFtDAO;
		super.dao = gfJsFtDAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_JSFT;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfJsFtDAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfJsFtDAO.updateObjectList(saveList, null);
			flag = "success";
			log.info("___[today["+today+"] init completed]__________________________"+code);
			GameHelpUtil.log("jsft", "___[today["+today+"] init completed]__________________________"+code);
		} else {
			flag = "inited";
			log.info("___[today["+today+"] has been inited]__________________________"+code);
			GameHelpUtil.log("jsft", "___[today["+today+"] has been inited]__________________________"+code);
		}
		return flag;
	}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfJsFtGaSession session = new GfJsFtGaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfJsFtGaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfJsFtGaSession>();
////			log.info("___[start today]__________________________");
//			
//			String startTimeStr = today + JsFtConstants.JS_FT_START_TIME_STR;//开始时间串
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
////			log.info("___[startTimeStr="+startTimeStr+"]");
//			
//			for (int i = 0; i < JsFtConstants.JS_FT_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*JsFtConstants.JS_FT_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfJsFtGaSession k10Session = new GfJsFtGaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,JsFtConstants.JS_FT_TIME_INTERVAL));
//				k10Session.setOpenStatus(JsFtConstants.JS_FT_OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				JsFtDAO.saveObject(k10Session);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfJsFtGaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfJsFtDAO.countObjects(hq);
	...
	public List<GfJsFtGaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfJsFtGaSession> sessionlist=new ArrayList<GfJsFtGaSession>();
		GfJsFtGaSession currentSession=gfJsFtDAO.getCurrentSession();
		GfJsFtGaSession tempsession =gfJsFtDAO.getPreviousSessionBySessionNo((Long.parseLong(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfJsFtGaSession) gfJsFtDAO.getObject(GfJsFtGaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfJsFtGaSession session =gfJsFtDAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(JsFtConstants.JS_FT_OPEN_STATUS_INIT) || openStatus1.equals(JsFtConstants.JS_FT_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(JsFtConstants.JS_FT_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfJsFtDAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_JSFT);					
					if(sessionInfo != null){
						...
						gfJsFtDAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfJsFtGaSession> updateAndOpenResult() {
//		List<GfJsFtGaSession> sessionlist=new ArrayList<GfJsFtGaSession>();
//		GfJsFtGaSession currentSession=gfJsFtDAO.getCurrentSession();
//		GfJsFtGaSession tempsession =gfJsFtDAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
//		}else{
//			tempsession=(GfJsFtGaSession) gfJsFtDAO.getObject(GfJsFtGaSession.class, (currentSession.getSessionId()-1));
//		}
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfjsft");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[JsFt start fetch result xml data...]________________---------JsFt");
//						GameHelpUtil.log("jsft", "___[JsFt start fetch result xml data...]________________---------JsFt");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[JsFt start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/JsFt.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("jsft", "JsFtServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("jsft", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("jsft", "JsFtServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateJsFtSessionOpenResultMethod(GfJsFtGaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_JSFT, Constants.PUB_STATUS_OPEN);
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							BigDecimal winMoney = JsFtUtil.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_JSFT);
			...
			gfJsFtDAO.updateObjectList(spList, null);
			gfJsFtDAO.updateObjectList(spDeList, null);
			gfJsFtDAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("jsft", "JsFtServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_JSFT);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_JSFT);
		...
		gfJsFtDAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfJsFtDAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_JSFT,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_JSFT,remark);
	}
	...
	public GfJsFtGaSession getCurrentSession(){
		return gfJsFtDAO.getCurrentSession();
	}
	public GfJsFtGaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfJsFtDAO.getPreviousSessionBySessionNo(sessionNo);
	}
	public void saveUserBetInfo(String room,Map<Integer,Integer> betMap,List<GaBetOption> list,GfJsFtGaSession gaPick11Session,User user,BigDecimal betAll){
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_JSFT);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfJsFtDAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_JSFT, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfJsFtDAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfJsFtGaSession GfJsFtGaSession = gfJsFtDAO.getCurrentSession(); // 当前期数信息
		if(GfJsFtGaSession != null){
			String curNo = GfJsFtGaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Long.parseLong(curNo) - 1)); //上期期号
			GfJsFtGaSession JsFtPreSession = getPreviousSessionBySessionNo(preNo);
			if(JsFtPreSession==null){
				JsFtPreSession=(GfJsFtGaSession) gfJsFtDAO.getObject(GfJsFtGaSession.class, GfJsFtGaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfJsFtGaTrend gdk10 ");
			List<Object> trendList = gfJsFtDAO.findObjects(hq); //上期冷热排行榜
			List<GfJsFtGaTrend> saveList = new ArrayList<GfJsFtGaTrend>(); // 本期冷热排行榜

			String results = JsFtPreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfJsFtGaTrend trend = (GfJsFtGaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfJsFtGaSession session,String openResult){

		boolean flag=false;
		session.setOpenResult(openResult);
		gfJsFtDAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	
	public boolean saveAndOpenResult(GfJsFtGaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateJsFtSessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("jsft", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfJsFtDAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfJsFtDTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfJsFtDAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfJsFtGaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_JSFT);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfJsFtGaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_JSFT);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_JSFT));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfJsFtDAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_JSFT) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_JSFT,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_JSFT,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfJsFtDAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_JsFt, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfJsFtGaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= JsFtDAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = JsFtUtil.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_JSFT,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfJsFtDAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_JsFt,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_JsFt);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_JsFt);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfJsFtGaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("jsft", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfJsFtGaSession session=JsFtDAO.getCurrentSession();
//		if(session!=null){
//			GfJsFtGaSession tempsession =JsFtDAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfJsFtGaOmit omit=gfJsFtDAO.getJsFtOmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfJsFtGaOmit preomit=gfJsFtDAO.getJsFtOmitBySessionNo((Long.parseLong(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfJsFtGaSession se = (GfJsFtGaSession) gfJsFtDAO.getObject(GfJsFtGaSession.class, tempsession.getSessionId()-1);
							preomit=gfJsFtDAO.getJsFtOmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfJsFtGaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfJsFtGaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("jsft", "JsFtServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfJsFtGaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("jsft", "JsFtServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("jsft", "JsFtServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("jsft", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("jsft", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("jsft", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_JSFT,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_JSFT,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_JSFT,remark);
	}
	...
	public GfJsFtGaSession getFirstSessionByDate(String date) {
		return gfJsFtDAO.getFirstSessionByDate(date);
	}
	public GfJsFtGaSession getEndSessionByDate(String date){
		return gfJsFtDAO.getEndSessionByDate(date);
	}
	public List<GfJsFtGaOmit> findGfJsFtGaOmitList(String string,
			List<Object> para, int num){
		return gfJsFtDAO.findGfJsFtGaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfJsFtGaSession tempsession) {
//		GfJsFtGaSession session=JsFtDAO.getCurrentSession();
//		if(session!=null){
//			GfJsFtGaSession tempsession =JsFtDAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_JSFT);
...
//			}
	 ...
	 * @return
	 */
	public String getTodaySessionNo(GfJsFtGaSession beforeSession,int index,String sessionNo1){
	...
	public String updateInitTodaySession() {
		...
		if(!isTodaySessionInit){
			...
			for (int i = 0; i < JsFtConstants.JS_FT_MAX_PART; i++) {
				//计算出当前场次的时间
				Date startTime=DateTimeUtil.getDateTimeOfSeconds(startDate,JsFtConstants.JS_FT_TIME_INTERVAL*i);
				Date endTime=DateTimeUtil.getDateTimeOfSeconds(startTime,JsFtConstants.JS_FT_TIME_INTERVAL);
	...
	@Override
	public boolean saveRevokePrize(GfJsFtGaSession session) {

		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_JSFT,session.getSessionNo());
		if(result){
