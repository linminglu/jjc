...
public class JsFtUtil {
	...
	public static int getTotalBetNum(String playType,String betBall){
		int totalBet = 0;
		if(JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_JI.equals(playType) 
				|| JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_YA.equals(playType)
				|| JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_GUAN.equals(playType) 
				|| JsFtConstants.GAME_TYPE_GF_DAXI_AO_JI.equals(playType)
				|| JsFtConstants.GAME_TYPE_GF_DAXI_AO_YA.equals(playType) 
				|| JsFtConstants.GAME_TYPE_GF_DAXI_AO_GUAN.equals(playType)){// 大小单双共6种玩法
			String[] arr = betBall.split(",");
			totalBet = totalBet + arr.length;
		}else if(JsFtConstants.GAME_TYPE_GF_DING_WEI_DAN_1_5.equals(playType)
				|| JsFtConstants.GAME_TYPE_GF_DING_WEI_DAN_6_10.equals(playType)){// 定位胆
			String[] arr = betBall.split("\\|");
			totalBet = totalBet + getDingWeiDanBetTotal(arr);
		}else if(JsFtConstants.GAME_TYPE_GF_QIAN_SAN.equals(playType)){// 前三
			String[] arr = betBall.split("\\|");
			if(arr.length != 3){
				Log.info("前三不正确");
				return 0;
		...
		}else if(JsFtConstants.GAME_TYPE_GF_QIAN_ER.equals(playType)){// 前二
			String[] arr = betBall.split("\\|");
			if(arr.length != 2){
				Log.info("前二不正确");
				return 0;
		...
		}else if(JsFtConstants.GAME_TYPE_GF_QIAN_YI.equals(playType)){// 前一
			if (!"".equals(betBall)) {
				String[] arr = betBall.split(",");
				totalBet = totalBet + arr.length;
			} else {
	...
	public static BigDecimal judgeWinMoney(String playType, String betBall,
		...
		if (results.length == 10) {
			if(JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_JI.equals(playType)
					|| JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_YA.equals(playType)
					|| JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_GUAN.equals(playType)){// 单双
				String resVal = "";
				if (JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_JI.equals(playType)) {
					resVal = results[2];// 开奖结果的第三位(季军)
				} else if (JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_YA.equals(playType)) {
					resVal = results[1];// 开奖结果的第二位(亚军)
				} else if (JsFtConstants.GAME_TYPE_GF_DAN_SHUANG_GUAN.equals(playType)) {
			...
			} else if(JsFtConstants.GAME_TYPE_GF_DAXI_AO_JI.equals(playType)
					|| JsFtConstants.GAME_TYPE_GF_DAXI_AO_YA.equals(playType)
					|| JsFtConstants.GAME_TYPE_GF_DAXI_AO_GUAN.equals(playType)){// 大小
				String resVal = "";
				if (JsFtConstants.GAME_TYPE_GF_DAXI_AO_JI.equals(playType)) {
					resVal = results[2];// 开奖结果的第三位(季军)
				} else if (JsFtConstants.GAME_TYPE_GF_DAXI_AO_YA.equals(playType)) {
					resVal = results[1];// 开奖结果的第二位(亚军)
				} else if (JsFtConstants.GAME_TYPE_GF_DAXI_AO_GUAN.equals(playType)) {
					resVal = results[0];// 开奖结果的第一位(冠军)
				}
				String daxiao = "";
			...
			} else if (JsFtConstants.GAME_TYPE_GF_QIAN_SAN.equals(playType)) { // 前三  格式02,05|06|
			...
			} else if (JsFtConstants.GAME_TYPE_GF_QIAN_YI.equals(playType)) {// 前一 格式   02,05,09,10
				String diyiRes = results[0];// 第一名结果
				String[] betBallArr = betBall.split(",");
				for (int i = 0;i < betBallArr.length;i++) {
