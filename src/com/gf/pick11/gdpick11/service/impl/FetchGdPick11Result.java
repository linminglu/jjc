...
//		
//		if(!Constants.getTimerOpen("gdpick11.fetchResult.gf")) return;
//		
//		try {
//
//			log.info("_______[pick11 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");	
//			GameHelpUtil.log("gdpick11", "_______[pick11 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//			Date now=new Date();
...
//				List<GfGdPick11GaSession> list=gfGdPick11Service.updateAndOpenResult();
//				List<GfGdPick11GaSession> savelist=new ArrayList<GfGdPick11GaSession>();
//				gfGdPick11Service.updateGaTrend();
//				
//				Collections.sort(list,new Comparator<GfGdPick11GaSession>(){
//				    public int compare(GfGdPick11GaSession o1, GfGdPick11GaSession o2){  
...
//				    });  
//
//				for(GfGdPick11GaSession session:list){
//					String flag = gfGdPick11Service.updateGdPick11SessionOpenResultMethod(session, session.getOpenResult(),null);
//					if(ParamUtils.chkString(flag)){
//						session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
//						savelist.add(session);
//					}else{
//						gfGdPick11Service.updateDayBetCount(session);
//					}
//					gfGdPick11Service.updateFetchAndOpenOmit(session);
//				}
//				gfGdPick11Service.updateObjectList(savelist, null);
//			}
//
//		} catch (Exception e) {
//			GameHelpUtil.log("gdpick11", "FetchGdPick11Result.java,line:81,"+e.toString());
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_GDPICK11);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_GDPICK11);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_GDPICK11)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_GDPICK11);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_GDPICK11);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfGdPick11GaSession curSession = gfGdPick11Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfGdPick11GaSession openSession = (GfGdPick11GaSession)gfGdPick11Service.getObject(GfGdPick11GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfGdPick11GaSession> list=gfGdPick11Service.updateAndOpenResult(sessionNoMap);
	        				List<GfGdPick11GaSession> savelist=new ArrayList<GfGdPick11GaSession>();
	        				gfGdPick11Service.updateGaTrend();
	        				Collections.sort(list,new Comparator<GfGdPick11GaSession>(){
	        				    public int compare(GfGdPick11GaSession o1, GfGdPick11GaSession o2){  
