...
import com.gf.pick11.sdpick11.util.SdPick11Util;
import com.ram.model.User;
import com.ram.service.user.IUserService;

public class SdPick11ServiceImpl extends BaseService implements ISdPick11Service {
	private ISdPick11DAO gfSdPick11DAO;
	private IUserService userService;
	private IGaService gaService;
	public void setGfSdPick11DAO(ISdPick11DAO gfSdPick11DAO) {
		this.gfSdPick11DAO = gfSdPick11DAO;
		super.dao = gfSdPick11DAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_SDPICK11;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfSdPick11DAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfSdPick11DAO.updateObjectList(saveList, null);
			flag = "success";
			log.info("___[today["+today+"] init completed]__________________________"+code);
			GameHelpUtil.log("sdpick11", "___[today["+today+"] init completed]__________________________"+code);
		} else {
			flag = "inited";
			log.info("___[today["+today+"] has been inited]__________________________"+code);
			GameHelpUtil.log("sdpick11", "___[today["+today+"] has been inited]__________________________"+code);
		}
		return flag;
	}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfSdPick11GaSession session = new GfSdPick11GaSession();
		...
		session.setOpenStatus(SdPick11Constants.SD_PICK11_OPEN_STATUS_INIT);
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfSdPick11GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfSdPick11DAO.countObjects(hq);
	...
	@Override
	public List<GfSdPick11GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfSdPick11GaSession> sessionlist=new ArrayList<GfSdPick11GaSession>();
		GfSdPick11GaSession currentSession=gfSdPick11DAO.getCurrentSession();
		GfSdPick11GaSession tempsession =gfSdPick11DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){//待开奖期
			tempsession=(GfSdPick11GaSession) gfSdPick11DAO.getObject(GfSdPick11GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfSdPick11GaSession session =gfSdPick11DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(SdPick11Constants.SD_PICK11_OPEN_STATUS_INIT) || openStatus1.equals(SdPick11Constants.SD_PICK11_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(SdPick11Constants.SD_PICK11_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfSdPick11DAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_SDPICK11);					
					if(sessionInfo != null){
						...
						gfSdPick11DAO.updateObject(sessionInfo, null);
...
//	@Override
//	public List<GfSdPick11GaSession> updateAndOpenResult() {
//		List<GfSdPick11GaSession> sessionlist=new ArrayList<GfSdPick11GaSession>();
//		GfSdPick11GaSession currentSession=gfSdPick11DAO.getCurrentSession();
//		GfSdPick11GaSession tempsession =gfSdPick11DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfsdpick11");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[sdpick11 start fetch result xml data...]________________---------sdpick11");
//						GameHelpUtil.log("sdpick11", "___[sdpick11 start fetch result xml data...]________________---------sdpick11");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[sdpick11 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/sdpick11.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("sdpick11", "SdPick11ServiceImpl.java,line:289,"+e.toString());
//					}
//				}
//			}
//        };
...
//							GameHelpUtil.log("sdpick11", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			GameHelpUtil.log("sdpick11", "SdPick11ServiceImpl.java,line:353,"+e.toString());
//			e.printStackTrace();
//		}
//        currentSession=null;
//        
...
//	}
	 ...
	 * @return
	 */
	public String updateSdPick11SessionOpenResultMethod(GfSdPick11GaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_SDPICK11, Constants.PUB_STATUS_OPEN);
			...
			para.add(Constants.GAME_TYPE_GF_SDPICK11);
			if(orderNum !=null){
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							BigDecimal winMoney = SdPick11Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_SDPICK11);
			...
			gfSdPick11DAO.updateObjectList(spList, null);
			gfSdPick11DAO.updateObjectList(spDeList, null);
			gfSdPick11DAO.updateObjectList(paList, null);
//			gfSdPick11DAO.updateObjectList(tradeList, null);
//			gfSdPick11DAO.updateObjectList(pointList, null);
//			gfSdPick11DAO.updateObjectList(userList, null);
//			gfSdPick11DAO.updateObjectList(gaWinCoList, null);
			return "";
		}catch(Exception e){
			GameHelpUtil.log("sdpick11", "SdPick11ServiceImpl.java,line:600,"+e.toString());
			e.printStackTrace();
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_SDPICK11,remark);
	}
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_SDPICK11);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_SDPICK11);
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfSdPick11DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_SDPICK11,remark,
				sessionNo,user.getUserType(),user.getLoginName()
				);
	}
	
	public GfSdPick11GaSession getCurrentSession(){
		return gfSdPick11DAO.getCurrentSession();
	}
	public GfSdPick11GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfSdPick11DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	public void saveUserBetInfo(String room,Map<Integer,Integer> betMap,List<GaBetOption> list,GfSdPick11GaSession gaPick11Session,User user,BigDecimal betAll){
...
//				betDetail.setPaperMoney(paperMoney);
				...
				betDetail.setGameType(Constants.GAME_TYPE_GF_SDPICK11);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
		...
		if(paperMoney.compareTo(betAll)==1||paperMoney.compareTo(betAll)==0){//红包足够投注，不用使用余额里面的钱
			...
			gfSdPick11DAO.updateObject(user, null);
		}else{
			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_SDPICK11, betAll.subtract(tempMoney), null);
		}
//		userService.savePointDetail(user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_BJPPick11, betAll.intValue(), null);
		...
		gfSdPick11DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfSdPick11GaSession GfSdPick11GaSession = gfSdPick11DAO.getCurrentSession(); // 当前期数信息
		if(GfSdPick11GaSession != null){
			String curNo = GfSdPick11GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
			GfSdPick11GaSession sdPick11PreSession = getPreviousSessionBySessionNo(preNo);
			if(sdPick11PreSession==null){
				sdPick11PreSession=(GfSdPick11GaSession) gfSdPick11DAO.getObject(GfSdPick11GaSession.class, GfSdPick11GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfSdPick11GaTrend gdk10 ");
			List<Object> trendList = gfSdPick11DAO.findObjects(hq); //上期冷热排行榜
			List<GfSdPick11GaTrend> saveList = new ArrayList<GfSdPick11GaTrend>(); // 本期冷热排行榜

			String results = sdPick11PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfSdPick11GaTrend trend = (GfSdPick11GaTrend) trendList.get(i);
	...
	public boolean saveOpenResult(GfSdPick11GaSession session,String openResult){

		boolean flag=false;
		session.setOpenResult(openResult);
		gfSdPick11DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	
	public boolean saveAndOpenResult(GfSdPick11GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateSdPick11SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("sdpick11", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfSdPick11DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfSdPick11DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfSdPick11DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfSdPick11GaSession currentSession,BigDecimal buyMoney,String isPrivacy) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_SDPICK11);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfSdPick11GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_SDPICK11);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_SDPICK11));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfSdPick11DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_SDPICK11) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_SDPICK11,remark);
			for(String op : list){
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_SDPICK11,remark);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfSdPick11GaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= gfSdPick11DAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = SdPick11Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_SDPICK11,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfSdPick11DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_SDPICK11,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_SDPICK11);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_SDPICK11);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfSdPick11GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("sdpick11", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfSdPick11GaSession session=gfSdPick11DAO.getCurrentSession();
//		if(session!=null){
//			GfSdPick11GaSession tempsession =gfSdPick11DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfSdPick11GaOmit omit=gfSdPick11DAO.getSdPick11OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfSdPick11GaOmit preomit=gfSdPick11DAO.getSdPick11OmitBySessionNo((Integer.parseInt(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfSdPick11GaSession se = (GfSdPick11GaSession) gfSdPick11DAO.getObject(GfSdPick11GaSession.class, tempsession.getSessionId()-1);
							preomit=gfSdPick11DAO.getSdPick11OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfSdPick11GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfSdPick11GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("sdpick11", "SdPick11ServiceImpl.java,line:1388,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfSdPick11GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("sdpick11", "SdPick11ServiceImpl.java,line:1421,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							gfSdPick11DAO.saveObjectDB(omit);
						}else{
							Map<String,String> map=new HashMap<String,String>();
							for(int i=1;i<=11;i++){
								map.put(""+i, i+"");
							...
							try {
								...
								GameHelpUtil.log("sdpick11", "SdPick11ServiceImpl.java,line:1484,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("sdpick11", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("sdpick11", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("sdpick11", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_SDPICK11,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_SDPICK11,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_SDPICK11,remark);
	}
	...
	public GfSdPick11GaSession getFirstSessionByDate(String date) {
		return gfSdPick11DAO.getFirstSessionByDate(date);
	}
	public GfSdPick11GaSession getEndSessionByDate(String date){
		return gfSdPick11DAO.getEndSessionByDate(date);
	}
	public List<GfSdPick11GaOmit> findGfSdPick11GaOmitList(String string,
			List<Object> para, int num){
		return gfSdPick11DAO.findGfSdPick11GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfSdPick11GaSession tempsession) {
//		GfSdPick11GaSession session=gfSdPick11DAO.getCurrentSession();
//		if(session!=null){
//			GfSdPick11GaSession tempsession =gfSdPick11DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_SDPICK11);
...
//			}
	...
	@Override
	public boolean saveRevokePrize(GfSdPick11GaSession session) {

		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_SDPICK11,session.getSessionNo());
		if(result){
