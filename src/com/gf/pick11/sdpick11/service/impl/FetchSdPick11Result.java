...
public class FetchSdPick11Result  extends QuartzJobBean {
...
//		
//		if(!Constants.getTimerOpen("sdpick11.fetchResult.gf")) return;
//		
//		try {
//
//			log.info("_______[pick11 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");	
//			GameHelpUtil.log("sdpick11", "_______[pick11 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//			Date now=new Date();
...
//				List<GfSdPick11GaSession> list=gfSdPick11Service.updateAndOpenResult();
//				List<GfSdPick11GaSession> savelist=new ArrayList<GfSdPick11GaSession>();
//				gfSdPick11Service.updateGaTrend();
//				
//				Collections.sort(list,new Comparator<GfSdPick11GaSession>(){
//				    public int compare(GfSdPick11GaSession o1, GfSdPick11GaSession o2){  
...
//				    });  
//
//				for(GfSdPick11GaSession session:list){
//					String flag = gfSdPick11Service.updateSdPick11SessionOpenResultMethod(session, session.getOpenResult(),null);
//					if(ParamUtils.chkString(flag)){
//						session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
//						savelist.add(session);
//					}else{
//						gfSdPick11Service.updateDayBetCount(session);
//					}
//					gfSdPick11Service.updateFetchAndOpenOmit(session);
//				}
//				gfSdPick11Service.updateObjectList(savelist, null);
//			}
//
//		} catch (Exception e) {
//			GameHelpUtil.log("sdpick11", "FetchSdPick11Result.java,line:81,"+e.toString());
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_SDPICK11);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_SDPICK11);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_SDPICK11)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_SDPICK11);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_SDPICK11);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfSdPick11GaSession curSession = gfSdPick11Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfSdPick11GaSession openSession = (GfSdPick11GaSession)gfSdPick11Service.getObject(GfSdPick11GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfSdPick11GaSession> list=gfSdPick11Service.updateAndOpenResult(sessionNoMap);
	        				List<GfSdPick11GaSession> savelist=new ArrayList<GfSdPick11GaSession>();
	        				gfSdPick11Service.updateGaTrend();
	        				Collections.sort(list,new Comparator<GfSdPick11GaSession>(){
	        				    public int compare(GfSdPick11GaSession o1, GfSdPick11GaSession o2){  
