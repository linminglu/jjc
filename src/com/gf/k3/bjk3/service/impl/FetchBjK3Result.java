...
public class FetchBjK3Result  extends QuartzJobBean {
...
//		
//		if(!Constants.getTimerOpen("bjk3.fetchResult.gf")) return;
//		
//		try {
//
//			log.info("_______[bjK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");	
//			GameHelpUtil.log("bjk3", "_______[bjK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//			Date now=new Date();
...
//				List<GfBjK3GaSession> list=gfBjK3Service.updateAndOpenResult();
//				List<GfBjK3GaSession> savelist=new ArrayList<GfBjK3GaSession>();
//				gfBjK3Service.updateGaTrend();
//				
//				Collections.sort(list,new Comparator<GfBjK3GaSession>(){
//				    public int compare(GfBjK3GaSession o1, GfBjK3GaSession o2){  
...
//				    });  
//
//				for(GfBjK3GaSession session:list){
//					String flag = gfBjK3Service.updateBjK3SessionOpenResultMethod(session, session.getOpenResult(),null);
//					if(ParamUtils.chkString(flag)){
//						session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
//						savelist.add(session);
//					}else{
//						gfBjK3Service.updateDayBetCount(session);
//					}
//					gfBjK3Service.updateFetchAndOpenOmit(session);
//				}
//				gfBjK3Service.updateObjectList(savelist, null);
//			}
//
//		} catch (Exception e) {
//			GameHelpUtil.log("bjk3", "FetchBjK3Result.java,line:80,"+e.toString());
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_BJK3);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_BJK3);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_BJK3)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_BJK3);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_BJK3);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfBjK3GaSession curSession = gfBjK3Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfBjK3GaSession openSession = (GfBjK3GaSession)gfBjK3Service.getObject(GfBjK3GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfBjK3GaSession> list=gfBjK3Service.updateAndOpenResult(sessionNoMap);
	        				List<GfBjK3GaSession> savelist=new ArrayList<GfBjK3GaSession>();
	        				gfBjK3Service.updateGaTrend();
	        				Collections.sort(list,new Comparator<GfBjK3GaSession>(){
	        				    public int compare(GfBjK3GaSession o1, GfBjK3GaSession o2){  
