...
import com.gf.k3.bjk3.util.BjK3Util;
...
public class BjK3ServiceImpl extends BaseService implements IBjK3Service {
	private IBjK3DAO gfBjK3DAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setBjK3DAO(IBjK3DAO gfBjK3DAO) {
//		gfBjK3DAO = gfBjK3DAO;
//		super.dao = gfBjK3DAO;
//	}

	public void setGfBjK3DAO(IBjK3DAO gfBjK3DAO) {
		this.gfBjK3DAO = gfBjK3DAO;
		super.dao = gfBjK3DAO;
	}
...
//		//获取彩种初始参数
//		String gameType = Constants.GAME_TYPE_GF_BJK3;
//		
//		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
//		hq.addPars(gameType);
//		GaSessionInfo cps = (GaSessionInfo)gfBjK3DAO.getObject(hq);//彩种信息
//		if(cps==null){
...
//				
//			}
//			gfBjK3DAO.updateObjectList(saveList, null);
//			flag = "success";
//			log.info("___[today["+today+"] init completed]__________________________"+code);
//			GameHelpUtil.log("bjk3", "___[today["+today+"] init completed]__________________________"+code);
//		} else {
//			flag = "inited";
//			log.info("___[today["+today+"] has been inited]__________________________"+code);
//			GameHelpUtil.log("bjk3", "___[today["+today+"] has been inited]__________________________"+code);
//		}
//		return flag;
//	}
	public String updateInitSession(String sessionNo1) {
		...
		//今天是否已经初始化场次
		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
		List<GfBjK3GaSession> saveList = null;
		if(!isTodaySessionInit){
			saveList = new ArrayList<GfBjK3GaSession>();
			String startTimeStr = today + BjK3Constants.BJ_K3_START_TIME_STR;//开始时间串
			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
			
			HQUtils hq = new HQUtils("from GfBjK3GaSession gks where gks.startTime>? and gks.startTime<? order by gks.sessionId desc ");
			...
			hq.addPars(todayEnd);
			List<Object> list=gfBjK3DAO.findObjects(hq);
			GfBjK3GaSession beforeSession=null;
			if(list!=null&&list.size()>0){
				beforeSession=(GfBjK3GaSession) list.get(0);
			}
			
			for (int i = 0; i < BjK3Constants.BJ_K3_MAX_PART; i++) {
				//计算出当前场次的时间
				long diffTime = startDate.getTime() + i*BjK3Constants.BJ_K3_TIME_INTERVAL * 60 * 1000;
				Date curSessionDate = new Date(diffTime);
				...
				GfBjK3GaSession bjK3Session = new GfBjK3GaSession();
				bjK3Session.setSessionNo(sessionNo);
				bjK3Session.setStartTime(curSessionDate);
				bjK3Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,BjK3Constants.BJ_K3_TIME_INTERVAL));
	 ...
	 * 获取今天的期号
	 */
	public String getTodaySessionNo(GfBjK3GaSession beforeSession,int index,String sessionNo1){
	...
	public boolean checkNextdaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfBjK3GaSession bgs where bgs.startTime>? and bgs.startTime<?");
		hq.addPars(DateTimeUtil.getDateTimeOfDays(todayStart, 1));
		hq.addPars(DateTimeUtil.getDateTimeOfDays(todayEnd, 1));
		Integer count = gfBjK3DAO.countObjects(hq);
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfBjK3GaSession session = new GfBjK3GaSession();
...
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfBjK3GaSession k10Session = new GfBjK3GaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,BjK3Constants.BJ_K3_TIME_INTERVAL));
//				k10Session.setOpenStatus(BjK3Constants.BJ_K3_OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				BjK3DAO.saveObject(k10Session);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfBjK3GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfBjK3DAO.countObjects(hq);
		return ParamUtils.chkInteger(count)?true:false;
	...
	public List<GfBjK3GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfBjK3GaSession> sessionlist=new ArrayList<GfBjK3GaSession>();
		GfBjK3GaSession currentSession=gfBjK3DAO.getCurrentSession();
		GfBjK3GaSession tempsession =gfBjK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfBjK3GaSession) gfBjK3DAO.getObject(GfBjK3GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfBjK3GaSession session =gfBjK3DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(BjK3Constants.BJ_K3_OPEN_STATUS_INIT) || openStatus1.equals(BjK3Constants.BJ_K3_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(BjK3Constants.BJ_K3_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfBjK3DAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_BJK3);					
					if(sessionInfo != null){
						...
						gfBjK3DAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfBjK3GaSession> updateAndOpenResult() {
//		List<GfBjK3GaSession> sessionlist=new ArrayList<GfBjK3GaSession>();
//		GfBjK3GaSession currentSession=gfBjK3DAO.getCurrentSession();
//		GfBjK3GaSession tempsession =gfBjK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfbjk3");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[BjK3 start fetch result xml data...]________________---------BjK3");
//						GameHelpUtil.log("bjk3", "___[BjK3 start fetch result xml data...]________________---------BjK3");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[BjK3 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/BjK3.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("bjk3", "BjK3ServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("bjk3", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("bjk3", "BjK3ServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateBjK3SessionOpenResultMethod(GfBjK3GaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_BJK3, Constants.PUB_STATUS_OPEN);
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							if (BjK3Constants.GAME_TYPE_GF_HEZHI.equals(playType)) {
								String[] results = result.split(",");
								Integer hezhi = Integer.valueOf(results[0])+Integer.valueOf(results[1])
										+Integer.valueOf(results[2]);
								betRate = gfBjK3DAO.getHeZhiBetRate("和值"+hezhi.toString());//
							} else {
								betRate = det.getBetRate();//
							}
							BigDecimal point = det.getPointMultiple();//倍率
							
							BigDecimal winMoney = BjK3Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_BJK3);
			...
			gfBjK3DAO.updateObjectList(spList, null);
			gfBjK3DAO.updateObjectList(spDeList, null);
			gfBjK3DAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("bjk3", "BjK3ServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_BJK3);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_BJK3);
		...
		gfBjK3DAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfBjK3DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_BJK3,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_BJK3,remark);
	}
	...
	public GfBjK3GaSession getCurrentSession(){
		return gfBjK3DAO.getCurrentSession();
	}
	public GfBjK3GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfBjK3DAO.getPreviousSessionBySessionNo(sessionNo);
	}
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_BJK3);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfBjK3DAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_BJK3, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfBjK3DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfBjK3GaSession GfBjK3GaSession = gfBjK3DAO.getCurrentSession(); // 当前期数信息
		if(GfBjK3GaSession != null){
			String curNo = GfBjK3GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
			GfBjK3GaSession BjK3PreSession = getPreviousSessionBySessionNo(preNo);
			if(BjK3PreSession==null){
				BjK3PreSession=(GfBjK3GaSession) gfBjK3DAO.getObject(GfBjK3GaSession.class, GfBjK3GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfBjK3GaTrend gdk10 ");
			List<Object> trendList = gfBjK3DAO.findObjects(hq); //上期冷热排行榜
			List<GfBjK3GaTrend> saveList = new ArrayList<GfBjK3GaTrend>(); // 本期冷热排行榜

			String results = BjK3PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfBjK3GaTrend trend = (GfBjK3GaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfBjK3GaSession session,String openResult){
		...
		boolean flag=false;
		session.setOpenResult(openResult);
		gfBjK3DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	...
	public boolean saveAndOpenResult(GfBjK3GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateBjK3SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("bjk3", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfBjK3DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfBjK3DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfBjK3DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfBjK3GaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_BJK3);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfBjK3GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_BJK3);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_BJK3));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfBjK3DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_BJK3) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_BJK3,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_BJK3,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfBjK3DAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_BjK3, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfBjK3GaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = BjK3Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_BJK3,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfBjK3DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_BjK3,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_BjK3);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_BjK3);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfBjK3GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("bjk3", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfBjK3GaSession session=BjK3DAO.getCurrentSession();
//		if(session!=null){
//			GfBjK3GaSession tempsession =BjK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfBjK3GaOmit omit=gfBjK3DAO.getBjK3OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfBjK3GaOmit preomit=gfBjK3DAO.getBjK3OmitBySessionNo((Integer.parseInt(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfBjK3GaSession se = (GfBjK3GaSession) gfBjK3DAO.getObject(GfBjK3GaSession.class, tempsession.getSessionId()-1);
							preomit=gfBjK3DAO.getBjK3OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfBjK3GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfBjK3GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("bjk3", "BjK3ServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfBjK3GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("bjk3", "BjK3ServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("bjk3", "BjK3ServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("bjk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("bjk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("bjk3", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_BJK3,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_BJK3,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_BJK3,remark);
	}
	...
	public GfBjK3GaSession getFirstSessionByDate(String date) {
		return gfBjK3DAO.getFirstSessionByDate(date);
	}
	public GfBjK3GaSession getEndSessionByDate(String date){
		return gfBjK3DAO.getEndSessionByDate(date);
	}
	public List<GfBjK3GaOmit> findGfBjK3GaOmitList(String string,
			List<Object> para, int num){
		return gfBjK3DAO.findGfBjK3GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfBjK3GaSession tempsession) {
//		GfBjK3GaSession session=BjK3DAO.getCurrentSession();
//		if(session!=null){
//			GfBjK3GaSession tempsession =BjK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_BJK3);
...
//			}
	...
	public boolean saveRevokePrize(GfBjK3GaSession session) {
		...
		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_BJK3,session.getSessionNo());
		if(result){
