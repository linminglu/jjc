...
public class FetchJxK3Result  extends QuartzJobBean {
...
//		
//		if(!Constants.getTimerOpen("jxk3.fetchResult.gf")) return;
//		
//		try {
//
//			log.info("_______[jxK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");	
//			GameHelpUtil.log("jxk3", "_______[jxK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//			Date now=new Date();
...
//				List<GfJxK3GaSession> list=gfJxK3Service.updateAndOpenResult();
//				List<GfJxK3GaSession> savelist=new ArrayList<GfJxK3GaSession>();
//				gfJxK3Service.updateGaTrend();
//				
//				Collections.sort(list,new Comparator<GfJxK3GaSession>(){
//				    public int compare(GfJxK3GaSession o1, GfJxK3GaSession o2){  
...
//				    });  
//
//				for(GfJxK3GaSession session:list){
//					String flag = gfJxK3Service.updateJxK3SessionOpenResultMethod(session, session.getOpenResult(),null);
//					if(ParamUtils.chkString(flag)){
//						session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
//						savelist.add(session);
//					}else{
//						gfJxK3Service.updateDayBetCount(session);
//					}
//					gfJxK3Service.updateFetchAndOpenOmit(session);
//				}
//				gfJxK3Service.updateObjectList(savelist, null);
//			}
//
//		} catch (Exception e) {
//			GameHelpUtil.log("jxk3", "FetchJxK3Result.java,line:80,"+e.toString());
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_JXK3);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_JXK3);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_JXK3)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_JXK3);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_JXK3);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfJxK3GaSession curSession = gfJxK3Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfJxK3GaSession openSession = (GfJxK3GaSession)gfJxK3Service.getObject(GfJxK3GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfJxK3GaSession> list=gfJxK3Service.updateAndOpenResult(sessionNoMap);
	        				List<GfJxK3GaSession> savelist=new ArrayList<GfJxK3GaSession>();
	        				gfJxK3Service.updateGaTrend();
	        				Collections.sort(list,new Comparator<GfJxK3GaSession>(){
	        				    public int compare(GfJxK3GaSession o1, GfJxK3GaSession o2){  
