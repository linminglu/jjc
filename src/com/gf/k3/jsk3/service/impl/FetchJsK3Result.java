...
//		
//		if(!Constants.getTimerOpen("jsk3.fetchResult.gf")) return;
//		
//		try {
//
//			log.info("_______[jsK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");	
//			GameHelpUtil.log("jsk3", "_______[jsK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//			Date now=new Date();
...
//				List<GfJsK3GaSession> list=gfJsK3Service.updateAndOpenResult();
//				List<GfJsK3GaSession> savelist=new ArrayList<GfJsK3GaSession>();
//				gfJsK3Service.updateGaTrend();
//				
//				Collections.sort(list,new Comparator<GfJsK3GaSession>(){
//				    public int compare(GfJsK3GaSession o1, GfJsK3GaSession o2){  
...
//				    });  
//
//				for(GfJsK3GaSession session:list){
//					String flag = gfJsK3Service.updateJsK3SessionOpenResultMethod(session, session.getOpenResult(),null);
//					if(ParamUtils.chkString(flag)){
//						session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
//						savelist.add(session);
//					}else{
//						gfJsK3Service.updateDayBetCount(session);
//					}
//					gfJsK3Service.updateFetchAndOpenOmit(session);
//				}
//				gfJsK3Service.updateObjectList(savelist, null);
//			}
//
//		} catch (Exception e) {
//			GameHelpUtil.log("jsk3", "FetchJsK3Result.java,line:80,"+e.toString());
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_JSK3);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_JSK3);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_JSK3)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_JSK3);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_JSK3);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfJsK3GaSession curSession = gfJsK3Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfJsK3GaSession openSession = (GfJsK3GaSession)gfJsK3Service.getObject(GfJsK3GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfJsK3GaSession> list=gfJsK3Service.updateAndOpenResult(sessionNoMap);
	        				List<GfJsK3GaSession> savelist=new ArrayList<GfJsK3GaSession>();
	        				gfJsK3Service.updateGaTrend();
