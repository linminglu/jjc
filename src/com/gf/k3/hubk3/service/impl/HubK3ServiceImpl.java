...
import com.gf.k3.hubk3.HubK3Constants;
import com.gf.k3.hubk3.dao.IHubK3DAO;
...
import com.gf.k3.hubk3.util.HubK3Util;
import com.gf.pick11.gdpick11.GdPick11Constants;
import com.ram.model.User;
import com.ram.service.user.IUserService;

public class HubK3ServiceImpl extends BaseService implements IHubK3Service {
	private IHubK3DAO gfHubK3DAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setHubK3DAO(IHubK3DAO gfHubK3DAO) {
//		gfHubK3DAO = gfHubK3DAO;
//		super.dao = gfHubK3DAO;
//	}

	public void setGfHubK3DAO(IHubK3DAO gfHubK3DAO) {
		this.gfHubK3DAO = gfHubK3DAO;
		super.dao = gfHubK3DAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_HUBK3;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfHubK3DAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfHubK3DAO.updateObjectList(saveList, null);
			flag = "success";
			log.info("___[today["+today+"] init completed]__________________________"+code);
			GameHelpUtil.log("hubk3", "___[today["+today+"] init completed]__________________________"+code);
		} else {
			flag = "inited";
			log.info("___[today["+today+"] has been inited]__________________________"+code);
			GameHelpUtil.log("hubk3", "___[today["+today+"] has been inited]__________________________"+code);
		}
		return flag;
	}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfHubK3GaSession session = new GfHubK3GaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfHubK3GaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfHubK3GaSession>();
////			log.info("___[start today]__________________________");
//			
//			String startTimeStr = today + HubK3Constants.HUB_K3_START_TIME_STR;//开始时间串
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
////			log.info("___[startTimeStr="+startTimeStr+"]");
//			
//			for (int i = 0; i < HubK3Constants.HUB_K3_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*HubK3Constants.HUB_K3_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfHubK3GaSession k10Session = new GfHubK3GaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,HubK3Constants.HUB_K3_TIME_INTERVAL));
//				k10Session.setOpenStatus(HubK3Constants.HUB_K3_OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				HubK3DAO.saveObject(k10Session);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfHubK3GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfHubK3DAO.countObjects(hq);
	...
	public List<GfHubK3GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfHubK3GaSession> sessionlist=new ArrayList<GfHubK3GaSession>();
		GfHubK3GaSession currentSession=gfHubK3DAO.getCurrentSession();
		GfHubK3GaSession tempsession =gfHubK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfHubK3GaSession) gfHubK3DAO.getObject(GfHubK3GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfHubK3GaSession session =gfHubK3DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(HubK3Constants.HUB_K3_OPEN_STATUS_INIT) || openStatus1.equals(HubK3Constants.HUB_K3_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(HubK3Constants.HUB_K3_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfHubK3DAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_HUBK3);					
					if(sessionInfo != null){
						...
						gfHubK3DAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfHubK3GaSession> updateAndOpenResult() {
//		List<GfHubK3GaSession> sessionlist=new ArrayList<GfHubK3GaSession>();
//		GfHubK3GaSession currentSession=gfHubK3DAO.getCurrentSession();
//		GfHubK3GaSession tempsession =gfHubK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfhubk3");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[HubK3 start fetch result xml data...]________________---------HubK3");
//						GameHelpUtil.log("hubk3", "___[HubK3 start fetch result xml data...]________________---------HubK3");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[HubK3 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/HubK3.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("hubk3", "HubK3ServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("hubk3", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("hubk3", "HubK3ServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateHubK3SessionOpenResultMethod(GfHubK3GaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_HUBK3, Constants.PUB_STATUS_OPEN);
			...
			para.add(Constants.GAME_TYPE_GF_HUBK3);
			if(orderNum !=null){
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							if (HubK3Constants.GAME_TYPE_GF_HEZHI.equals(playType)) {
								String[] results = result.split(",");
								Integer hezhi = Integer.valueOf(results[0])+Integer.valueOf(results[1])
										+Integer.valueOf(results[2]);
								betRate = gfHubK3DAO.getHeZhiBetRate("和值"+hezhi.toString());//
							} else {
								betRate = det.getBetRate();//
							}
							BigDecimal point = det.getPointMultiple();//倍率
							
							BigDecimal winMoney = HubK3Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_HUBK3);
			...
			gfHubK3DAO.updateObjectList(spList, null);
			gfHubK3DAO.updateObjectList(spDeList, null);
			gfHubK3DAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("hubk3", "HubK3ServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_HUBK3);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_HUBK3);
		...
		gfHubK3DAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfHubK3DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_HUBK3,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_HUBK3,remark);
	}
	...
	public GfHubK3GaSession getCurrentSession(){
		return gfHubK3DAO.getCurrentSession();
	}
	public GfHubK3GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfHubK3DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	public void saveUserBetInfo(String room,Map<Integer,Integer> betMap,List<GaBetOption> list,GfHubK3GaSession gaPick11Session,User user,BigDecimal betAll){
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_HUBK3);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfHubK3DAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_HUBK3, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfHubK3DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfHubK3GaSession GfHubK3GaSession = gfHubK3DAO.getCurrentSession(); // 当前期数信息
		if(GfHubK3GaSession != null){
			String curNo = GfHubK3GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
			GfHubK3GaSession HubK3PreSession = getPreviousSessionBySessionNo(preNo);
			if(HubK3PreSession==null){
				HubK3PreSession=(GfHubK3GaSession) gfHubK3DAO.getObject(GfHubK3GaSession.class, GfHubK3GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfHubK3GaTrend gdk10 ");
			List<Object> trendList = gfHubK3DAO.findObjects(hq); //上期冷热排行榜
			List<GfHubK3GaTrend> saveList = new ArrayList<GfHubK3GaTrend>(); // 本期冷热排行榜

			String results = HubK3PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfHubK3GaTrend trend = (GfHubK3GaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfHubK3GaSession session,String openResult){

		boolean flag=false;
		session.setOpenResult(openResult);
		gfHubK3DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	
	public boolean saveAndOpenResult(GfHubK3GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateHubK3SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("hubk3", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfHubK3DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfHubK3DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfHubK3DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfHubK3GaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_HUBK3);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfHubK3GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_HUBK3);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_HUBK3));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfHubK3DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_HUBK3) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_HUBK3,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_HUBK3,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfHubK3DAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_HubK3, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfHubK3GaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= HubK3DAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = HubK3Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_HUBK3,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfHubK3DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_HubK3,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_HubK3);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_HubK3);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfHubK3GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("hubk3", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfHubK3GaSession session=HubK3DAO.getCurrentSession();
//		if(session!=null){
//			GfHubK3GaSession tempsession =HubK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfHubK3GaOmit omit=gfHubK3DAO.getHubK3OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfHubK3GaOmit preomit=gfHubK3DAO.getHubK3OmitBySessionNo((Integer.parseInt(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfHubK3GaSession se = (GfHubK3GaSession) gfHubK3DAO.getObject(GfHubK3GaSession.class, tempsession.getSessionId()-1);
							preomit=gfHubK3DAO.getHubK3OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfHubK3GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfHubK3GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("hubk3", "HubK3ServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfHubK3GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("hubk3", "HubK3ServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("hubk3", "HubK3ServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("hubk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("hubk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("hubk3", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_HUBK3,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_HUBK3,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_HUBK3,remark);
	}
	...
	public GfHubK3GaSession getFirstSessionByDate(String date) {
		return gfHubK3DAO.getFirstSessionByDate(date);
	}
	public GfHubK3GaSession getEndSessionByDate(String date){
		return gfHubK3DAO.getEndSessionByDate(date);
	}
	public List<GfHubK3GaOmit> findGfHubK3GaOmitList(String string,
			List<Object> para, int num){
		return gfHubK3DAO.findGfHubK3GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfHubK3GaSession tempsession) {
//		GfHubK3GaSession session=HubK3DAO.getCurrentSession();
//		if(session!=null){
//			GfHubK3GaSession tempsession =HubK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_HUBK3);
...
//			}
	...
	@Override
	public boolean saveRevokePrize(GfHubK3GaSession session) {

		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_HUBK3,session.getSessionNo());
		if(result){
