...
import com.gf.k3.ahk3.util.AhK3Util;
import com.gf.pick11.gdpick11.GdPick11Constants;
import com.ram.model.User;
import com.ram.service.user.IUserService;

public class AhK3ServiceImpl extends BaseService implements IAhK3Service {
	private IAhK3DAO gfAhK3DAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setAhK3DAO(IAhK3DAO gfAhK3DAO) {
//		gfAhK3DAO = gfAhK3DAO;
//		super.dao = gfAhK3DAO;
//	}

	public void setGfAhK3DAO(IAhK3DAO gfAhK3DAO) {
		this.gfAhK3DAO = gfAhK3DAO;
		super.dao = gfAhK3DAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_AHK3;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfAhK3DAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfAhK3DAO.updateObjectList(saveList, null);
			flag = "success";
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfAhK3GaSession session = new GfAhK3GaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfAhK3GaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfAhK3GaSession>();
////			log.info("___[start today]__________________________");
//			
//			String startTimeStr = today + AhK3Constants.AH_K3_START_TIME_STR;//开始时间串
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
////			log.info("___[startTimeStr="+startTimeStr+"]");
//			
//			for (int i = 0; i < AhK3Constants.AH_K3_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*AhK3Constants.AH_K3_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfAhK3GaSession k10Session = new GfAhK3GaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,AhK3Constants.AH_K3_TIME_INTERVAL));
//				k10Session.setOpenStatus(AhK3Constants.AH_K3_OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				AhK3DAO.saveObject(k10Session);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfAhK3GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfAhK3DAO.countObjects(hq);
	...
	public List<GfAhK3GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfAhK3GaSession> sessionlist=new ArrayList<GfAhK3GaSession>();
		GfAhK3GaSession currentSession=gfAhK3DAO.getCurrentSession();
		GfAhK3GaSession tempsession =gfAhK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfAhK3GaSession) gfAhK3DAO.getObject(GfAhK3GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfAhK3GaSession session =gfAhK3DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(AhK3Constants.AH_K3_OPEN_STATUS_INIT) || openStatus1.equals(AhK3Constants.AH_K3_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(AhK3Constants.AH_K3_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfAhK3DAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_AHK3);					
					if(sessionInfo != null){
						...
						gfAhK3DAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfAhK3GaSession> updateAndOpenResult() {
//		List<GfAhK3GaSession> sessionlist=new ArrayList<GfAhK3GaSession>();
//		GfAhK3GaSession currentSession=gfAhK3DAO.getCurrentSession();
//		GfAhK3GaSession tempsession =gfAhK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfahk3");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[AhK3 start fetch result xml data...]________________---------AhK3");
//						GameHelpUtil.log("ahk3", "___[AhK3 start fetch result xml data...]________________---------AhK3");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[AhK3 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/AhK3.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("ahk3", "AhK3ServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("ahk3", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("ahk3", "AhK3ServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateAhK3SessionOpenResultMethod(GfAhK3GaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_AHK3, Constants.PUB_STATUS_OPEN);
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							if (AhK3Constants.GAME_TYPE_GF_HEZHI.equals(playType)) {
								String[] results = result.split(",");
								Integer hezhi = Integer.valueOf(results[0])+Integer.valueOf(results[1])
										+Integer.valueOf(results[2]);
								betRate = gfAhK3DAO.getHeZhiBetRate("和值"+hezhi.toString());//
							} else {
								betRate = det.getBetRate();//
							}
							BigDecimal point = det.getPointMultiple();//倍率
							
							BigDecimal winMoney = AhK3Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_AHK3);
			...
			gfAhK3DAO.updateObjectList(spList, null);
			gfAhK3DAO.updateObjectList(spDeList, null);
			gfAhK3DAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("ahk3", "AhK3ServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_AHK3);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_AHK3);
		...
		gfAhK3DAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfAhK3DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_AHK3,remark,
	...
	public GfAhK3GaSession getCurrentSession(){
		return gfAhK3DAO.getCurrentSession();
	}
	public GfAhK3GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfAhK3DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	public void saveUserBetInfo(String room,Map<Integer,Integer> betMap,List<GaBetOption> list,GfAhK3GaSession gaPick11Session,User user,BigDecimal betAll){
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_AHK3);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfAhK3DAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_AHK3, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfAhK3DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfAhK3GaSession GfAhK3GaSession = gfAhK3DAO.getCurrentSession(); // 当前期数信息
		if(GfAhK3GaSession != null){
			String curNo = GfAhK3GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
			GfAhK3GaSession AhK3PreSession = getPreviousSessionBySessionNo(preNo);
			if(AhK3PreSession==null){
				AhK3PreSession=(GfAhK3GaSession) gfAhK3DAO.getObject(GfAhK3GaSession.class, GfAhK3GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfAhK3GaTrend gdk10 ");
			List<Object> trendList = gfAhK3DAO.findObjects(hq); //上期冷热排行榜
			List<GfAhK3GaTrend> saveList = new ArrayList<GfAhK3GaTrend>(); // 本期冷热排行榜

			String results = AhK3PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfAhK3GaTrend trend = (GfAhK3GaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfAhK3GaSession session,String openResult){

		boolean flag=false;
		session.setOpenResult(openResult);
		gfAhK3DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	
	public boolean saveAndOpenResult(GfAhK3GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateAhK3SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("ahk3", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfAhK3DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfAhK3DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfAhK3DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfAhK3GaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_AHK3);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfAhK3GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_AHK3);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_AHK3));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfAhK3DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_AHK3) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_AHK3,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_AHK3,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfAhK3DAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_AhK3, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfAhK3GaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= AhK3DAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = AhK3Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_AHK3,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfAhK3DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_AhK3,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_AhK3);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_AhK3);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfAhK3GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("ahk3", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfAhK3GaSession session=AhK3DAO.getCurrentSession();
//		if(session!=null){
//			GfAhK3GaSession tempsession =AhK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfAhK3GaOmit omit=gfAhK3DAO.getAhK3OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfAhK3GaOmit preomit=gfAhK3DAO.getAhK3OmitBySessionNo((Integer.parseInt(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfAhK3GaSession se = (GfAhK3GaSession) gfAhK3DAO.getObject(GfAhK3GaSession.class, tempsession.getSessionId()-1);
							preomit=gfAhK3DAO.getAhK3OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfAhK3GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfAhK3GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("ahk3", "AhK3ServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfAhK3GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("ahk3", "AhK3ServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("ahk3", "AhK3ServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("ahk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("ahk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("ahk3", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	public GfAhK3GaSession getFirstSessionByDate(String date) {
		return gfAhK3DAO.getFirstSessionByDate(date);
	}
	public GfAhK3GaSession getEndSessionByDate(String date){
		return gfAhK3DAO.getEndSessionByDate(date);
	}
	public List<GfAhK3GaOmit> findGfAhK3GaOmitList(String string,
			List<Object> para, int num){
		return gfAhK3DAO.findGfAhK3GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfAhK3GaSession tempsession) {
//		GfAhK3GaSession session=AhK3DAO.getCurrentSession();
//		if(session!=null){
//			GfAhK3GaSession tempsession =AhK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_AHK3);
...
//			}
	...
	@Override
	public boolean saveRevokePrize(GfAhK3GaSession session) {

		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_AHK3,session.getSessionNo());
		if(result){
