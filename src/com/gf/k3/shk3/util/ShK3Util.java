...
public class ShK3Util {
	...
	public static BigDecimal judgeWinMoney(String playType, String betBall,
		...
		if(ShK3Constants.GAME_TYPE_GF_YIGEHAO.equals(playType)){//猜一个号就中奖
//			String resultVal=results[0]+","+results[1]+","+results[2];
		...
		}else if(ShK3Constants.GAME_TYPE_GF_ERBUTONG.equals(playType)){//二不同号
		...
		}else if(ShK3Constants.GAME_TYPE_GF_ERTONGDAN.equals(playType)){//二同号单选
			String[] one = betBall.split("\\*")[0].split(",");
			String[] two = betBall.split("\\*")[1].split(",");
			List<String> list=new ArrayList<String>();
		...
		}else if(ShK3Constants.GAME_TYPE_GF_ERTONGFU.equals(playType)){//二同号复选
			String array[]=betBall.split(",");
			int num=0;
			for(int i=0;i<array.length;i++){//
				if(array[i].equals(results[0])||array[i].equals(results[1])){
		...
		}else if(ShK3Constants.GAME_TYPE_GF_SANLIANTONG.equals(playType)){//三连号通选
			String value = results[0]+results[1]+results[2];
			if ("123".equals(value) || "234".equals(value)|| "345".equals(value)
					|| "456".equals(value)) {
				winMoney=winMoney.add(betRate);
			}
		}else if(ShK3Constants.GAME_TYPE_GF_SANBUTONG.equals(playType)){//三不同号	
			String array[]=betBall.split(",");
			int num=0;
			for(int i=0;i<array.length;i++){//
		...
		}else if(ShK3Constants.GAME_TYPE_GF_SANTONGDAN.equals(playType)){//三同号单选
			String value = results[0]+results[1]+results[2];
			if (value.equals(betBall)) {
				winMoney=winMoney.add(betRate);
			}
		}else if(ShK3Constants.GAME_TYPE_GF_SANTONGTONG.equals(playType)){//三同号通选
			String value = results[0]+results[1]+results[2];
			if ("111".equals(value) || "222".equals(value)|| "333".equals(value)
					|| "444".equals(value)|| "555".equals(value)|| "666".equals(value)) {
				winMoney=winMoney.add(betRate);
			}
		}else if(ShK3Constants.GAME_TYPE_GF_HEZHI.equals(playType)){//和值
