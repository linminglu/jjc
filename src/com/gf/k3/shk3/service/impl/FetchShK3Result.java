...
public class FetchShK3Result  extends QuartzJobBean {
...
//		
//		if(!Constants.getTimerOpen("shk3.fetchResult.gf")) return;
//		
//		try {
//
//			log.info("_______[shK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");	
//			GameHelpUtil.log("shk3", "_______[shK3 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//			Date now=new Date();
...
//				List<GfShK3GaSession> list=gfShK3Service.updateAndOpenResult();
//				List<GfShK3GaSession> savelist=new ArrayList<GfShK3GaSession>();
//				gfShK3Service.updateGaTrend();
//				
//				Collections.sort(list,new Comparator<GfShK3GaSession>(){
//				    public int compare(GfShK3GaSession o1, GfShK3GaSession o2){  
...
//				    });  
//
//				for(GfShK3GaSession session:list){
//					String flag = gfShK3Service.updateShK3SessionOpenResultMethod(session, session.getOpenResult(),null);
//					if(ParamUtils.chkString(flag)){
//						session.setOpenSuccess(Constants.PUB_STATUS_OPEN);//这些期开奖计算出错
//						savelist.add(session);
//					}else{
//						gfShK3Service.updateDayBetCount(session);
//					}
//					gfShK3Service.updateFetchAndOpenOmit(session);
//				}
//				gfShK3Service.updateObjectList(savelist, null);
//			}
//
//		} catch (Exception e) {
//			GameHelpUtil.log("shk3", "FetchShK3Result.java,line:80,"+e.toString());
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_GF_SHK3);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_GF_SHK3);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_GF_SHK3)){
		...
		//清除之前的定时
		Timer preTimer = GameHelpUtil.getFetchTimerMap(Constants.GAME_TYPE_GF_SHK3);
		if(preTimer!=null){
			preTimer.cancel();
		}
		//重置定时
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_GF_SHK3);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GfShK3GaSession curSession = gfShK3Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GfShK3GaSession openSession = (GfShK3GaSession)gfShK3Service.getObject(GfShK3GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null){
	        				...
	        				List<GfShK3GaSession> list=gfShK3Service.updateAndOpenResult(sessionNoMap);
	        				List<GfShK3GaSession> savelist=new ArrayList<GfShK3GaSession>();
	        				gfShK3Service.updateGaTrend();
	        				Collections.sort(list,new Comparator<GfShK3GaSession>(){
	        				    public int compare(GfShK3GaSession o1, GfShK3GaSession o2){  
