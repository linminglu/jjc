...
import com.gf.k3.shk3.util.ShK3Util;
import com.gf.pick11.gdpick11.GdPick11Constants;
import com.ram.model.User;
import com.ram.service.user.IUserService;

public class ShK3ServiceImpl extends BaseService implements IShK3Service {
	private IShK3DAO gfShK3DAO;
	private IUserService userService;
	private IGaService gaService;
//	public void setShK3DAO(IShK3DAO gfShK3DAO) {
//		gfShK3DAO = gfShK3DAO;
//		super.dao = gfShK3DAO;
//	}

	public void setGfShK3DAO(IShK3DAO gfShK3DAO) {
		this.gfShK3DAO = gfShK3DAO;
		super.dao = gfShK3DAO;
	}
	...
	public String updateInitSession(String initDay){
		//获取彩种初始参数
		String gameType = Constants.GAME_TYPE_GF_SHK3;
		
		HQUtils hq = new HQUtils("from GaSessionInfo gsi where gsi.gameType=?");
		hq.addPars(gameType);
		GaSessionInfo cps = (GaSessionInfo)gfShK3DAO.getObject(hq);//彩种信息
		if(cps==null){
		...
		if(!isTodaySessionInit){
			...
			gfShK3DAO.updateObjectList(saveList, null);
			flag = "success";
			log.info("___[today["+today+"] init completed]__________________________"+code);
			GameHelpUtil.log("shk3", "___[today["+today+"] init completed]__________________________"+code);
		} else {
			flag = "inited";
			log.info("___[today["+today+"] has been inited]__________________________"+code);
			GameHelpUtil.log("shk3", "___[today["+today+"] has been inited]__________________________"+code);
		}
		return flag;
	}
	...
	public List<Object> buildSessionObject(String gameType,List<Object>saveList,String sessionNo,Date curSessionDate,Integer interval){
		GfShK3GaSession session = new GfShK3GaSession();
...
//		//今天是否已经初始化场次
//		boolean isTodaySessionInit = this.checkTodaySessionInit(now);
//		List<GfShK3GaSession> saveList=null;
//		if(!isTodaySessionInit){
//			saveList=new ArrayList<GfShK3GaSession>();
////			log.info("___[start today]__________________________");
//			
//			String startTimeStr = today + ShK3Constants.SH_K3_START_TIME_STR;//开始时间串
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
////			log.info("___[startTimeStr="+startTimeStr+"]");
//			
//			for (int i = 0; i < ShK3Constants.SH_K3_MAX_PART; i++) {
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*ShK3Constants.SH_K3_TIME_INTERVAL * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
//				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
//				log.info("___[start today]__________________________sessionNo:"+sessionNo);
//				GfShK3GaSession k10Session = new GfShK3GaSession();
//				k10Session.setSessionNo(sessionNo);
//				k10Session.setStartTime(curSessionDate);
//				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,ShK3Constants.SH_K3_TIME_INTERVAL));
//				k10Session.setOpenStatus(ShK3Constants.SH_K3_OPEN_STATUS_INIT);
//				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
////				ShK3DAO.saveObject(k10Session);
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GfShK3GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gfShK3DAO.countObjects(hq);
	...
	public List<GfShK3GaSession> updateAndOpenResult(Map<String, SessionItem> sessionNoMap){
		List<GfShK3GaSession> sessionlist=new ArrayList<GfShK3GaSession>();
		GfShK3GaSession currentSession=gfShK3DAO.getCurrentSession();
		GfShK3GaSession tempsession =gfShK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(GfShK3GaSession) gfShK3DAO.getObject(GfShK3GaSession.class, (currentSession.getSessionId()-1));
		}
		final String lastSessionNo=tempsession.getSessionNo();
		
		if(!sessionNoMap.isEmpty()){
			for(String key:sessionNoMap.keySet()){
				GfShK3GaSession session =gfShK3DAO.getPreviousSessionBySessionNo(key);
				if(session!=null){
					String openStatus1 = session.getOpenStatus();//开奖状态
					if(openStatus1.equals(ShK3Constants.SH_K3_OPEN_STATUS_INIT) || openStatus1.equals(ShK3Constants.SH_K3_OPEN_STATUS_OPENING)){
						...
						session.setOpenStatus(ShK3Constants.SH_K3_OPEN_STATUS_OPENED);
						session.setSaveTime(new Date());
						
						gfShK3DAO.saveObject(session);
				...
				if(tempsession.getSessionNo().equals(key)){
					GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_GF_SHK3);					
					if(sessionInfo != null){
						...
						gfShK3DAO.updateObject(sessionInfo, null);
					}
				}
			}
		}
...
//	@Override
//	public List<GfShK3GaSession> updateAndOpenResult() {
//		List<GfShK3GaSession> sessionlist=new ArrayList<GfShK3GaSession>();
//		GfShK3GaSession currentSession=gfShK3DAO.getCurrentSession();
//		GfShK3GaSession tempsession =gfShK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
...
//						// ---------------------------------------------------------------------------
//
//						GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("gfshk3");
//            		    String officalURL ="";
...
//            		    }
//
//						log.info("___[ShK3 start fetch result xml data...]________________---------ShK3");
//						GameHelpUtil.log("shk3", "___[ShK3 start fetch result xml data...]________________---------ShK3");
//						// ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new
//						// Date())+"___[ShK3 start fetch result xml data...]________________",
//						// Constants.getWebRootPath()+"/gamelogo/ShK3.txt",
//						// true);
//
...
//						countRun = null;
//						interrupt();
//						GameHelpUtil.log("shk3", "ShK3ServiceImpl.java,line:358,"+e.toString());
//					}
//				}
//
//			}
...
//							GameHelpUtil.log("shk3", "___[open result success sessionNO["+session.getSessionNo()+"]]");
//						}
//					}
//				}
//				sessionNoMap.clear();
...
//			e.printStackTrace();
//			GameHelpUtil.log("shk3", "ShK3ServiceImpl.java,line:414,"+e.toString());
//		}
//        currentSession=null;
//        
//        return sessionlist;
//	}
	 ...
	 * @return
	 */
	public String updateShK3SessionOpenResultMethod(GfShK3GaSession session,String result,String orderNum){
		try{
			...
			userService.updateUserTradeDetailStatus(session.getSessionNo(), 
					Constants.GAME_TYPE_GF_SHK3, Constants.PUB_STATUS_OPEN);
			...
			para.add(Constants.GAME_TYPE_GF_SHK3);
			if(orderNum !=null){
...
//				    String orderNum = sp.getOrderNum();
				    ...
				    if(spde !=null && spde.size() >0){
						for(int j=0;j<spde.size();j++){
							...
							if (ShK3Constants.GAME_TYPE_GF_HEZHI.equals(playType)) {
								String[] results = result.split(",");
								Integer hezhi = Integer.valueOf(results[0])+Integer.valueOf(results[1])
										+Integer.valueOf(results[2]);
								betRate = gfShK3DAO.getHeZhiBetRate("和值"+hezhi.toString());//
							} else {
								betRate = det.getBetRate();//
							}
							BigDecimal point = det.getPointMultiple();//倍率
							
							BigDecimal winMoney = ShK3Util.judgeWinMoney(playType,betBall,betRate,result).multiply(new BigDecimal(multiple));
...
//				    sp.setBetFlag(Constants.PUB_STATUS_CLOSE);
					...
					if(winCash.compareTo(new BigDecimal(0))==1){
						...
						if(Constants.PROCUREMENT_SERVICE.equals(sp.getBetType())&&Constants.ADD_NO.equals(sp.getIsAddNo())
							...
							para2.add(Constants.GAME_TYPE_GF_SHK3);
			...
			gfShK3DAO.updateObjectList(spList, null);
			gfShK3DAO.updateObjectList(spDeList, null);
			gfShK3DAO.updateObjectList(paList, null);

			return "";
		}catch(Exception e){
			e.printStackTrace();
			GameHelpUtil.log("shk3", "ShK3ServiceImpl.java,line:670,"+e.toString());
	...
	public  void updateWinCount(Map<Integer,BigDecimal> moneyMap){
		...
		for(Integer key:moneyMap.keySet()){
			...
			parame.add(Constants.GAME_TYPE_GF_SHK3);
			...
			if(gaWinCountList.size()== 0){
				...
				gaWinCount.setGameType(Constants.GAME_TYPE_GF_SHK3);
		...
		gfShK3DAO.updateObjectList(gaWinCoList, null);
	}
	...
	private void updateOpenData(GaBetPart part,String remark,String sessionNo) {
		User user = (User)gfShK3DAO.getObject(User.class, part.getUserId());
		userService.saveTradeDetail(user,part.getUserId(), Constants.TRADE_TYPE_INCOME,
				...
				Constants.GAME_TYPE_GF_SHK3,remark,
	...
	private void updateOpenDataRefund(GaBetSponsor betSp,String remark) {
		userService.saveTradeDetail(null,betSp.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAWBACK, betSp.getMoney(), betSp.getJointId(), Constants.GAME_TYPE_GF_SHK3,remark);
	}
	...
	public GfShK3GaSession getCurrentSession(){
		return gfShK3DAO.getCurrentSession();
	}
	public GfShK3GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gfShK3DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	public void saveUserBetInfo(String room,Map<Integer,Integer> betMap,List<GaBetOption> list,GfShK3GaSession gaPick11Session,User user,BigDecimal betAll){
...
//				betDetail.setGameType(Constants.GAME_TYPE_GF_SHK3);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//			gfShK3DAO.updateObject(user, null);
//		}else{
//			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.GAME_TYPE_GF_SHK3, betAll.subtract(tempMoney), null);
//		}
...
//		
//		gfShK3DAO.updateObjectList(betDetailList, null);
	}
	...
	public void updateGaTrend() {
		GfShK3GaSession GfShK3GaSession = gfShK3DAO.getCurrentSession(); // 当前期数信息
		if(GfShK3GaSession != null){
			String curNo = GfShK3GaSession.getSessionNo(); // 当前期号
			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
			GfShK3GaSession ShK3PreSession = getPreviousSessionBySessionNo(preNo);
			if(ShK3PreSession==null){
				ShK3PreSession=(GfShK3GaSession) gfShK3DAO.getObject(GfShK3GaSession.class, GfShK3GaSession.getSessionId()-1);
			}
			Map<String,Boolean> resultMap; // 开奖结果
			
			HQUtils hq = new HQUtils(" from GfShK3GaTrend gdk10 ");
			List<Object> trendList = gfShK3DAO.findObjects(hq); //上期冷热排行榜
			List<GfShK3GaTrend> saveList = new ArrayList<GfShK3GaTrend>(); // 本期冷热排行榜

			String results = ShK3PreSession.getOpenResult();
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GfShK3GaTrend trend = (GfShK3GaTrend) trendList.get(i);
...
//	}
	...
	public boolean saveOpenResult(GfShK3GaSession session,String openResult){

		boolean flag=false;
		session.setOpenResult(openResult);
		gfShK3DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	
	public boolean saveAndOpenResult(GfShK3GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		String flag1 = updateShK3SessionOpenResultMethod(session, session.getOpenResult(),null);
		if(!ParamUtils.chkString(flag1)){
			...
			GameHelpUtil.log("shk3", "___[open result fail sessionNO["+session.getSessionNo()+"], please check...]");
		}
		return flag;
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gfShK3DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GfShK3DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gfShK3DAO.findGaBetDetailById(hql,para);
	}
	...
	public User saveSponsorBet(User user, List<String> list,
			Map<String, Integer> seMap, int num, int buyNum,String isGuaranteed,int guNum,BigDecimal betmoney,BigDecimal needMoney,int betNum,GfShK3GaSession currentSession,BigDecimal buyMoney) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		List<GaBetOption>  opList= gaService.findGaBetOptionByGameType(Constants.GAME_TYPE_GF_SHK3);
		for (String key : seMap.keySet()) {
			String sessionNo = key; //期号
			int multiple = seMap.get(key); //倍数
//			GfShK3GaSession se = this.getPreviousSessionBySessionNo(sessionNo);//获取期号信息
...
//			BigDecimal money = new BigDecimal(betNum *2 /num*buyNum); //购买金额
			...
			sp.setGameType(Constants.GAME_TYPE_GF_SHK3);
			sp.setGameName(Constants.getGameName(Constants.GAME_TYPE_GF_SHK3));
...
//			sp.setIsWinStop(FcConstants.WIN_NO_STOP);
			...
			sp = (GaBetSponsor) gfShK3DAO.saveObjectDB(sp);
			orderNum = sessionNo + Constants.getGameOrderName(Constants.GAME_TYPE_GF_SHK3) +sp.getJointId();
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BUY_LOTO, buyMoney, sp.getJointId(),Constants.GAME_TYPE_GF_SHK3,remark);
			for(String op : list){
...
//			user.setAggregateBetMoney(aggregateBetMoney.add(buyMoney));
			...
			user=userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_FROZEN, betmoney.subtract(buyMoney), sp.getJointId(),Constants.GAME_TYPE_GF_SHK3,remark);
			userService.updateUserMoneyAndBetMoney(user.getUserId());
			userService.updateUserBanlance(user.getUserId());
		}
		gfShK3DAO.updateObjectList(saveList, null);
//		userService.saveTradeDetail(user, user.getUserId(),  Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_ShK3, betmoney, regId);
	 ...
	 * 代购 自己购买
	 */
	public User saveProcurementServiceBet(User user,List<String> list, Map<String, Integer> seMap,String isAddNo, String isWinStop,BigDecimal betmoney,int betNum,GfShK3GaSession currentSession) {
		List<Object> saveList = new ArrayList<Object>();
		String batchNum =""; //批次号，是否属于同一追号
		batchNum = System.currentTimeMillis() +DcbUtil.getTwo();
		//投注与明细关联
...
//			hql.addHsql(" and op.gameType =? ");
//			hql.addPars(Constants.GAME_TYPE_GF_DCB);
//			List<Object>  list= ShK3DAO.findObjects(hql);
...
//			String[] options = bet.split(";");
			for(String op : list){
				...
				int betCount = ShK3Util.getTotalBetNum(playType+"",opTitle);//投注注数
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(),
				...
				Constants.GAME_TYPE_GF_SHK3,remark, sessionNo, user.getUserType(),user.getLoginName());
		...
		gfShK3DAO.updateObjectList(rlList, null);
...
//		List<GfDcbDTO>  gbslist=gaService.findGaBetSponsorListByGameTypeAndBetType(Constants.GAME_TYPE_GF_ShK3,Constants.PUB_STATUS_OPEN);//合买
...
//					udt.setModelType(Constants.GAME_TYPE_GF_ShK3);
...
//								udt1.setModelType(Constants.GAME_TYPE_GF_ShK3);
...
//	}
	 ...
	 */
	@Override
	public void updateFetchAndOpenOmit(GfShK3GaSession tempsession){
		log.info(">>>>>>>>>>>>>>>>>>>>遗漏");
		GameHelpUtil.log("shk3", ">>>>>>>>>>>>>>>>>>>>遗漏");
//		GfShK3GaSession session=ShK3DAO.getCurrentSession();
//		if(session!=null){
//			GfShK3GaSession tempsession =ShK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			...
			if(tempsession!=null){
				if(ParamUtils.chkString(tempsession.getOpenResult())){
					...
					GfShK3GaOmit omit=gfShK3DAO.getShK3OmitBySessionNo(tempsession.getSessionNo());
					
					if(omit==null){
						GfShK3GaOmit preomit=gfShK3DAO.getShK3OmitBySessionNo((Integer.parseInt(tempsession.getSessionNo())-1)+"");
						if(preomit == null){
							GfShK3GaSession se = (GfShK3GaSession) gfShK3DAO.getObject(GfShK3GaSession.class, tempsession.getSessionId()-1);
							preomit=gfShK3DAO.getShK3OmitBySessionNo(se.getSessionNo());
						}

						if(preomit!=null){
							omit=new GfShK3GaOmit();
							...
							for(int i=0;i<array.length;i++){
									try {
										f= GfShK3GaOmit.class.getDeclaredField("fenbu"+Integer.parseInt(array[i]));
										f.setAccessible(true);  
										f.set(omit, 0);
										map.put(Integer.parseInt(array[i])+"", "");
									} catch (Exception e) {
										GameHelpUtil.log("shk3", "ShK3ServiceImpl.java,line:1512,"+e.toString());
										return;
									}
									sum=sum+Integer.parseInt(array[i]);
							...
							for(String key:map.keySet()){
								if(ParamUtils.chkString(map.get(key))){
									try {
										f= GfShK3GaOmit.class.getDeclaredField("fenbu"+key);
										f.setAccessible(true);
										f.set(omit, Integer.parseInt(f.get(preomit).toString())+1);
									} catch (Exception e) {
										GameHelpUtil.log("shk3", "ShK3ServiceImpl.java,line:1545,"+e.toString());
										e.printStackTrace();
										return;
									} 
								}
							...
							try {
								...
								GameHelpUtil.log("shk3", "ShK3ServiceImpl.java,line:1608,"+e.toString());
								e.printStackTrace();
								return;
							}
							omit.setHe(sum);
					...
					log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
					GameHelpUtil.log("shk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期无结果");
				}	
			}else{
				log.info(">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
				GameHelpUtil.log("shk3", ">>>>>>>>>>>>>>>>>>>>遗漏前一期不存在");
			}
//		}

		log.info(">>>>>>>>>>>>>>>>>>>>遗漏结束");
		GameHelpUtil.log("shk3", ">>>>>>>>>>>>>>>>>>>>遗漏结束");
	}	
...
//						User user= dto.getUser();
	...
	private void updateUserBaodiBack(GaBetSponsor sponsor,BigDecimal reMoney,String remark) {//退回部分保底金额
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_GUA_BACK, reMoney, sponsor.getJointId(), Constants.GAME_TYPE_GF_SHK3,remark);
	}
	
	
	private void updateUserBetBack(GaBetSponsor sponsor,String remark) {//方案撤单  退回全部保底
		userService.saveTradeDetail(null,sponsor.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, sponsor.getGuaranteedMoney(), sponsor.getJointId(), Constants.GAME_TYPE_GF_SHK3,remark);
	}
	
	
	private void updateUserBetBack(GaBetPart betPart, BigDecimal betMoney,
			String remark) {//退回参与用户金额
		userService.saveTradeDetail(null,betPart.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_DRAW, betMoney, betPart.getRid(), Constants.GAME_TYPE_GF_SHK3,remark);
	}
	...
	public GfShK3GaSession getFirstSessionByDate(String date) {
		return gfShK3DAO.getFirstSessionByDate(date);
	}
	public GfShK3GaSession getEndSessionByDate(String date){
		return gfShK3DAO.getEndSessionByDate(date);
	}
	public List<GfShK3GaOmit> findGfShK3GaOmitList(String string,
			List<Object> para, int num){
		return gfShK3DAO.findGfShK3GaOmitList(string,para,num);
	}

	@Override
	public void updateDayBetCount(GfShK3GaSession tempsession) {
//		GfShK3GaSession session=ShK3DAO.getCurrentSession();
//		if(session!=null){
//			GfShK3GaSession tempsession =ShK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(session.getSessionNo())-1)+"");
			if(tempsession!=null){
				...
				//如果上期已开奖
					...
					spPara.add(Constants.GAME_TYPE_GF_SHK3);
...
//			}
	...
	@Override
	public boolean saveRevokePrize(GfShK3GaSession session) {

		boolean result = gaService.saveGfRevokePrize(session.getSessionId(), Constants.GAME_TYPE_GF_SHK3,session.getSessionNo());
		if(result){
