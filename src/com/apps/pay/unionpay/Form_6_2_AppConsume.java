...
public class Form_6_2_AppConsume extends DemoBase {
	...
	public static void main(String[] args) {
		...
		// 商户号码，请改成自己的商户号
		data.put("merId", "898111172780106");
		// 商户订单号，8-40位数字字母
		data.put("orderId",
	...
	public static Map<String, String> resmap(OrderDTO dto) {
		...
		data.put("frontUrl", "http://localhost:8080/ACPTest/acp_front_url.do");
		// 后台通知地址 -- 同步支付通知
		data.put("backUrl", dto.getCallback());
		// 接入类型，商户接入填0 0- 商户 ， 1： 收单， 2：平台商户
		data.put("accessType", "0");
		// 商户号码，请改成自己的商户号
		data.put("merId", Constants.getUnionPartnerId());
		// 商户订单号，8-40位数字字母
		data.put("orderId", dto.getOrderNum());

		// 订单发送时间，取系统时间
		data.put("txnTime",
				new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
		// 交易金额，单位分
		BigDecimal payMoney = dto.getTotalPrice();
		data.put("txnAmt", ParamUtils.BigFormatIntString(payMoney));
		// 交易币种
		data.put("currencyCode", "156");
		// 请求方保留域，透传字段，查询、通知、对账文件中均会原样出现
		// data.put("reqReserved", "透传信息");
