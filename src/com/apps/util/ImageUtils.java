...
public class ImageUtils {
	...
	public static void main(String[] args) {
		ImageUtils.scale2("D:/wb070801012.jpg", "D:/abc_scale2.jpg", 100, 150, true);// 测试OK
	}
...
//		 方法一：按比例缩放
		...
		// 方法二：按高度和宽度缩放
		ImageUtils.scale2("D:/wb070801012.jpg", "D:/abc_scale2.jpg", 150, 100, true);// 测试OK
		// 2-切割图像：
		// 方法一：按指定起点坐标和宽高切割
	...
	/** 
	  ...
	  *            缩放后的图像宽度
	  */
	public static long scalePic(String srcImageFile, String tarImageFile, int picWidth) {
		try {
			BufferedImage src = ImageIO.read(new File(srcImageFile)); // 读入文件
			...
			double scale;
			System.out.println("____[scalePic]["+width+","+picWidth+"]"+srcImageFile+","+tarImageFile);
			if (width > picWidth) {
				scale = (double) picWidth / (double) width;
				System.out.println("___[scale]"+scale);
				...
				g.dispose();
				ImageIO.write(tag, "JPEG", new File(tarImageFile));// 输出到文件流
				return new File(tarImageFile).length();
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
