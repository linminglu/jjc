...
public class JsK3ServiceImpl  extends BaseService implements IJsK3Service {
	private IJsK3DAO jsK3DAO;
	private IUserService userService;
	private IGaService gaService;
	
	public void setJsK3DAO(IJsK3DAO jsK3DAO) {
		this.jsK3DAO = jsK3DAO;
		super.dao = jsK3DAO;
...
//		log.info("___[start]__________________________");
		...
		List<JsK3GaSession> saveList = null;
		if(!isTodaySessionInit){
//			log.info("___[start today]__________________________");
			saveList = new ArrayList<JsK3GaSession>();
			String startTimeStr = today + JsK3Constants.JSK3_START_TIME_S;//开始时间串
			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
			
//			log.info("___[startTimeStr="+startTimeStr+"]");
			
			for (int i = 0; i < JsK3Constants.JSK3_MAX_PART; i++) {
				//计算出当前场次的时间
				long diffTime = startDate.getTime() + i*JsK3Constants.JSK3_TIME_INTERVAL * 60 * 1000;
				Date curSessionDate = new Date(diffTime);
				
				//String openResult = GameConstants.getRandomPick11Result();//随机生成5个中奖数字[1-21]
				String sessionNo = this.getTodaySessionNo(now, i+1);//期号
				log.info("___[start today]__________________________sessionNo:"+sessionNo);
				JsK3GaSession jsK3Session = new JsK3GaSession();
				jsK3Session.setSessionNo(sessionNo);
				jsK3Session.setStartTime(curSessionDate);
				jsK3Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,JsK3Constants.JSK3_TIME_INTERVAL));
				jsK3Session.setOpenStatus(JsK3Constants.JSK3_OPEN_STATUS_INIT);
				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
//				jsK3DAO.saveObject(jsK3Session);
				saveList.add(jsK3Session);
				//log.info("___[out]__[sessionNo="+sessionNo+"][diffTime="+diffTime+"][startDate.getTime()="+startDate.getTime()+"]");
			}
			jsK3DAO.updateObjectList(saveList, null);
			flag = "success";
...
//		log.info("___[start]__________________________");
		...
		//今天日期处理 yyyy-MM-dd
		Date now = DateTimeUtil.getDateTimeOfDays(DateTimeUtil.getJavaUtilDateNow(), 1);
		String today = DateTimeUtil.DateToString(now);
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from JsK3GaSession bgs where bgs.startTime>? and bgs.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = jsK3DAO.countObjects(hq);
	 ...
	 * @return
	 */
	public boolean openJsK3SessionOpenResultMethod(JsK3GaSession session,String result){
...
//						updateOpenData(detail,remark.toString());
				...
				userService.updateUserTradeDetailStatus(session.getSessionNo(), 
						Constants.GAME_TYPE_XY_JSK3, Constants.PUB_STATUS_OPEN);
...
//	public boolean openJsK3SessionOpenResultMethod(JsK3GaSession jsK3session,String result){
//		HQUtils hql = new HQUtils("from GaBetDetail gd where 1 = 1");
//		hql.addHsql(" and gd.sessionId=? ");
//		hql.addPars(jsK3session.getSessionId());
//		hql.addHsql(" and gd.betFlag=? " );
//		hql.addPars("1");
//		hql.addHsql(" and gd.gameType=? ");
//		hql.addPars(Constants.GAME_TYPE_XY_JSK3);
//		
...
//		bet.setSessionId(jsK3session.getSessionId());
//		bet.setSessionNo(jsK3session.getSessionNo());
//		
//		String countResult = "";
//		String openResult = jsK3session.getOpenResult();
//		if(ParamUtils.chkString(openResult)){
//			String array[]=openResult.split(",");
//			int sum=0;				
//			if(array.length>=2){
...
//				
//				
//				String flag=judgeWin(jsK3session.getOpenResult(),detail);//0未中奖  1打和  2中奖
//				detail.setOpenResult(countResult);
//				if(flag.equals("2")){//中奖
////					User user=(User) gaService.getObject(User.class, detail.getUserId());
//					detail.setWinResult(JsK3Constants.JSK3_WIN);//中奖
...
////					userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME, Constants.CASH_TYPE_CASH_BET_JSK3, wincash, detail.getBetDetailId());	
//					StringBuffer remark=new StringBuffer();
//					remark.append("彩票中奖 奖金 ").append(wincash).append("元");
////					user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, wincash, detail.getBetDetailId(), Constants.GAME_TYPE_XY_JSK3,remark.toString());
...
//				}else{//未中奖
//					detail.setWinCash(new BigDecimal(0));
//					detail.setWinResult(JsK3Constants.JSK3_WIN_NOT);//未中奖
//					totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//					try {
//						jsK3DAO.updateObject(detail, null);
//					} catch (Exception e) {
//						e.printStackTrace();
//					}
//				}
...
//		jsK3DAO.saveObject(detail);
//		user=userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_JSK3,remark);
//	}
	...
	public String updateFetchAndOpenResult(Map<String, SessionItem> sessionNoMap) {
		...
		//当前场次及开奖场次处理
		JsK3GaSession curtSession = jsK3DAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		JsK3GaSession lastSession = (JsK3GaSession)jsK3DAO.getObject(JsK3GaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<JsK3GaSession> openedList = new ArrayList<JsK3GaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			...
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				JsK3GaSession session = jsK3DAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = openJsK3SessionOpenResultMethod(session, result);
						if(flag){
			...
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_JSK3);
			if(sessionInfo!=null){
				...
				jsK3DAO.saveObject(sessionInfo);
			...
			for(JsK3GaSession session:openedList){
...
//		JsK3GaSession currentSession= jsK3DAO.getCurrentSession();
//		JsK3GaSession tempsession =jsK3DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession==null){
//			tempsession=(JsK3GaSession) jsK3DAO.getObject(JsK3GaSession.class, (currentSession.getSessionId()-1));
//		}
//		final String lastSessionNo=tempsession.getSessionNo();
//		
//			if(!sessionNoMap.isEmpty()){
//				for(String key:sessionNoMap.keySet()){
//					HQUtils hq = new HQUtils("from JsK3GaSession gks where gks.sessionNo=?");
//					hq.addPars(key);
//					JsK3GaSession session = (JsK3GaSession)jsK3DAO.getObject(hq);
//					if(session!=null){
//						String openStatus = session.getOpenStatus();//开奖状态
//						
//						if(openStatus.equals(JsK3Constants.JSK3_OPEN_STATUS_INIT) || openStatus.equals(JsK3Constants.JSK3_OPEN_STATUS_OPENING)){//状态为初始化 或 开奖中 则开始开奖
//							//更新开奖结果
...
//							
//							//进入开奖程序方法
//							boolean flag = this.openJsK3SessionOpenResultMethod(session, openResult);
//							
//							if(flag){
//								String countResult="";
//								//更新场次状态，TODO 盈亏数据也需要在开奖
//								session.setOpenTime(DateTimeUtil.stringToDate(sessionItem.getTime(), "yyyy-MM-dd HH:mm:ss"));
//								session.setOpenStatus(JsK3Constants.JSK3_OPEN_STATUS_OPENED);
//								jsK3DAO.saveObject(session);
//							}else{
//								GameHelpUtil.log(Constants.GAME_TYPE_XY_JSK3,"open flag FAIL -_- ["+session.getSessionNo()+"]");
//							}
//						}
//						// 把资金明细里投注记录状态值改为有效
//						userService.updateUserTradeDetailStatus(session.getSessionNo(), 
...
//				}
//				
//				GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_XY_JSK3);
//				if(sessionInfo!=null){
//					SessionItem lastItem = (SessionItem)sessionNoMap.get(lastSessionNo);//上一期
//					if(lastItem!=null){
...
//            	
//            			//从这里 ---------------------------------------------------------------------------
//            		    GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("xyjsk3");
//            		    String officalURL ="";
...
//            		    }
//            		    
//	               		log.info("___[jsK3 start fetch result xml data...]________________");
//	               		ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new Date())+"___[jsK3 start fetch result xml data...]________________", Constants.getWebRootPath()+"/gamelogo/bjlu28.txt", true);
...
//	}
	...
	public JsK3GaSession getCurrentSession() {
		return jsK3DAO.getCurrentSession();
	}
	
	public JsK3GaSession getPreviousSessionBySessionNo(String sessionNo) {
		return jsK3DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	public void saveUserBetInfo(String room, Map<Integer, Integer> betMap,
			List<GaBetOption> list, JsK3GaSession session, User user,
			BigDecimal betAll) {
		//彩种缓存
		GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_JSK3);
		//投注与明细关联
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll, null, 
				Constants.GAME_TYPE_XY_JSK3,remark,session.getSessionNo(),user.getUserType(),user.getLoginName());
		...
		jsK3DAO.updateObjectList(rlList, null);
	}
...
//				betDetail.setBetName(this.getBetNameByOptionType(betOption.getPlayType(),betOption.getOptionType().toString()));
//				betDetail.setOptionTitle(betOption.getOptionTitle());
//				betDetail.setGameType(Constants.GAME_TYPE_XY_JSK3);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//		
//
////			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_JSK3, betAll.subtract(tempMoney), null);
...
//		user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll.subtract(tempMoney), null, Constants.GAME_TYPE_XY_JSK3,remark.toString());
//
//
//		//更新用户实时余额  --by.cuisy.20171209
//		userService.updateUserMoney(user.getUserId());
//		userService.updateUserBanlance(user.getUserId());
//		
//		jsK3DAO.updateObjectList(betDetailList, null);	
//	}
	
	public List<JsK3GaTrend> findJsK3TrendList() {
		return jsK3DAO.findJsK3GaTrendList();
	}
	
	public PaginationSupport findJsK3GaSessionList(String hql,
			List<Object> para, int pageNum, int pageSize) {
		return jsK3DAO.findJsK3GaSessionList(hql,para,pageNum,pageSize);
	}
	 ...
	 public void updateTrendResult(JsK3GaSession session){
		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
			List<JsK3GaTrend> list=jsK3DAO.findJsK3GaTrendAllList();
			List<JsK3GaTrend> savelist=new ArrayList<JsK3GaTrend>();
...
//				return;
//			}
//			JsK3GaSession cureent = jsK3DAO.getCurrentSession();
//			String lastSessionNo=(Integer.parseInt(cureent.getSessionNo())-1)+"";
//			JsK3GaSession session =jsK3DAO.getPreviousSessionBySessionNo(lastSessionNo);
//			if(session == null){
//				session = (JsK3GaSession) jsK3DAO.getObject(JsK3GaSession.class, cureent.getSessionId()-1);
//			}
//			if(session.getOpenStatus().equals(JsK3Constants.JSK3_OPEN_STATUS_OPENED)){
//				List<JsK3GaTrend> list=jsK3DAO.findJsK3GaTrendAllList();
//				List<JsK3GaTrend> savelist=new ArrayList<JsK3GaTrend>();
//				Map<String,Boolean> map=getResult(session.getOpenResult().split(","));
//				if(!map.isEmpty()){
//					for(int i=0;i<list.size();i++){
//						JsK3GaTrend trend=list.get(i);
//						if(map.get(trend.getTrendTitle())!=null&&map.get(trend.getTrendTitle())==true){
//							trend.setTrendCount(trend.getTrendCount()+1);				
//						}else{
//							trend.setTrendCount(0);
...
//			}
//		}
		public boolean saveOpenResult(JsK3GaSession session,String openResult){
			boolean flag=false;
			session.setOpenResult(openResult);
			jsK3DAO.updateObject(session, null);
			flag=true;
			return flag;
		}
		
		public boolean saveAndOpenResult(JsK3GaSession session,String openResult){
			boolean flag=false;
			session.setOpenResult(openResult);
			boolean flag1 = openJsK3SessionOpenResultMethod(session, openResult);
			if(flag1){      
				session.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
				session.setOpenStatus(JsK3Constants.JSK3_OPEN_STATUS_OPENED);
				jsK3DAO.updateObject(session, null);
		...
		public PaginationSupport findJsK3GaBetList(String hql,
				List<Object> para, int pageNum, int pageSize) {
			return jsK3DAO.findJsK3GaBetList(hql, para, pageNum, pageSize);
		}

		@Override
		public PaginationSupport findGaBetDetail(String hql, List<Object> para,
				int pageNum, int pageSize) {
			return jsK3DAO.findGaBetDetail(hql, para, pageNum, pageSize);
		}
		@Override
		public List<JsK3DTO> findGaBetDetailById(String hql,
				List<Object> para) {
			return jsK3DAO.findGaBetDetailById(hql, para);
		}
...
//			int number=Integer.parseInt(split[3]);
		...
		@Override
		public boolean saveRevokePrize(JsK3GaSession session) {
			//删除JsK3GaBet表的记录
			List<Object> para = new ArrayList<Object>();
			StringBuffer hql = new StringBuffer();
			hql.append(" and sessionId = ? ");
			para.add(session.getSessionId());
			jsK3DAO.deleteJsK3GaBet(hql.toString(),para);

			boolean result = gaService.saveXyRevokePrize(session.getSessionId(), Constants.GAME_TYPE_XY_JSK3,session.getSessionNo());
			if(result){
