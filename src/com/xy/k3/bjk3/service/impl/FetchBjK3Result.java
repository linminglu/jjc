...
//		if(!Constants.getTimerOpen("bjk3.fetchResult.xy")) return;
//		
//		try {
//			log.info("_______[BjK3 fetch session]["+DateTimeUtil.getDateTime()+"]___________________");
//
//			bjK3Service.updateFetchAndOpenResult();
//
//			log.info("_______[BjK3 fetch session end]["+DateTimeUtil.getDateTime()+"]___________________");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_BJK3);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_BJK3);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_BJK3)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_BJK3, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_BJK3);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		BjK3GaSession curSession = bjK3Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		BjK3GaSession openSession = (BjK3GaSession)bjK3Service.getObject(BjK3GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = bjK3Service.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_BJK3,openSessionNo);//移除开奖标识
