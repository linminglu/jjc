...
public class FetchSfPk10Result  extends QuartzJobBean {
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_SFPK10);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_SFPK10);//抓取间隔毫秒
	int delayOpenTime = GameHelpUtil.getDelayOpenTime(Constants.GAME_TYPE_XY_SFPK10);//延迟开奖秒数(系统开)
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_SFPK10)){
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_SFPK10);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		SfPk10GaSession curSession = sfPk10Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		SfPk10GaSession openSession = (SfPk10GaSession)sfPk10Service.getObject(SfPk10GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
...
//	        		}
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = sfPk10Service.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_SFPK10,openSessionNo);//移除开奖标识
