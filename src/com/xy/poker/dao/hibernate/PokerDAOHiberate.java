package com.xy.poker.dao.hibernate;
...
public class PokerDAOHiberate extends AbstractBaseDAOHibernate implements
		IPokerDAO {
	...
	public PokerGaSession getCurrentSession() {
		...
		//Date nowEnd = DateTimeUtil.getDateTimeOfMinutes(now, PokerConstants.LUCKY28_TIME_INTERVAL);
		
		String todayYyyymmdd = DateTimeUtil.DateToString(now);
		Date todayStart = DateTimeUtil.parse(todayYyyymmdd+" 00:00:00");
		Date todayEnd = DateTimeUtil.parse(todayYyyymmdd+" 08:22:00");
		
		Date todayStart1 = DateTimeUtil.parse(todayYyyymmdd+" 23:02:00");
		Date todayEnd1 = DateTimeUtil.parse(todayYyyymmdd+" 23:59:59");
		
		if(now.compareTo(todayStart)>=0&&now.compareTo(todayEnd)==-1){
			now=DateTimeUtil.parse(todayYyyymmdd+" 08:23:00");
		}else if(now.compareTo(todayStart1)>=0&&now.compareTo(todayEnd1)<=0){
			now=DateTimeUtil.getDateTimeOfDays(DateTimeUtil.parse(todayYyyymmdd+" 08:23:00"),1);
		}
		List<PokerGaSession> list = getSession()
				.createQuery(
						"from PokerGaSession gks where gks.startTime<=? and gks.endTime>? and (gks.openStatus=? or gks.openStatus=?) order by gks.sessionId")
				.setTimestamp(0, now).setTimestamp(1, now)
				.setString(2, PokerConstants.POKER_OPEN_STATUS_INIT)
				.setString(3, PokerConstants.POKER_OPEN_STATUS_OPENING)
				.setMaxResults(1).list();
		if (list != null && list.size() > 0) {
			return (PokerGaSession) list.get(0);
		}
		return null;
	}

	public PokerGaSession getPreviousSessionBySessionNo(String sessionNo) {
		String hql = " from PokerGaSession gks where gks.sessionNo=? ";
	...
	public List<PokerGaTrend> findPokerGaTrendList() {
		String hql = " from PokerGaTrend gks where gks.trendCount >1 order by gks.trendCount desc";
		Query record = getSession().createQuery(hql);
		List list = record.list();
		return list;
	}
	...
	public PaginationSupport findPokerGaSessionList(String hql,
		...
		Query query = makeQuerySQL(" from PokerGaSession ho where 1=1 " + hql,
				para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL(
				"select count(ho.sessionId) from PokerGaSession ho where 1=1 "
						+ hql, para);
	...
	public List<PokerGaTrend> findPokerGaTrendAllList() {
		String hql = " from PokerGaTrend gks where 1=1 ";
		Query record = getSession().createQuery(hql);
		List list = record.list();
		return list;
	}
	...
	public PaginationSupport findPokerGaBetList(String hql, List<Object> para,
		...
		Query query = makeQuerySQL(
				" select new com.xy.poker.model.dto.PokerDTO(ho.sessionNo,ho.totalPoint,ho.winCash,ho.totalPoint-ho.winCash) from PokerGaBet ho where 1=1 "
						+ hql,
				para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL(
				"select count(ho.betId) from PokerGaBet ho where 1=1"
						+ hql, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
			int pageNum, int pageSize) {
		Query query = makeQuerySQL(
				" select new com.xy.poker.model.dto.PokerDTO(ga,u) from GaBetDetail ga,User u where 1=1 "
	...
	public List<PokerDTO> findGaBetDetailById(String hql, List<Object> para) {
		Query query = getSession()
				.createQuery(
						" select new com.xy.poker.model.dto.PokerDTO(ga,u) from GaBetDetail ga,User u where 1=1 and ga.userId = u.userId "
								+ hql).setParameter(0, para.get(0));
		List<PokerDTO> list = query.list();
		return list;
	}

	@Override
	public void deletePokerGaBet(String hql, List<Object> para) {
		String delHql=" delete from PokerGaBet where 1=1 ";
