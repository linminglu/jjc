 ...
 * 定时抓去官网开奖数据
 */
public class FetchBjLu28Result extends QuartzJobBean {
...
//		if(!Constants.getTimerOpen("bjlu28.fetchResult.xy")) return;
//		
//		try {
//			log.info("_______[BjLu28 fetch session]["+DateTimeUtil.getDateTime()+"]___________________");
//
//			bjLu28Service.updateFetchAndOpenResult();
//
//			log.info("_______[BjLu28 fetch session end]["+DateTimeUtil.getDateTime()+"]___________________");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_BJLU28);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_BJLU28);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_BJLU28)){
		...
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_BJLU28);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		BjLu28GaSession curSession = bjLu28Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		BjLu28GaSession openSession = (BjLu28GaSession)bjLu28Service.getObject(BjLu28GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = bjLu28Service.updateFetchAndOpenResult(sessionNoMap);
