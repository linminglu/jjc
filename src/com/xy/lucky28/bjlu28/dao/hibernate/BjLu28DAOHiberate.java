package com.xy.lucky28.bjlu28.dao.hibernate;
...
public class BjLu28DAOHiberate  extends AbstractBaseDAOHibernate implements IBjLu28DAO {
	...
	public BjLu28GaSession getCurrentSession() {
		...
		List<BjLu28GaSession> list = getSession().createQuery("from BjLu28GaSession gks where gks.startTime<=? and gks.endTime>? and (gks.openStatus=? or gks.openStatus=?) order by gks.sessionId")
		.setTimestamp(0, now)
		.setTimestamp(1, now)
		.setString(2, BjLu28Constants.LUCKY28_OPEN_STATUS_INIT)
		.setString(3, BjLu28Constants.LUCKY28_OPEN_STATUS_OPENING)
		.setMaxResults(1)
		.list();
		if(list!=null && list.size()>0){
			return (BjLu28GaSession)list.get(0);
		}
		return null;
	}
	...
	public BjLu28GaSession getPreviousSessionBySessionNo(String sessionNo) {
		String  hql=" from BjLu28GaSession gks where gks.sessionNo=? ";
	...
	public List<BjLu28GaTrend> findBjLu28GaTrendList() {
		String  hql=" from BjLu28GaTrend gks where gks.trendCount >1 order by gks.trendCount desc";
		Query record =getSession().createQuery(hql);
		List list=record.list();
		return list;
	}
	...
	public PaginationSupport findBjLu28GaSessionList(String hql,
		...
		Query query = makeQuerySQL( " from BjLu28GaSession ho where 1=1 "+hql,para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from BjLu28GaSession ho where 1=1 "+hql, para);
	...
	public List<BjLu28GaTrend> findBjLu28GaTrendAllList(){
		String  hql=" from BjLu28GaTrend gks where 1=1 ";
		Query record =getSession().createQuery(hql);
		List list=record.list();
		return list;
	}
	...
	public PaginationSupport findBjLu28GaBetList(String hql,
			List<Object> para, int pageNum, int pageSize) {
		Query query = makeQuerySQL( " select new com.xy.lucky28.bjlu28.model.dto.BjLu28BetDTO(ho.sessionNo,ho.totalPoint,ho.winCash,ho.totalPoint-ho.winCash) from BjLu28GaBet ho where 1=1 "
				+ hql, para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.betId) from BjLu28GaBet ho where 1=1" +
				hql, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		Query query = makeQuerySQL( " select new com.xy.lucky28.bjlu28.model.dto.BjLu28DTO(ga,u) from GaBetDetail ga,User u where 1=1 "
	...
	public List<BjLu28DTO> findGaBetDetailById(String hql, List<Object> para) {
	    Query query = getSession().createQuery(" select new com.xy.lucky28.bjlu28.model.dto.BjLu28DTO(ga,u) from GaBetDetail ga,User u where 1=1 and ga.userId = u.userId "
	    		+ hql)
	    		.setParameter(0, para.get(0));
	    List<BjLu28DTO> list = query.list();
		return list;
	}

	@Override
	public void deleteBjLu28GaBet(String hql, List<Object> para) {
		String delHql=" delete from BjLu28GaBet where 1=1 ";
