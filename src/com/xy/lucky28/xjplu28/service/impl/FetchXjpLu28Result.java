 ...
 * 定时抓去官网开奖数据
 */
public class FetchXjpLu28Result extends QuartzJobBean {
...
//		if(!Constants.getTimerOpen("xjplu28.fetchResult.xy")) return;
//		
//		try {
//			log.info("_______[xjpLu28 fetch session]["+DateTimeUtil.getDateTime()+"]___________________");
//
//			xjpLu28Service.updateFetchAndOpenResult();
//
//			log.info("_______[xjpLu28 fetch session end]["+DateTimeUtil.getDateTime()+"]___________________");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_XJPLU28);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_XJPLU28);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_XJPLU28)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_XJPLU28, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_XJPLU28);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		XjpLu28GaSession curSession = xjpLu28Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		XjpLu28GaSession openSession = (XjpLu28GaSession)xjpLu28Service.getObject(XjpLu28GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = xjpLu28Service.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_XJPLU28,openSessionNo);//移除开奖标识
