...
import com.xy.k10.gxk10.GxK10Constants;
import com.xy.k10.gxk10.dao.IGxK10DAO;
import com.xy.k10.gxk10.model.GxK10GaBet;
...
public class GxK10ServiceImpl extends BaseService implements IGxK10Service {
	private IGxK10DAO gxK10DAO;
	private IUserService userService;
	private IGaService gaService;
	public void setGxK10DAO(IGxK10DAO gxK10dao) {
		gxK10DAO = gxK10dao;
		super.dao = gxK10DAO;
	}
	...
	public String updateFetchAndOpenResult(Map<String, SessionItem> sessionNoMap) {
		...
		//当前场次及开奖场次处理
		GxK10GaSession curtSession = gxK10DAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		GxK10GaSession lastSession = (GxK10GaSession)gxK10DAO.getObject(GxK10GaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<GxK10GaSession> openedList = new ArrayList<GxK10GaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			...
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				GxK10GaSession session = gxK10DAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = openGxK10SessionOpenResultMethod(session, result);
						if(flag){
			...
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_GXK10);
			if(sessionInfo!=null){
				...
				gxK10DAO.saveObject(sessionInfo);
			...
			for(GxK10GaSession session:openedList){
...
//		GxK10GaSession currentSession=gxK10DAO.getCurrentSession();
//		GxK10GaSession tempsession =gxK10DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
//			tempsession=(GxK10GaSession) gxK10DAO.getObject(GxK10GaSession.class, (currentSession.getSessionId()-1));
//		}
//		final String lastSessionNo=tempsession.getSessionNo();
//			if(!sessionNoMap.isEmpty()){
//				for(String key:sessionNoMap.keySet()){
//					GxK10GaSession session =gxK10DAO.getPreviousSessionBySessionNo(key);
//					if(session!=null){
...
//							boolean flag = openGxK10SessionOpenResultMethod(session, session.getOpenResult());
//							if(flag){
...
//								gxK10DAO.saveObject(session);
//							}else{
//								GameHelpUtil.log(Constants.GAME_TYPE_XY_GXK10,"open flag FAIL -_- ["+session.getSessionNo()+"]");
//							}
//						}
//						// 把资金明细里投注记录状态值改为有效
//						userService.updateUserTradeDetailStatus(session.getSessionNo(), 
//								Constants.GAME_TYPE_XY_GXK10, Constants.PUB_STATUS_OPEN);
//					}
//				}
//				
//				GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_XY_GXK10);
//				if(sessionInfo != null){
//					SessionItem lastItem = (SessionItem)sessionNoMap.get(lastSessionNo);//上一期
//					if(lastItem!=null){
//						sessionInfo.setOpenResult(lastItem.getResult());
//						sessionInfo.setOpenSessionNo(lastSessionNo);
//						sessionInfo.setEndTime(DateTimeUtil.StringToDate(lastItem.getTime()));							
//					}
//				}
//				sessionInfo.setLatestSessionNo(currentSession.getSessionNo());
//				gxK10DAO.updateObject(sessionInfo, null);
//				CacheUtil.updateGameList();
//			}
//	}
...
//
//            			//从这里 ---------------------------------------------------------------------------
//            		    GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("xygxk10");
//            		    String officalURL ="";
...
//            		    }
//            		    sessionInfo=null;
//	               		log.info("___[gxk10 start fetch result xml data...]________________---------gxk10");
//	               		ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new Date())+"___[gxk10 start fetch result xml data...]________________", Constants.getWebRootPath()+"/gamelogo/gxk10.txt", true);
//	    	            
...
//	            	    ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new Date())+resultXML, Constants.getWebRootPath()+"/gamelogo/gxk10.txt", true);
//
...
//						sessionInfo.setOpenSessionNo(lastSessionNo);
//						sessionInfo.setEndTime(DateTimeUtil.StringToDate(timeMap.get(lastSessionNo)));							
//					}else{
...
//							sessionInfo.setEndTime(DateTimeUtil.StringToDate(timeMap.get(lastNo)));							
//						}
//					}
//				}
////				else{
////					sessionInfo=new  GaSessionInfo();
////					sessionInfo.setGameTitle("广西快乐十分");
////					sessionInfo.setGameType(Constants.GAME_TYPE_XY_GXK10);
////					sessionInfo.setLatestSessionNo(currentSession.getSessionNo());
////					if(sessionNoMap.get(lastSessionNo)!=null){
////						sessionInfo.setOpenResult(sessionNoMap.get(lastSessionNo));
////						sessionInfo.setOpenSessionNo(lastSessionNo);
...
//	}
	 ...
	 * @return
	 */
	public boolean openGxK10SessionOpenResultMethod(GxK10GaSession session,String result){
		String gameCode = Constants.getGameCodeOfGameType(Constants.GAME_TYPE_XY_GXK10);
		try{
...
//	public boolean openGxK10SessionOpenResultMethod(GxK10GaSession session,String result){
//		try{
////			GxK10GaSession session =bjPk10DAO.getPreviousSessionBySessionNo(sessionNo);
//			List<Object> para = new ArrayList<Object>();
//			StringBuffer hql = new StringBuffer();
...
//							
//							detail.setPayoff(wincash.subtract(new BigDecimal(detail.getBetMoney())).setScale(2, BigDecimal.ROUND_HALF_UP));
////						userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME, Constants.CASH_TYPE_CASH_BET_GXK10, wincash, detail.getBetDetailId());	
//							
...
//							totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//							try {
//								gxK10DAO.updateObject(detail, null);
//							} catch (Exception e) {
//								e.printStackTrace();
//							}
//						}
...
//						if(flag.equals("2")){//中奖
//							
//							detail.setWinResult(GxK10Constants.GX_K10_WIN);//中奖
...
////						user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, wincash, detail.getBetDetailId(), Constants.GAME_TYPE_XY_GXK10,remark.toString());
...
//						}else if(flag.equals("1")){//打和
////						User user=(User) gaService.getObject(User.class, detail.getUserId());
//							detail.setWinResult(GxK10Constants.GX_K10_WIN_HE);//打和
...
//						}else{//未中奖
//							detail.setWinCash(new BigDecimal(0));
//							detail.setWinResult(GxK10Constants.GX_K10_WIN_NOT);//未中奖
//							totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//							try {
//								gxK10DAO.updateObject(detail, null);
//							} catch (Exception e) {
...
//		gxK10DAO.saveObject(detail);
//		user=userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_GXK10,remark);
//	}

	
	public GxK10GaSession getCurrentSession(){
		return gxK10DAO.getCurrentSession();
	}
	public GxK10GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gxK10DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	public User saveUserBetInfo(String room,Map<Integer,Integer> betMap,List<GaBetOption> list,GxK10GaSession session,User user,BigDecimal betAll){
		//彩种缓存
		GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_GXK10);
		//投注与明细关联
...
//				betDetail.setBetOptionId(firstItem.getBetOptionId());//在上面循环判断中赋值
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll, null, 
				Constants.GAME_TYPE_XY_GXK10,remark,session.getSessionNo(),user.getUserType(),user.getLoginName());
		...
		gxK10DAO.updateObjectList(rlList, null);
...
//				
//				betDetail.setGameType(Constants.GAME_TYPE_XY_GXK10);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
//		
//
////			user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_GXK10, betAll.subtract(tempMoney), null);
...
//		user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll.subtract(tempMoney), null, Constants.GAME_TYPE_XY_GXK10,remark.toString());
//
//
//		//更新用户实时余额  --by.cuisy.20171209
//		userService.updateUserMoney(user.getUserId());
//		userService.updateUserBanlance(user.getUserId());
//		
//		gxK10DAO.updateObjectList(betDetailList, null);
//		return user;
//	}
	...
	public void updateTrendResult(GxK10GaSession session) {
		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
			Map<String,Boolean> resultMap; // 开奖结果
			HQUtils hq = new HQUtils(" from GxK10GaTrend gdk10 ");
			List<Object> trendList = gxK10DAO.findObjects(hq); //上期冷热排行榜
			List<GxK10GaTrend> saveList = new ArrayList<GxK10GaTrend>(); // 本期冷热排行榜
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GxK10GaTrend trend = (GxK10GaTrend) trendList.get(i);
			...
			gxK10DAO.updateObjectList(saveList, null);
		}
	}
	
//	@Override
//	public void updateGaTrend() {
//		GxK10GaSession gxK10GaSession = gxK10DAO.getCurrentSession(); // 当前期数信息
//		if(gxK10GaSession != null){
//			String curNo = gxK10GaSession.getSessionNo(); // 当前期号
...
//			List<GxK10GaTrend> saveList = new ArrayList<GxK10GaTrend>(); // 本期冷热排行榜
//
//			GxK10GaSession gxK10PreSession = getPreviousSessionBySessionNo(preNo);
//			if(gxK10PreSession==null){
//				gxK10PreSession=(GxK10GaSession) gxK10DAO.getObject(GxK10GaSession.class, gxK10GaSession.getSessionId()-1);
//			}
//			String results = gxK10PreSession.getOpenResult();
//			
//			if(results != null && results.length() > 0){
//				resultMap = this.transResult(results);
//			}else{
...
//	}
	...
	public boolean saveOpenResult(GxK10GaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			session.setOpenResult(openResult);
			gxK10DAO.updateObject(session, null);
			flag=true;
		}
		return flag;
	}
	...
	public boolean saveAndOpenResult(GxK10GaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			buffer=buffer.substring(0, buffer.length()-1);
			session.setOpenResult(openResult);		
			boolean flag1 = openGxK10SessionOpenResultMethod(session, session.getOpenResult());
			if(flag1){
				session.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
				session.setOpenStatus(GameConstants.OPEN_STATUS_OPENED);
				gxK10DAO.updateObject(session, null);
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gxK10DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GxK10DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gxK10DAO.findGaBetDetailById(hql,para);
	}
	...
	public String updateInitSession(int days) {
		...
		if(!isTodaySessionInit){
			List<GxK10GaSession> saveList = new ArrayList<GxK10GaSession>();
			String theDay = DateTimeUtil.DateToString(time);//初始化期号日期的yyyy-MM-dd格式
			String startDateStr = theDay + GxK10Constants.GX_K10_START_TIME_STR;//开始时间串
			Date startDate = DateTimeUtil.strToDateMul(startDateStr);//开始时间
			
			String tempNo= null;//期号的前已部分。
			String sessionNo = null;//期号
			Date startTime = null;//开始时间
			Date endTime = null;//结束时间
			if("01/01".equals(DateTimeUtil.getMMDD_str(time))){//如果是每年的1月1号
				tempNo = DateTimeUtil.dateToString(time, "yyyy")+"000";
			}else{
				GxK10GaSession beforeSession = gxK10DAO.getPreSession(startDate);//指定日期前一天最后一期
				if(beforeSession != null && beforeSession.getSessionNo() != null){
			        tempNo = beforeSession.getSessionNo().substring(0,7);
				}
			}
			if(!ParamUtils.chkString(tempNo)) tempNo = getTheDayInitSessionNo();
			System.out.println("_______________"+tempNo);
			
			if(ParamUtils.chkString(tempNo)){
				for (int i = 0; i < GxK10Constants.GX_K10_MAX_PART; i++) {
//					//计算出当前场次的时间
					startTime = DateTimeUtil.getDateTimeOfSeconds(startDate, i*GxK10Constants.GX_K10_TIME_INTERVAL2);
					endTime = DateTimeUtil.getDateTimeOfSeconds(startDate, (i+1)*GxK10Constants.GX_K10_TIME_INTERVAL2);
					
					sessionNo = this.getTheDaySessionNo(tempNo, i+1);//期号
					GxK10GaSession k10Session = new GxK10GaSession();
					k10Session.setSessionNo(sessionNo);
					k10Session.setStartTime(startTime);
					k10Session.setEndTime(endTime);
					k10Session.setOpenStatus(GxK10Constants.GX_K10_OPEN_STATUS_INIT);
	...
	/**
	 * 生成指定日期期号，
	 * @param tempNo  tempNo为上一期期号0-7的字符串
	 * @param index
	 * @return
	 */
	public String getTheDaySessionNo(String tempNo,int index){	
		return (Integer.parseInt(tempNo)+1)+String.format("%02d", index);
	}
	public String getTheDayInitSessionNo(){	
		String[] defStr = "046/2018-02-22 09:00:00".split("\\/");//这天第一期
		Integer defNo = Integer.valueOf(defStr[0])-1;
		Date defDate = DateTimeUtil.parse(defStr[1]);
		Date now = DateTimeUtil.getJavaUtilDateNow();
		int diffDay = DateTimeUtil.daysBetween(defDate, now);
		
		Integer diffNo = defNo+diffDay;
		return DateTimeUtil.getYyyy(now)+(diffNo<100?"0"+(diffNo<10?"0"+diffNo:diffNo):diffNo);
	}
	/**
	 * 检查指定日期的场次是否已经生成
	 * false=未生成
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GxK10GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gxK10DAO.countObjects(hq);
	...
	// 排列组合生成投注
		...
		public boolean saveRevokePrize(GxK10GaSession session) {
			...
			para.add(session.getSessionId());
			gxK10DAO.deleteGxK10GaBet(hql.toString(),para);

			boolean result = gaService.saveXyRevokePrize(session.getSessionId(), Constants.GAME_TYPE_XY_GXK10,session.getSessionNo());
			if(result){
