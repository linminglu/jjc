...
public class FetchGxK10Result  extends QuartzJobBean {
...
//		if(!Constants.getTimerOpen("gxk10.fetchResult.xy")) return;
//		
//		try {
//			log.info("_______[gxk10 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//
//				gxK10Service.updateFetchAndOpenResult();
//
//			log.info("_______[gxk10 fetch result form offical end]["+DateTimeUtil.getDateTime()+"]___________________");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_GXK10);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_GXK10);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_GXK10)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_GXK10, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_GXK10);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GxK10GaSession curSession = gxK10Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GxK10GaSession openSession = (GxK10GaSession)gxK10Service.getObject(GxK10GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = gxK10Service.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_GXK10,openSessionNo);//移除开奖标识
