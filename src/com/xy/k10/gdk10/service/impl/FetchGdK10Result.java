...
public class FetchGdK10Result  extends QuartzJobBean {
...
//		if(!Constants.getTimerOpen("gdk10.fetchResult.xy")) return;
//		
//		try {
//			log.info("_______[gdk10 fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
////			
//			Date now=new Date();
...
//
//				gdK10Service.updateFetchAndOpenResult();
//
//			}
//
//			log.info("_______[gdk10 fetch result form offical end]["+DateTimeUtil.getDateTime()+"]___________________");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_GDK10);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_GDK10);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_GDK10)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_GDK10, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_GDK10);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		GdK10GaSession curSession = gdK10Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		GdK10GaSession openSession = (GdK10GaSession)gdK10Service.getObject(GdK10GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = gdK10Service.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_GDK10,openSessionNo);//移除开奖标识
