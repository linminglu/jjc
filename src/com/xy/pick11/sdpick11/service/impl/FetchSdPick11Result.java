...
//		if(!Constants.getTimerOpen("sdpick11.fetchResult.xy")) return;
//		
//		try {
...
//					sdPick11Service.updateFetchAndOpenResult();	
//			}
...
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_SDPICK11);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_SDPICK11);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_SDPICK11)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_SDPICK11, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_SDPICK11);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		SdPick11GaSession curSession = sdPick11Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		SdPick11GaSession openSession = (SdPick11GaSession)sdPick11Service.getObject(SdPick11GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = sdPick11Service.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_SDPICK11,openSessionNo);//移除开奖标识
