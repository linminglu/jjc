...
public class SdPick11ServiceImpl extends BaseService implements ISdPick11Service {
	private ISdPick11DAO sdPick11DAO;
	private IUserService userService;
	private IGaService gaService;
	public void setSdPick11DAO(ISdPick11DAO sdPick11dao) {
		sdPick11DAO = sdPick11dao;
		super.dao = sdPick11DAO;
	}
...
//		log.info("___[start]__________________________");
		...
		List<SdPick11GaSession> saveList = null;
		if(!isTodaySessionInit){
//			log.info("___[start today]__________________________");
			saveList = new ArrayList<SdPick11GaSession>();
			String startTimeStr = today + SdPick11Constants.SD_PICK11_START_TIME_STR;//开始时间串
			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
			
//			log.info("___[startTimeStr="+startTimeStr+"]");
			
			for (int i = 0; i < SdPick11Constants.SD_PICK11_MAX_PART; i++) {
				//计算出当前场次的时间
				long diffTime = startDate.getTime() + i*SdPick11Constants.SD_PICK11_TIME_INTERVAL * 60 * 1000;
				...
				SdPick11GaSession k10Session = new SdPick11GaSession();
				k10Session.setSessionNo(sessionNo);
				k10Session.setStartTime(curSessionDate);
				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,SdPick11Constants.SD_PICK11_TIME_INTERVAL));
				k10Session.setOpenStatus(SdPick11Constants.SD_PICK11_OPEN_STATUS_INIT);
				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
//				sdPick11DAO.saveObject(k10Session);
				saveList.add(k10Session);
				//log.info("___[out]__[sessionNo="+sessionNo+"][diffTime="+diffTime+"][startDate.getTime()="+startDate.getTime()+"]");
			}
			sdPick11DAO.updateObjectList(saveList, null);
			flag = "success";
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from SdPick11GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = sdPick11DAO.countObjects(hq);
	...
	public String updateFetchAndOpenResult(Map<String, SessionItem> sessionNoMap) {
		...
		//当前场次及开奖场次处理
		SdPick11GaSession curtSession = sdPick11DAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		SdPick11GaSession lastSession = (SdPick11GaSession)sdPick11DAO.getObject(SdPick11GaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<SdPick11GaSession> openedList = new ArrayList<SdPick11GaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			...
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				SdPick11GaSession session = sdPick11DAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = openSdPick11SessionOpenResultMethod(session, result);
						if(flag){
							...
							sdPick11DAO.saveObject(session);
			...
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_SDPICK11);
			if(sessionInfo!=null){
				...
				sdPick11DAO.saveObject(sessionInfo);
			...
			for(SdPick11GaSession session:openedList){
...
//		SdPick11GaSession currentSession=sdPick11DAO.getCurrentSession();
...
//				SdPick11GaSession session =sdPick11DAO.getPreviousSessionBySessionNo(key);
//				if(session!=null){
...
//						boolean flag = openSdPick11SessionOpenResultMethod(session, session.getOpenResult());
//						if(flag){
...
//							sdPick11DAO.saveObject(session);
//						}else{
//							GameHelpUtil.log(Constants.GAME_TYPE_XY_SDPICK11,"open flag FAIL -_- ["+session.getSessionNo()+"]");
//						}
//					}
//					// 把资金明细里投注记录状态值改为有效
//					userService.updateUserTradeDetailStatus(session.getSessionNo(), 
//							Constants.GAME_TYPE_XY_SDPICK11, Constants.PUB_STATUS_OPEN);
//				}
//			}
//			
//			GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_XY_SDPICK11);
//			if(sessionInfo != null){
//				SessionItem lastItem = (SessionItem)sessionNoMap.get(lastSessionNo);//上一期
...
//		SdPick11GaSession currentSession=sdPick11DAO.getCurrentSession();
//		SdPick11GaSession tempsession =sdPick11DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
//		}else{
//			tempsession=(SdPick11GaSession) sdPick11DAO.getObject(SdPick11GaSession.class, (currentSession.getSessionId()-1));
//		}
...
//            		 
//            			//从这里 ---------------------------------------------------------------------------
//            		    GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("xysdpick11");
//            		    String officalURL ="";
...
//            		    sessionInfo=null;
//
//	               		log.info("___[sdpick11 start fetch result xml data...]________________---------sdpick11______"+officalURL);
////	               		ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new Date())+"___[sdpick11 start fetch result xml data...]________________", Constants.getWebRootPath()+"/gamelogo/sdpick11.txt", true);
//	    	            
...
////	            	    ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new Date())+resultXML, Constants.getWebRootPath()+"/gamelogo/sdpick11.txt", true);
//
...
//								session.setOpenStatus(SdPick11Constants.SD_PICK11_OPEN_STATUS_OPENED);
//								sdPick11DAO.saveObject(session);
//								log.info("___[sdpick11][open result success sessionNO["+session.getSessionNo()+"]]");
//							}else{
//								log.info("___[sdpick11][open result fail sessionNO["+session.getSessionNo()+"], please check...]");
//							}
//						}
//					}
//				}
...
//						}
//					}
//					sdPick11DAO.updateObject(sessionInfo, null);
//					CacheUtil.updateGameList();
//				}			
//				sessionNoMap.clear();
//				timeMap.clear();
...
//	}
	 ...
	 * @return
	 */
	public boolean openSdPick11SessionOpenResultMethod(SdPick11GaSession session,String result){
		String gameCode = Constants.getGameCodeOfGameType(Constants.GAME_TYPE_XY_SDPICK11);
		try{
			...
			//本期投注统计表
			SdPick11GaBet bet=new SdPick11GaBet();
...
//	public boolean openSdPick11SessionOpenResultMethod(SdPick11GaSession session,String result){
//		log.info("___[sdpick11][open stat***************** sessionNO["+session.getSessionNo()+"]]");
//		try{
////			SdPick11GaSession session =bjPk10DAO.getPreviousSessionBySessionNo(sessionNo);
//			List<Object> para = new ArrayList<Object>();
//			StringBuffer hql = new StringBuffer();
...
//			hql.append(" order by ho.betTime desc");
//			List<GaBetDetail> list=gaService.findGaBetDetailList(hql.toString(), para);
//			log.info("[sdPick11开奖记录=]___________________"+list.size());
//			SdPick11GaBet bet=new SdPick11GaBet();
...
//					if(flag.equals("2")){//中奖
//						
//						detail.setWinResult(SdPick11Constants.SD_PICK11_WIN);//中奖
...
//					}else if(flag.equals("1")){//打和
//					
//						detail.setWinResult(SdPick11Constants.SD_PICK11_WIN_HE);//打和
//						BigDecimal wincash=new BigDecimal(detail.getBetMoney());//投注总钱数
//
//						detail.setWinCash(wincash);
////							userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME, Constants.CASH_TYPE_CASH_BET_SDPICK11, wincash, detail.getBetDetailId());				
//
//						remark.append("彩票打和 退款")
//						    .append(wincash).append("元");
////							user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, wincash, detail.getBetDetailId(), Constants.GAME_TYPE_XY_SDPICK11,remark.toString());
//
//						try {
//							updateOpenData(detail,null,remark.toString());
//						} catch (Exception e) {
...
//					}else{//未中奖
//						detail.setWinCash(new BigDecimal(0));
//						detail.setWinResult(SdPick11Constants.SD_PICK11_WIN_NOT);//未中奖
//						totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//						try {
//							sdPick11DAO.updateObject(detail, null);
//						} catch (Exception e) {
//							e.printStackTrace();
//						}
//					}
...
//			try {
//				sdPick11DAO.saveObject(bet, null);
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
//			return true;
...
//	}
	...
	public void updateOpenData(GaBetDetail detail,String remark){
		...
		userService.saveTradeDetail(null,detail.getUserId(), 
				...
				Constants.GAME_TYPE_XY_SDPICK11,
...
//		sdPick11DAO.saveObject(detail);
//		user=userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_SDPICK11,remark);
//	}
	
//	public void updateOpenData(GaBetDetail detail,User user){
//		
//		sdPick11DAO.saveObject(detail);
//		sdPick11DAO.saveObject(user);
////		user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_SDPICK11,remark);
//		
//	}
	
	
	public SdPick11GaSession getCurrentSession(){
		return sdPick11DAO.getCurrentSession();
	}
	public SdPick11GaSession getPreviousSessionBySessionNo(String sessionNo){
		return sdPick11DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	public void saveUserBetInfo(String room,Map<String,Integer> betMap,List<GaBetOption> list,SdPick11GaSession session,User user,BigDecimal betAll){
		//彩种缓存
		GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_SDPICK11);
		//投注与明细关联
		...
		for (Map.Entry<String, Integer> entry : betMap.entrySet()) { 
			...
			betDetail.setGameType(Constants.GAME_TYPE_XY_SDPICK11);
			...
			sdPick11DAO.saveObject(betDetail);
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll, null, 
				Constants.GAME_TYPE_XY_SDPICK11,remark,session.getSessionNo(),user.getUserType(),user.getLoginName());
		...
		sdPick11DAO.updateObjectList(rlList, null);
	}
...
//			betDetail.setBetName(betOption.getTitle());
//			betDetail.setOptionTitle(optionTitle);
//			betDetail.setGameType(Constants.GAME_TYPE_XY_SDPICK11);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
////		
//
////			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_SDPICK11, betAll.subtract(tempMoney), null);
//		StringBuilder remark = new StringBuilder();
//		remark.append("购买彩票 扣款 ")
//		    .append(betAll.subtract(tempMoney)).append("元");
//		
//		user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll.subtract(tempMoney), null, Constants.GAME_TYPE_XY_SDPICK11,remark.toString());
//		
//		//更新用户实时余额  --by.cuisy.20171209
//		userService.updateUserMoney(user.getUserId());
//
...
//		
//		sdPick11DAO.updateObjectList(betDetailList, null);
//	}
	...
	public void updateTrendResult(SdPick11GaSession session) {
		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
			Map<String,Boolean> resultMap; // 开奖结果
			HQUtils hq = new HQUtils(" from SdPick11GaTrend gdk10 ");
			List<Object> trendList = sdPick11DAO.findObjects(hq); //上期冷热排行榜
			List<SdPick11GaTrend> saveList = new ArrayList<SdPick11GaTrend>(); // 本期冷热排行榜
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					SdPick11GaTrend trend = (SdPick11GaTrend) trendList.get(i);
...
//		SdPick11GaSession sdPick11GaSession = sdPick11DAO.getCurrentSession(); // 当前期数信息
//		if(sdPick11GaSession != null){
//			String curNo = sdPick11GaSession.getSessionNo(); // 当前期号
//			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
//			SdPick11GaSession sdPick11PreSession = getPreviousSessionBySessionNo(preNo);
//			if(sdPick11PreSession==null){
//				sdPick11PreSession=(SdPick11GaSession) sdPick11DAO.getObject(SdPick11GaSession.class, sdPick11GaSession.getSessionId()-1);
//			}
//			Map<String,Boolean> resultMap; // 开奖结果
//			
//			HQUtils hq = new HQUtils(" from SdPick11GaTrend gdk10 ");
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean saveOpenResult(SdPick11GaSession session,String openResult){
		...
		boolean flag=false;
		session.setOpenResult(openResult);
		sdPick11DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	...
	public boolean saveAndOpenResult(SdPick11GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		boolean flag1 = openSdPick11SessionOpenResultMethod(session, session.getOpenResult());
		if(flag1){
			session.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
			session.setOpenStatus(SdPick11Constants.SD_PICK11_OPEN_STATUS_OPENED);
			sdPick11DAO.updateObject(session, null);
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return sdPick11DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<SdPick11DTO> findGaBetDetailById(String hql, List<Object> para) {
		return sdPick11DAO.findGaBetDetailById(hql,para);
	}
	...
	public boolean saveRevokePrize(SdPick11GaSession session) {
		...
		para.add(session.getSessionId());
		sdPick11DAO.deleteSdPick11GaBet(hql.toString(),para);

		boolean result = gaService.saveXyRevokePrize(session.getSessionId(), Constants.GAME_TYPE_XY_SDPICK11,session.getSessionNo());
		if(result){
