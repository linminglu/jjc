...
import com.xy.pick11.gdpick11.dao.IGdPick11DAO;
...
public class GdPick11ServiceImpl extends BaseService implements IGdPick11Service {
	private IGdPick11DAO gdPick11DAO;
	private IUserService userService;
	private IGaService gaService;
	public void setGdPick11DAO(IGdPick11DAO gdPick11dao) {
		gdPick11DAO = gdPick11dao;
		super.dao = gdPick11DAO;
	}
...
//		log.info("___[start]__________________________");
		...
		List<GdPick11GaSession> saveList = null;
		if(!isTodaySessionInit){
//			log.info("___[start today]__________________________");
			saveList = new ArrayList<GdPick11GaSession>();
			String startTimeStr = today + GdPick11Constants.GD_PICK11_START_TIME_STR;//开始时间串
			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
			
//			log.info("___[startTimeStr="+startTimeStr+"]");
			
			for (int i = 0; i < GdPick11Constants.GD_PICK11_MAX_PART; i++) {
				//计算出当前场次的时间
				long diffTime = startDate.getTime() + i*GdPick11Constants.GD_PICK11_TIME_INTERVAL * 60 * 1000;
				...
				GdPick11GaSession k10Session = new GdPick11GaSession();
				k10Session.setSessionNo(sessionNo);
				k10Session.setStartTime(curSessionDate);
				k10Session.setEndTime(DateTimeUtil.getDateTimeOfMinutes(curSessionDate,GdPick11Constants.GD_PICK11_TIME_INTERVAL));
				k10Session.setOpenStatus(GdPick11Constants.GD_PICK11_OPEN_STATUS_INIT);
				//k10Session.setOpenResult(openResult);//开奖号由系统抓取获取得
//				gdPick11DAO.saveObject(k10Session);
				saveList.add(k10Session);
				//log.info("___[out]__[sessionNo="+sessionNo+"][diffTime="+diffTime+"][startDate.getTime()="+startDate.getTime()+"]");
			}
			gdPick11DAO.updateObjectList(saveList, null);
			flag = "success";
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from GdPick11GaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = gdPick11DAO.countObjects(hq);
	...
	public String updateFetchAndOpenResult(Map<String, SessionItem> sessionNoMap) {
		...
		//当前场次及开奖场次处理
		GdPick11GaSession curtSession = gdPick11DAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		GdPick11GaSession lastSession = (GdPick11GaSession)gdPick11DAO.getObject(GdPick11GaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<GdPick11GaSession> openedList = new ArrayList<GdPick11GaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			...
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				GdPick11GaSession session = gdPick11DAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = openGdPick11SessionOpenResultMethod(session, result);
						if(flag){
							...
							gdPick11DAO.saveObject(session);
			...
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_GDPICK11);
			if(sessionInfo!=null){
				...
				gdPick11DAO.saveObject(sessionInfo);
			...
			for(GdPick11GaSession session:openedList){
...
//		GdPick11GaSession currentSession=gdPick11DAO.getCurrentSession();
...
//				GdPick11GaSession session =gdPick11DAO.getPreviousSessionBySessionNo(key);
//				if(session!=null){
...
//						boolean flag = openGdPick11SessionOpenResultMethod(session, session.getOpenResult());
//						if(flag){
...
//							gdPick11DAO.saveObject(session);
//						}else{
//							GameHelpUtil.log(Constants.GAME_TYPE_XY_GDPICK11,"open flag FAIL -_- ["+session.getSessionNo()+"]");
//						}
//					}
//					// 把资金明细里投注记录状态值改为有效
//					userService.updateUserTradeDetailStatus(session.getSessionNo(), 
//							Constants.GAME_TYPE_XY_GDPICK11, Constants.PUB_STATUS_OPEN);
//				}
//			}
//			
//			GaSessionInfo sessionInfo = gaService.findGaSessionInfo(Constants.GAME_TYPE_XY_GDPICK11);
//			if(sessionInfo != null){
//				SessionItem lastItem = (SessionItem)sessionNoMap.get(lastSessionNo);//上一期
...
//		GdPick11GaSession currentSession=gdPick11DAO.getCurrentSession();
//		GdPick11GaSession tempsession =gdPick11DAO.getPreviousSessionBySessionNo((Integer.parseInt(currentSession.getSessionNo())-1)+"");	
//		if(tempsession!=null){
//		}else{
//			tempsession=(GdPick11GaSession) gdPick11DAO.getObject(GdPick11GaSession.class, (currentSession.getSessionId()-1));
//		}
...
//            		 
//            			//从这里 ---------------------------------------------------------------------------
//            		    GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("xygdpick11");
//            		    String officalURL ="";
...
//            		    sessionInfo=null;
//
//	               		log.info("___[gdpick11 start fetch result xml data...]________________---------gdpick11______"+officalURL);
////	               		ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new Date())+"___[gdpick11 start fetch result xml data...]________________", Constants.getWebRootPath()+"/gamelogo/gdpick11.txt", true);
//	    	            
...
////	            	    ManageFile.writeTextToFile(DateTimeUtil.DateToStringAll(new Date())+resultXML, Constants.getWebRootPath()+"/gamelogo/gdpick11.txt", true);
//
...
//								session.setOpenStatus(GdPick11Constants.GD_PICK11_OPEN_STATUS_OPENED);
//								gdPick11DAO.saveObject(session);
//								log.info("___[gdpick11][open result success sessionNO["+session.getSessionNo()+"]]");
//							}else{
//								log.info("___[gdpick11][open result fail sessionNO["+session.getSessionNo()+"], please check...]");
//							}
//						}
//					}
//				}
...
//						}
//					}
//					gdPick11DAO.updateObject(sessionInfo, null);
//					CacheUtil.updateGameList();
//				}			
//				sessionNoMap.clear();
//				timeMap.clear();
...
//	}
	 ...
	 * @return
	 */
	public boolean openGdPick11SessionOpenResultMethod(GdPick11GaSession session,String result){
		String gameCode = Constants.getGameCodeOfGameType(Constants.GAME_TYPE_XY_GDPICK11);
		try{
			...
			//本期投注统计表
			GdPick11GaBet bet=new GdPick11GaBet();
...
//	public boolean openGdPick11SessionOpenResultMethod(GdPick11GaSession session,String result){
//		log.info("___[gdpick11][open stat***************** sessionNO["+session.getSessionNo()+"]]");
//		try{
////			GdPick11GaSession session =bjPk10DAO.getPreviousSessionBySessionNo(sessionNo);
//			List<Object> para = new ArrayList<Object>();
//			StringBuffer hql = new StringBuffer();
...
//			hql.append(" order by ho.betTime desc");
//			List<GaBetDetail> list=gaService.findGaBetDetailList(hql.toString(), para);
//			log.info("[gdPick11开奖记录=]___________________"+list.size());
//			GdPick11GaBet bet=new GdPick11GaBet();
...
//					if(flag.equals("2")){//中奖
//						
//						detail.setWinResult(GdPick11Constants.GD_PICK11_WIN);//中奖
...
//					}else if(flag.equals("1")){//打和
//					
//						detail.setWinResult(GdPick11Constants.GD_PICK11_WIN_HE);//打和
...
////							user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, wincash, detail.getBetDetailId(), Constants.GAME_TYPE_XY_GDPICK11,remark.toString());
//
//						try {
//							updateOpenData(detail,null,remark.toString());
//						} catch (Exception e) {
...
//					}else{//未中奖
//						detail.setWinCash(new BigDecimal(0));
//						detail.setWinResult(GdPick11Constants.GD_PICK11_WIN_NOT);//未中奖
//						totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//						try {
//							gdPick11DAO.updateObject(detail, null);
//						} catch (Exception e) {
//							e.printStackTrace();
//						}
//					}
...
//			try {
//				gdPick11DAO.saveObject(bet, null);
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
//			return true;
...
//	}
	...
	public void updateOpenData(GaBetDetail detail,String remark){
		...
		userService.saveTradeDetail(null,detail.getUserId(), 
				...
				Constants.GAME_TYPE_XY_GDPICK11,
...
//		gdPick11DAO.saveObject(detail);
//		user=userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_GDPICK11,remark);
//	}
	
//	public void updateOpenData(GaBetDetail detail,User user){
//		
//		gdPick11DAO.saveObject(detail);
//		gdPick11DAO.saveObject(user);
////		user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_GDPICK11,remark);
//		
//	}
	
	
	public GdPick11GaSession getCurrentSession(){
		return gdPick11DAO.getCurrentSession();
	}
	public GdPick11GaSession getPreviousSessionBySessionNo(String sessionNo){
		return gdPick11DAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	public void saveUserBetInfo(String room,Map<String,Integer> betMap,List<GaBetOption> list,GdPick11GaSession session,User user,BigDecimal betAll){
		//彩种缓存
		GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_GDPICK11);
		//投注与明细关联
		...
		for (Map.Entry<String, Integer> entry : betMap.entrySet()) { 
			...
			betDetail.setGameType(Constants.GAME_TYPE_XY_GDPICK11);
			...
			gdPick11DAO.saveObject(betDetail);
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll, null, 
				Constants.GAME_TYPE_XY_GDPICK11,remark,session.getSessionNo(),user.getUserType(),user.getLoginName());
		...
		gdPick11DAO.updateObjectList(rlList, null);
	}
...
//			betDetail.setBetName(betOption.getTitle());
//			betDetail.setOptionTitle(optionTitle);
//			betDetail.setGameType(Constants.GAME_TYPE_XY_GDPICK11);//游戏类型 0=三份彩  1=北京赛车  2=幸运28  3=重庆时时彩  4=PC蛋蛋  5=广东快乐10分
...
////		
//
////			userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_GDPICK11, betAll.subtract(tempMoney), null);
//		StringBuilder remark = new StringBuilder();
//		remark.append("购买彩票 扣款 ")
//		    .append(betAll.subtract(tempMoney)).append("元");
//		
//		user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll.subtract(tempMoney), null, Constants.GAME_TYPE_XY_GDPICK11,remark.toString());
//		
//		//更新用户实时余额  --by.cuisy.20171209
//		userService.updateUserMoney(user.getUserId());
//
...
//		
//		gdPick11DAO.updateObjectList(betDetailList, null);
//	}
	...
	public void updateTrendResult(GdPick11GaSession session) {
		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
			Map<String,Boolean> resultMap; // 开奖结果
			HQUtils hq = new HQUtils(" from GdPick11GaTrend gdk10 ");
			List<Object> trendList = gdPick11DAO.findObjects(hq); //上期冷热排行榜
			List<GdPick11GaTrend> saveList = new ArrayList<GdPick11GaTrend>(); // 本期冷热排行榜
			...
			if(trendList != null && trendList.size() > 0 ){
				for(int i = 0; i < trendList.size(); i ++){
					GdPick11GaTrend trend = (GdPick11GaTrend) trendList.get(i);
...
//		GdPick11GaSession gdPick11GaSession = gdPick11DAO.getCurrentSession(); // 当前期数信息
//		if(gdPick11GaSession != null){
//			String curNo = gdPick11GaSession.getSessionNo(); // 当前期号
//			String preNo = String.valueOf((Integer.parseInt(curNo) - 1)); //上期期号
//			GdPick11GaSession gdPick11PreSession = getPreviousSessionBySessionNo(preNo);
//			if(gdPick11PreSession==null){
//				gdPick11PreSession=(GdPick11GaSession) gdPick11DAO.getObject(GdPick11GaSession.class, gdPick11GaSession.getSessionId()-1);
//			}
//			Map<String,Boolean> resultMap; // 开奖结果
//			
//			HQUtils hq = new HQUtils(" from GdPick11GaTrend gdk10 ");
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean saveOpenResult(GdPick11GaSession session,String openResult){
		...
		boolean flag=false;
		session.setOpenResult(openResult);
		gdPick11DAO.updateObject(session, null);
		flag=true;
		return flag;
	}
	...
	public boolean saveAndOpenResult(GdPick11GaSession session,String openResult){
		boolean flag=false;
		session.setOpenResult(openResult);		
		boolean flag1 = openGdPick11SessionOpenResultMethod(session, session.getOpenResult());
		if(flag1){
			session.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
			session.setOpenStatus(GdPick11Constants.GD_PICK11_OPEN_STATUS_OPENED);
			gdPick11DAO.updateObject(session, null);
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return gdPick11DAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<GdPick11DTO> findGaBetDetailById(String hql, List<Object> para) {
		return gdPick11DAO.findGaBetDetailById(hql,para);
	}
	...
	public boolean saveRevokePrize(GdPick11GaSession session) {
		...
		para.add(session.getSessionId());
		gdPick11DAO.deleteGdPick11GaBet(hql.toString(),para);

		boolean result = gaService.saveXyRevokePrize(session.getSessionId(), Constants.GAME_TYPE_XY_GDPICK11,session.getSessionNo());
		if(result){
