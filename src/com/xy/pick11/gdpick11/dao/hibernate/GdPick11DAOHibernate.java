package com.xy.pick11.gdpick11.dao.hibernate;
...
public class GdPick11DAOHibernate extends AbstractBaseDAOHibernate implements IGdPick11DAO {
	...
	public GdPick11GaSession getCurrentSession() {
		...
		List<GdPick11GaSession> list = getSession().createQuery("from GdPick11GaSession gks where gks.startTime<=? and gks.endTime>? and (gks.openStatus=? or gks.openStatus=?) order by gks.sessionId")
		.setTimestamp(0, now)
		.setTimestamp(1, now)
		...
		if(list!=null && list.size()>0){
			return (GdPick11GaSession)list.get(0);
		}
		return null;
	}
	public GdPick11GaSession getPreviousSessionBySessionNo(String sessionNo){
		String  hql=" from GdPick11GaSession gks where gks.sessionNo=? ";
	...
	public List<GdPick11GaTrend> findGdPick11GaTrendList(){
		String  hql=" from GdPick11GaTrend gks where gks.trendCount >1 order by gks.trendCount desc";
		Query record =getSession().createQuery(hql);
		List list=record.list();
		return list;
	}
	public PaginationSupport  findGdPick11GaSessionList(String hql, List<Object> para,int pageNum,int pageSize){
		Query query = makeQuerySQL( " from GdPick11GaSession ho where 1=1 "+hql,para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from GdPick11GaSession ho where 1=1 "+hql, para);
	...
	public PaginationSupport  findGdPick11GaBetList(String hql, List<Object> para,int pageNum,int pageSize){
		Query query = makeQuerySQL( " select new com.xy.pick11.gdpick11.model.dto.GdPick11DTO(ho.sessionNo,ho.totalPoint,ho.winCash,ho.totalPoint-ho.winCash) from GdPick11GaBet ho where 1=1 "
				+hql, para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.betId) from GdPick11GaBet ho where 1=1" +
				hql, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		Query query = makeQuerySQL( " select new com.xy.pick11.gdpick11.model.dto.GdPick11DTO(ga,u) from GaBetDetail ga,User u where 1=1 "
	...
	public List<GdPick11DTO> findGaBetDetailById(String hql, List<Object> para) {
	    Query query = getSession().createQuery(" select new com.xy.pick11.gdpick11.model.dto.GdPick11DTO(ga,u) from GaBetDetail ga,User u where 1=1 and ga.userId = u.userId "
	    		+ hql)
	    		.setParameter(0, para.get(0));
	    List<GdPick11DTO> list = query.list();
		return list;
	}
	...
	public void deleteGdPick11GaBet(String hql, List<Object> para) {
		String delHql=" delete from GdPick11GaBet where 1=1 ";
