...
//		if(!Constants.getTimerOpen("jxpick11.fetchResult.xy")) return;
//		
//		try {
...
//					jxPick11Service.updateFetchAndOpenResult();	
//			}
...
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_JXPICK11);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_JXPICK11);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_JXPICK11)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_JXPICK11, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_JXPICK11);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		JxPick11GaSession curSession = jxPick11Service.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		JxPick11GaSession openSession = (JxPick11GaSession)jxPick11Service.getObject(JxPick11GaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = jxPick11Service.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_JXPICK11,openSessionNo);//移除开奖标识
