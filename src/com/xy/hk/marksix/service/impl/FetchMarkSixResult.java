 ...
 * 定时抓去官网开奖数据
 */
public class FetchMarkSixResult extends QuartzJobBean {
...
//		if(!Constants.getTimerOpen("marksix.fetchResult.xy")) return;
//		
//		try {
//			log.info("_______[MarkSix fetch session]["+DateTimeUtil.getDateTime()+"]___________________");
//			
//			Date now=new Date();
//			String endTime=DateTimeUtil.DateToString(now);
//			Date date=DateTimeUtil.stringToDate(endTime+" 23:59:59", "yyyy-MM-dd HH:mm:ss");			
//			if(now.getTime()<date.getTime()){
//					markSixService.updateFetchAndOpenResult();
//			}	
//			
//			log.info("_______[MarkSix fetch session end]["+DateTimeUtil.getDateTime()+"]___________________");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_MARKSIX);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_MARKSIX);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_MARKSIX)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_MARKSIX, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_MARKSIX);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		MarkSixGaSession curSession = markSixService.getCurrentSession(now);
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		MarkSixGaSession openSession = (MarkSixGaSession)markSixService.getObject(MarkSixGaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = markSixService.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_MARKSIX,openSessionNo);//移除开奖标识
