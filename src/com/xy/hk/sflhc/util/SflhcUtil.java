 ...
 * 
 */
public class SflhcUtil {
	/**
	 * 从0到49获取随机不重复7个数
	 * @return
	 */
	public static String getRandomResult(){
		String[] ball = {"01","02","03","04","05","06","07","08","09",
		...
		for(int i=0; i<6; i++){
			...
			randomResult = randomResult.append(ball[index]).append(",");
			ball[index] = ball[len];
		}
		randomResult.deleteCharAt(randomResult.length()-1).append("+").append(ball[Math.abs(rand.nextInt() % 43)]);
	...
	public static Map<String, Boolean> getOneZodiac(Date startTime ,String result) {
	    ...
	    int year = lunar.year;
	    Map<String,String> lumap = CacheUtil.getSflhcMap(String.valueOf(year), startTime);
		shu = lumap.get("shu");
		zhu = lumap.get("zhu");
		gou = lumap.get("gou");
		ji = lumap.get("ji");
...
//		int codeValue = Integer.parseInt(code);
	...
	public static void main(String args[]) {
		for(int i=0; i<50; i++){
			System.out.println(getRandomSecond()+"-----");
		}
