...
import com.xy.ssc.cqssc.model.CqSscGaBet;
...
public class CqSscServiceImpl extends BaseService implements ICqSscService {
	...
	public void setCqSscDAO(ICqSscDAO cqSscdao) {
		cqSscDAO = cqSscdao;
		super.dao = cqSscDAO;
	}
...
//			log.info("___[start today]__________________________");
			...
			for (int i = 0; i < CqSscConstants.CQ_SSC_MAX_PART_NIGHT; i++) {//经观察得知
				...
				CqSscGaSession k10Session = new CqSscGaSession();
...
//				cqSscDAO.saveObject(k10Session);
...
//				cqSscDAO.saveObject(k10Session);
			...
			cqSscDAO.updateObjectList(saveList, null);
			flag = "success";
...
//			log.info("___[today has been inited]__________________________");
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from CqSscGaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = cqSscDAO.countObjects(hq);
...
//			log.info("___[today has been inited]__________________________");
	...
	public String updateFetchAndOpenResult(Map<String, SessionItem> sessionNoMap) {
		...
		//当前场次及开奖场次处理
		CqSscGaSession curtSession = cqSscDAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		CqSscGaSession lastSession = (CqSscGaSession)cqSscDAO.getObject(CqSscGaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<CqSscGaSession> openedList = new ArrayList<CqSscGaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			...
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				CqSscGaSession session = cqSscDAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = openCqSscSessionOpenResultMethod(session, result);
						if(flag){
			...
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_CQSSC);
			if(sessionInfo!=null){
				...
				cqSscDAO.saveObject(sessionInfo);
			...
			for(CqSscGaSession session:openedList){
...
//		CqSscGaSession crrrentsession =cqSscDAO.getCurrentSession();
//		CqSscGaSession tempsession =cqSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(crrrentsession.getSessionNo())-1)+"");	
//		String lastSessionNo=tempsession.getSessionNo();
//		
//			if(!sessionNoMap.isEmpty()){
//				for(String key:sessionNoMap.keySet()){
//					CqSscGaSession session =cqSscDAO.getPreviousSessionBySessionNo(key);
//					if(session!=null){
...
//							
//							boolean flag = openCqSscSessionOpenResultMethod(session, session.getOpenResult());
//							
//							if(flag){
...
//								cqSscDAO.saveObject(session);
//							}else{
//								GameHelpUtil.log(Constants.GAME_TYPE_XY_CQSSC,"open flag FAIL -_- ["+session.getSessionNo()+"]");
//							}
//						}
//						// 把资金明细里投注记录状态值改为有效
//						userService.updateUserTradeDetailStatus(session.getSessionNo(), 
//								Constants.GAME_TYPE_XY_CQSSC, Constants.PUB_STATUS_OPEN);
//					}
//				}
//				
//				GaSessionInfo sessionInfo=gaService.findGaSessionInfo(Constants.GAME_TYPE_XY_CQSSC);
//				if(sessionInfo!=null){
//					SessionItem lastItem = (SessionItem)sessionNoMap.get(lastSessionNo);//上一期
...
//				}
//				cqSscDAO.updateObject(sessionInfo, null);
//				CacheUtil.updateGameList();
//			}
//	}
...
//		if(tempsession!=null){
//		}else{
//			tempsession=(CqSscGaSession) cqSscDAO.getObject(CqSscGaSession.class, (crrrentsession.getSessionId()-1));
//		}
//		String  lastSessionNo=tempsession.getSessionNo();
...
//
//       		//从这里 ---------------------------------------------------------------------------
//		    GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("xycqssc");
...
//               		  		log.info("___[cqssc start fetch result xml data...]_______________dddddddddddd_");
//   							break out;
//   						}
...
////					sessionInfo.setGameType(Constants.GAME_TYPE_XY_CQSSC);
////					sessionInfo.setLatestSessionNo(crrrentsession.getSessionNo());
////					if(sessionNoMap.get(lastSessionNo)!=null){
////						sessionInfo.setOpenResult(sessionNoMap.get(lastSessionNo));
////						sessionInfo.setOpenSessionNo(lastSessionNo);
...
//	}
	...
	public void updateTrendResult(CqSscGaSession session){
		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
			List<CqSscGaTrend> list=cqSscDAO.findCqSscGaTrendAllList();
			List<CqSscGaTrend> savelist=new ArrayList<CqSscGaTrend>();
			Map<String,Boolean> map=getTrendResult(session.getOpenResult());
			if(!map.isEmpty()){
				for(CqSscGaTrend trend:list){
...
//		}
//		
//		CqSscGaSession session =cqSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(cqSscDAO.getCurrentSession().getSessionNo())-1)+"");	
//		if(session!=null){
//		}else{
//			session=(CqSscGaSession) cqSscDAO.getObject(CqSscGaSession.class, (cqSscDAO.getCurrentSession().getSessionId()-1));
//		}
//		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
//			List<CqSscGaTrend> list=cqSscDAO.findCqSscGaTrendAllList();
//			List<CqSscGaTrend> savelist=new ArrayList<CqSscGaTrend>();
...
//	}
	 ...
	 * @return
	 */
	public boolean openCqSscSessionOpenResultMethod(CqSscGaSession session,String result){
		String gameCode = Constants.getGameCodeOfGameType(Constants.GAME_TYPE_XY_CQSSC);
		try{
			List<Object> para = new ArrayList<Object>();
			StringBuffer hql = new StringBuffer();
			hql.append(" and ho.gameType=?");
			para.add(Constants.GAME_TYPE_XY_CQSSC);
...
//	public boolean openCqSscSessionOpenResultMethod(CqSscGaSession session,String result){
//		try{
////			CqSscGaSession session =cqSscDAO.getPreviousSessionBySessionNo(sessionNo);
//			List<Object> para = new ArrayList<Object>();
//			StringBuffer hql = new StringBuffer();
...
//						detail.setOpenResult(result);
//						if(wincash.compareTo(new BigDecimal(0)) == 1){//中奖
//							detail.setWinResult(CqSscConstants.CQ_SSC_WIN);//中奖
...
////							userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME, Constants.CASH_TYPE_CASH_BET_CQSSC, wincash, detail.getBetDetailId());			
...
//							}
//						}else if(wincash.compareTo(new BigDecimal(0)) == 0){//打和
//							detail.setWinResult(CqSscConstants.CQ_SSC_WIN_HE);//打和
...
//							totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//							detail.setWinCash(new BigDecimal(0));
//							detail.setWinResult(CqSscConstants.CQ_SSC_WIN_NOT);//未中奖
//							try {
//								cqSscDAO.updateObject(detail, null);
//							} catch (Exception e) {
//								e.printStackTrace();
//							}
//						}
...
//		cqSscDAO.saveObject(detail);
//		user=userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_CQSSC,remark);
//	}

	public CqSscGaSession getCurrentSession(){
		return cqSscDAO.getCurrentSession();
	}
	public CqSscGaSession getPreviousSessionBySessionNo(String sessionNo){
		return cqSscDAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	public User saveUserBetInfo(String room,Map<String,Integer> betMap,List<GaBetOption> list,CqSscGaSession session,User user,BigDecimal betAll){
		//彩种缓存
		GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_CQSSC);
		//投注与明细关联
		...
		for(Map.Entry<String, Integer> entry:betMap.entrySet()){
			...
			betDetail.setGameType(Constants.GAME_TYPE_XY_CQSSC);
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll, null, 
				Constants.GAME_TYPE_XY_CQSSC,remark,session.getSessionNo(),user.getUserType(),user.getLoginName());
		...
		cqSscDAO.updateObjectList(rlList, null);
...
//			cqSscDAO.updateObject(user, null);
//		}else{
////			user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_CQSSC, betAll.subtract(tempMoney), null);
//			StringBuilder remark = new StringBuilder();
//			remark.append("购买彩票 扣款 ")
//			    .append(betAll.subtract(tempMoney)).append("元");
//			user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll.subtract(tempMoney), null, Constants.GAME_TYPE_XY_CQSSC,remark.toString());
//
//			//更新用户实时余额  --by.cuisy.20171209
//			userService.updateUserMoney(user.getUserId());
//			userService.updateUserBanlance(user.getUserId());
//		}
////		userService.saveTradeDetail(user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_CQSSC, betAll, null);
//		cqSscDAO.updateObjectList(betDetailList, null);
//		return  user;
//	}
	...
	public boolean saveOpenResult(CqSscGaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			session.setOpenResult(openResult);
			cqSscDAO.updateObject(session, null);
			flag=true;
		}
		return flag;
	...
	public boolean saveAndOpenResult(CqSscGaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			buffer=buffer.substring(0, buffer.length()-1);
			session.setOpenResult(openResult);		
			boolean flag1 = openCqSscSessionOpenResultMethod(session, session.getOpenResult());
			if(flag1){
				session.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
				session.setOpenStatus(GameConstants.OPEN_STATUS_OPENED);
				cqSscDAO.updateObject(session, null);
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return cqSscDAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<CqSscDTO> findGaBetDetailById(String hql, List<Object> para) {
		return cqSscDAO.findGaBetDetailById(hql, para);
	}
	@Override
	public boolean saveRevokePrize(CqSscGaSession session) {
		...
		para.add(session.getSessionId());
		cqSscDAO.deleteCqSscGaBet(hql.toString(),para);

		boolean result = gaService.saveXyRevokePrize(session.getSessionId(), Constants.GAME_TYPE_XY_CQSSC,session.getSessionNo());
		if(result){
