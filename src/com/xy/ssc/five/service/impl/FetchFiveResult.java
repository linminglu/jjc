...
//		if(!Constants.getTimerOpen("five.fetchResult.xy")) return;
//		
//    	try {
...
//    					session.setEstimateResult(FiveUtil.getRandomResult(5));
//    					fiveService.saveObject(session, null);
//    				}
//    			}
//    		}
//    		fiveService.updateFetchAndOpenResult();	
//
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_FC);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_FC);//抓取间隔毫秒
	int delayOpenTime = GameHelpUtil.getDelayOpenTime(Constants.GAME_TYPE_XY_FC);//延迟开奖秒数(系统开)
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_FC)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_FC, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_FC);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		FiveGaSession curSession = fiveService.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		FiveGaSession openSession = (FiveGaSession)fiveService.getObject(FiveGaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
...
//	        		}
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = fiveService.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_FC,openSessionNo);//移除开奖标识
