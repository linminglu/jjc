...
public class BjSscServiceImpl   extends BaseService implements IBjSscService {
	private IBjSscDAO bjSscDAO;
	private IUserService userService;
	private IGaService gaService;
	public void setBjSscDAO(IBjSscDAO bjSscdao) {
		bjSscDAO = bjSscdao;
		super.dao = bjSscDAO;
	}
	...
	public String updateInitSession(int days) {
		...
		if(!isSessionInit){
			List<BjSscGaSession> saveList = new ArrayList<BjSscGaSession>();
			...
			for (int i = 0; i < BjSscConstants.BJ_SSC_MAX_PART_NIGHT; i++) {
				...
				BjSscGaSession session = new BjSscGaSession();
			...
			bjSscDAO.updateObjectList(saveList, null);
			flag = "success";
		} else {
			flag = "inited";
		}
	...
	public boolean checkSessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from BjSscGaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = bjSscDAO.countObjects(hq);
	...
	public String updateFetchAndOpenResult(Map<String, SessionItem> sessionNoMap) {
		...
		//当前场次及开奖场次处理
		BjSscGaSession curtSession = bjSscDAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		BjSscGaSession lastSession = (BjSscGaSession)bjSscDAO.getObject(BjSscGaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<BjSscGaSession> openedList = new ArrayList<BjSscGaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			...
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				BjSscGaSession session = bjSscDAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = openBjSscSessionOpenResultMethod(session, result);
						if(flag){
			...
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_BJSSC);
			if(sessionInfo!=null){
				...
				bjSscDAO.saveObject(sessionInfo);
			...
			for(BjSscGaSession session:openedList){
	...
	public void updateFetchAndOpenResult_bak(Map<String, SessionItem> sessionNoMap) {
		BjSscGaSession crrrentsession =bjSscDAO.getCurrentSession();
		BjSscGaSession tempsession =bjSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(crrrentsession.getSessionNo())-1)+"");	
		if(tempsession==null){
			tempsession=(BjSscGaSession) bjSscDAO.getObject(BjSscGaSession.class, (crrrentsession.getSessionId()-1));
		}
		if(tempsession != null){
			// 把资金明细里投注记录状态值改为有效
			userService.updateUserTradeDetailStatus(tempsession.getSessionNo(), 
					Constants.GAME_TYPE_XY_BJSSC, Constants.PUB_STATUS_OPEN);
				...
				try {
					if(!ParamUtils.chkString(tempsession.getOpenResult())){	
						...
						if(ParamUtils.chkString(value)){
							...
							result = gaService.getMinResult(tempsession.getSessionNo(),Constants.GAME_TYPE_XY_BJSSC,tempsession.getEstimateResult(),openNum);
						}
						if(!ParamUtils.chkString(result)){
							//result = BjSscUtil.getRandomResult(1);
							SessionItem sessionItem = sessionNoMap.get(tempsession.getSessionNo());
							result = sessionItem.getResult();
						}
						
						GameHelpUtil.log(Constants.getGameCodeOfGameType(Constants.GAME_TYPE_XY_BJSSC),"results  _____ [rnd="+value+"]["+tempsession.getSessionNo()+"]=>"+result);
						
						tempsession.setOpenResult(result);
					}
					
					boolean flag = openBjSscSessionOpenResultMethod(tempsession, tempsession.getOpenResult());
					if(flag){
						tempsession.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
						tempsession.setOpenStatus(GameConstants.OPEN_STATUS_OPENED);
						bjSscDAO.saveObject(tempsession);
					}else{
						GameHelpUtil.log(Constants.GAME_TYPE_XY_BJSSC,"open flag FAIL -_- ["+tempsession.getSessionNo()+"]");
					}
					
					GaSessionInfo sessionInfo=gaService.findGaSessionInfo(Constants.GAME_TYPE_XY_BJSSC);
					if(sessionInfo!=null){
					...
					bjSscDAO.updateObject(sessionInfo, null);
				} catch(Exception e){
					e.printStackTrace();
				}
			}
...
//				try {
//					//为了开奖时间更像是从接口获取
//					Thread.sleep(BjSscUtil.getRandomSecond()*1000);
//					if(!ParamUtils.chkString(tempsession.getOpenResult())){	
//						String result = "";
//						Param param=CacheUtil.getParam(Constants.PARAM_BJSSC_OPEN_NUM);
//						String value=param.getValue();
...
//	}
	...
	public void updateTrendResult(BjSscGaSession session){
		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
			List<BjSscGaTrend> list=bjSscDAO.findBjSscGaTrendAllList();
			List<BjSscGaTrend> savelist=new ArrayList<BjSscGaTrend>();
			Map<String,Boolean> map=getTrendResult(session.getOpenResult());
			if(!map.isEmpty()){
				for(BjSscGaTrend trend:list){
...
//		}
//		
//		BjSscGaSession session =bjSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(bjSscDAO.getCurrentSession().getSessionNo())-1)+"");	
//		if(session!=null){
//		}else{
//			session=(BjSscGaSession) bjSscDAO.getObject(BjSscGaSession.class, (bjSscDAO.getCurrentSession().getSessionId()-1));
//		}
//		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
//			List<BjSscGaTrend> list=bjSscDAO.findBjSscGaTrendAllList();
//			List<BjSscGaTrend> savelist=new ArrayList<BjSscGaTrend>();
...
//	}
	 ...
	 * @return
	 */
	public boolean openBjSscSessionOpenResultMethod(BjSscGaSession session,String result){
		String gameCode = Constants.getGameCodeOfGameType(Constants.GAME_TYPE_XY_BJSSC);
		try{
...
//	public boolean openBjSscSessionOpenResultMethod_bak(BjSscGaSession session,String result){
//		try{
////			BjSscGaSession session =bjSscDAO.getPreviousSessionBySessionNo(sessionNo);
//			List<Object> para = new ArrayList<Object>();
//			StringBuffer hql = new StringBuffer();
//			hql.append(" and ho.gameType=? ");// 0=三份彩  1=北京赛车  2=幸运28  3=北京时时彩  4=PC蛋蛋  5=广东快乐10分
//			para.add(Constants.GAME_TYPE_XY_BJSSC);
//			hql.append(" and ho.sessionId=? ");
//			para.add(session.getSessionId());
...
//						if(wincash.compareTo(new BigDecimal(0)) == 1){//中奖
//							
//							detail.setWinResult(BjSscConstants.BJ_SSC_WIN);//中奖
...
////							userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME, Constants.CASH_TYPE_CASH_BET_BJSSC, wincash, detail.getBetDetailId());			
...
//							}
//						}else if(wincash.compareTo(new BigDecimal(0)) == 0){//打和
//							detail.setWinResult(BjSscConstants.BJ_SSC_WIN_HE);//打和
...
//							totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//							detail.setWinCash(new BigDecimal(0));
//							detail.setWinResult(BjSscConstants.BJ_SSC_WIN_NOT);//未中奖
//							try {
//								bjSscDAO.updateObject(detail, null);
//							} catch (Exception e) {
//								e.printStackTrace();
//							}
//						}
...
//	}
	...
	public BjSscGaSession getCurrentSession(){
		return bjSscDAO.getCurrentSession();
	}
	public BjSscGaSession getPreviousSessionBySessionNo(String sessionNo){
		return bjSscDAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	public User saveUserBetInfo(String room,Map<String,Integer> betMap,List<GaBetOption> list,BjSscGaSession session,User user,BigDecimal betAll){
		//彩种缓存
		GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_BJSSC);
		//投注与明细关联
		...
		for(Map.Entry<String, Integer> entry:betMap.entrySet()){
			...
			betDetail.setGameType(Constants.GAME_TYPE_XY_BJSSC);
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll, null, 
				Constants.GAME_TYPE_XY_BJSSC,remark,session.getSessionNo(),user.getUserType(),user.getLoginName());
		...
		bjSscDAO.updateObjectList(rlList, null);
...
//
//			UserTradeDetailRl  rl=new UserTradeDetailRl();
//			betDetail=(GaBetDetail)bjSscDAO.saveObjectDB(betDetail);
//			rl.setBetDetailId(betDetail.getBetDetailId());
//			rlList.add(rl);
//		}
//		
...
//			bjSscDAO.updateObject(user, null);
//		}else{
////			user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_BJSSC, betAll.subtract(tempMoney), null);
//			StringBuilder remark = new StringBuilder();
//			remark.append("购买彩票 扣款 ")
//			    .append(betAll.subtract(tempMoney)).append("元");
////			user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll.subtract(tempMoney), null, Constants.GAME_TYPE_BJSSC,remark.toString());
...
//					Constants.GAME_TYPE_XY_BJSSC,remark.toString(),gaK10Session.getSessionNo(),user.getUserType(),user.getLoginName());
//
//			//更新用户实时余额  --by.cuisy.20171209
//			userService.updateUserMoney(user.getUserId());
//			userService.updateUserBanlance(user.getUserId());
...
//	}
	...
	public boolean saveOpenResult(BjSscGaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			bjSscDAO.updateObject(session, null);
			flag=true;
		}
		return flag;
	...
	public boolean saveAndOpenResult(BjSscGaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			buffer=buffer.substring(0, buffer.length()-1);
			session.setOpenResult(openResult);		
			boolean flag1 = openBjSscSessionOpenResultMethod(session, session.getOpenResult());
			if(flag1){
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return bjSscDAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<BjSscDTO> findGaBetDetailById(String hql, List<Object> para) {
		return bjSscDAO.findGaBetDetailById(hql, para);
	}
	...
	public boolean saveRevokePrize(BjSscGaSession session) {
		...
		para.add(session.getSessionId());
		bjSscDAO.deleteBjSscGaBet(hql.toString(),para);

		boolean result = gaService.saveXyRevokePrize(session.getSessionId(), Constants.GAME_TYPE_XY_BJSSC,session.getSessionNo());
		if(result){
