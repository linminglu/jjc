...
//		if(!Constants.getTimerOpen("xjssc.fetchResult.xy")) return;
//		
//		try {
//			log.info("_______[xjssc fetch result form offical]["+DateTimeUtil.getDateTime()+"]___________________");
//			xjSscService.updateFetchAndOpenResult();			
//
//			log.info("_______[xjssc fetch result form offical end]["+DateTimeUtil.getDateTime()+"]___________________");
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
//	}
	...
	int fetchMaxCount = GameHelpUtil.getFetchMaxCount(Constants.GAME_TYPE_XY_XJSSC);//最大抓取次数 
	int fetchDiffTime = GameHelpUtil.getFetchInterval(Constants.GAME_TYPE_XY_XJSSC);//抓取间隔毫秒
	...
	protected void executeInternal(JobExecutionContext arg0) throws JobExecutionException {
		...
		//定时有效范围
		if(!GameHelpUtil.checkTimerRange(Constants.GAME_TYPE_XY_XJSSC)){
		...
		GameHelpUtil.setFetchTimerMap(Constants.GAME_TYPE_XY_XJSSC, fetchTimer);
		TimerTask task = new TimerTask() {
	        @Override
	        public void run() {
	        	GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_XJSSC);//彩种缓存
	        	try {
	        		//当前系统时间
	        		Date now = DateTimeUtil.getJavaUtilDateNow();
	        		//当前时间期
	        		XjSscGaSession curSession = xjSscService.getCurrentSession();
	        		if(curSession==null){
	        		...
	        		//待开奖期
	        		XjSscGaSession openSession = (XjSscGaSession)xjSscService.getObject(XjSscGaSession.class,curSession.getSessionId()-1);
	        		if(openSession==null){
	        		...
	        		if(!opening){
	        			...
	        			if(sessionNoMap!=null && !sessionNoMap.isEmpty()){
	        				...
	        				String flag = xjSscService.updateFetchAndOpenResult(sessionNoMap);
	        				if(flag.startsWith("success")){
	        					GameHelpUtil.removeOpening(Constants.GAME_TYPE_XY_XJSSC,openSessionNo);//移除开奖标识
