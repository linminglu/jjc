...
import com.xy.ssc.xjssc.model.XjSscGaBet;
...
public class XjSscServiceImpl   extends BaseService implements IXjSscService {
	private IXjSscDAO xjSscDAO;
	private IUserService userService;
	private IGaService gaService;
	public void setXjSscDAO(IXjSscDAO xjSscdao) {
		xjSscDAO = xjSscdao;
		super.dao = xjSscDAO;
	}
...
//		log.info("___[start]__________________________");
		...
		List<XjSscGaSession> saveList = null;
		if(!isTodaySessionInit){
//			log.info("___[start today]__________________________");
			saveList = new ArrayList<XjSscGaSession>();
			String startTimeStr = today + XjSscConstants.XJ_SSC_START_TIME_STR;//晚上开始时间串00:00:00
			log.info("startTimeStr>>>"+startTimeStr);
			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
			for (int i = XjSscConstants.XJ_SSC_MAX_PART_DAY; i < XjSscConstants.XJ_SSC_MAX_PART; i++) {
				//计算出当前场次的时间
				long diffTime = startDate.getTime() + (i-XjSscConstants.XJ_SSC_MAX_PART_DAY)*XjSscConstants.XJ_SSC_TIME_INTERVAL_DAY * 60 * 1000;
				Date curSessionDate = new Date(diffTime);
				Date curSessionDate1 = DateTimeUtil.getDateTimeOfDays(new Date(diffTime ), -1); //0点以后生成上期的
				
				//String openResult = GameConstants.getRandomK10Result();//随机生成5个中奖数字[1-21]
				String sessionNo = this.getTodaySessionNo(curSessionDate1, i+1);//期号
				log.info("___[start today]__________________________sessionNo:"+sessionNo);
				XjSscGaSession k10Session = new XjSscGaSession();
...
//				xjSscDAO.saveObject(k10Session);
			...
			xjSscDAO.updateObjectList(saveList, null);
			flag = "success";
...
//			Date startDate = DateTimeUtil.strToDateMul(startTimeStr);//开始时间
//			
//			for (int i = 0; i < XjSscConstants.XJ_SSC_MAX_PART_DAY; i++) {//经观察得知
//				//计算出当前场次的时间
//				long diffTime = startDate.getTime() + i*XjSscConstants.XJ_SSC_TIME_INTERVAL_DAY * 60 * 1000;
//				Date curSessionDate = new Date(diffTime);
//				
...
//	}
	...
	public boolean checkTodaySessionInit(Date now){
		...
		HQUtils hq = new HQUtils("from XjSscGaSession gks where gks.startTime>? and gks.startTime<?");
		hq.addPars(todayStart);
		hq.addPars(todayEnd);
		Integer count = xjSscDAO.countObjects(hq);
...
//		}
		return DateTimeUtil.DateToStringYY(now) + String.format("%03d", index);
	}
//	public String getTodaySessionNo(Date now,int index){
//		if(index<10){
//			return DateTimeUtil.DateToStringYY(now)+"00"+index;
...
//		Date now = DateTimeUtil.stringToDate("2017-04-17 09:15:00", "yyyy-MM-dd HH:mm:ss");
		...
		//今天是否已经初始化场次
		boolean isTodaySessionInit = this.checkNestdaySessionInit(now);
		List<XjSscGaSession> saveList = new ArrayList<XjSscGaSession>();
		if(!isTodaySessionInit){
//			log.info("___[start today]__________________________");
			saveList = new ArrayList<XjSscGaSession>();
...
//	}
	...
	public String updateFetchAndOpenResult(Map<String, SessionItem> sessionNoMap) {
		...
		//当前场次及开奖场次处理
		XjSscGaSession curtSession = xjSscDAO.getCurrentSession();
		if(curtSession==null) return "fail::no current session";
		XjSscGaSession lastSession = (XjSscGaSession)xjSscDAO.getObject(XjSscGaSession.class,curtSession.getSessionId()-1);
		...
		//迭代开奖无序
		List<XjSscGaSession> openedList = new ArrayList<XjSscGaSession>();//开奖场次
		String sNo;//期号
		...
		try {
			...
			for(Map.Entry<String, SessionItem> entry:sessionNoMap.entrySet()){
				...
				XjSscGaSession session = xjSscDAO.getPreviousSessionBySessionNo(sNo);
				if(session!=null){//开奖
					...
					if(status.equals(GameConstants.OPEN_STATUS_INIT) || status.equals(GameConstants.OPEN_STATUS_OPENING)){
						...
						boolean flag = openXjSscSessionOpenResultMethod(session, result);
						if(flag){
			...
			GaSessionInfo sessionInfo = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_XJSSC);
			if(sessionInfo!=null){
				...
				xjSscDAO.saveObject(sessionInfo);
			...
			for(XjSscGaSession session:openedList){
...
//		XjSscGaSession crrrentsession =xjSscDAO.getCurrentSession();
//		XjSscGaSession tempsession =xjSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(crrrentsession.getSessionNo())-1)+"");	
//		String lastSessionNo=tempsession.getSessionNo();
//		
//			if(!sessionNoMap.isEmpty()){
//				for(String key:sessionNoMap.keySet()){
//					XjSscGaSession session =xjSscDAO.getPreviousSessionBySessionNo(key);
//					if(session!=null){
...
//							
//							boolean flag = openXjSscSessionOpenResultMethod(session, session.getOpenResult());
//							
//							if(flag){
...
//								xjSscDAO.saveObject(session);
//							}else{
//								GameHelpUtil.log(Constants.GAME_TYPE_XY_XJSSC,"open flag FAIL -_- ["+session.getSessionNo()+"]");
//							}
//						}
//						// 把资金明细里投注记录状态值改为有效
//						userService.updateUserTradeDetailStatus(session.getSessionNo(), 
//								Constants.GAME_TYPE_XY_XJSSC, Constants.PUB_STATUS_OPEN);
//					}
//				}
//				
//				GaSessionInfo sessionInfo=gaService.findGaSessionInfo(Constants.GAME_TYPE_XY_XJSSC);
//				if(sessionInfo!=null){
//					SessionItem lastItem = (SessionItem)sessionNoMap.get(lastSessionNo);//上一期
...
//				}
//				xjSscDAO.updateObject(sessionInfo, null);
//				CacheUtil.updateGameList();
//			}
//	}
...
//		if(tempsession!=null){
//		}else{
//			tempsession=(XjSscGaSession) xjSscDAO.getObject(XjSscGaSession.class, (crrrentsession.getSessionId()-1));
//		}
//		String  lastSessionNo=tempsession.getSessionNo();
...
//
//       		//从这里 ---------------------------------------------------------------------------
//		    GaSessionInfo sessionInfo=CacheUtil.getGameOpenUrl().get("xyxjssc");
...
//               		  		log.info("___[xjssc start fetch result xml data...]_______________dddddddddddd_");
//   							break out;
//   						}
...
////					sessionInfo.setGameType(Constants.GAME_TYPE_XY_XJSSC);
////					sessionInfo.setLatestSessionNo(crrrentsession.getSessionNo());
////					if(sessionNoMap.get(lastSessionNo)!=null){
////						sessionInfo.setOpenResult(sessionNoMap.get(lastSessionNo));
////						sessionInfo.setOpenSessionNo(lastSessionNo);
...
//	}
	...
	public void updateTrendResult(XjSscGaSession session){
		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
			List<XjSscGaTrend> list=xjSscDAO.findXjSscGaTrendAllList();
			List<XjSscGaTrend> savelist=new ArrayList<XjSscGaTrend>();
...
//		}
//		
//		XjSscGaSession session =xjSscDAO.getPreviousSessionBySessionNo((Integer.parseInt(xjSscDAO.getCurrentSession().getSessionNo())-1)+"");	
//		if(session!=null){
//		}else{
//			session=(XjSscGaSession) xjSscDAO.getObject(XjSscGaSession.class, (xjSscDAO.getCurrentSession().getSessionId()-1));
//		}
//		if(session.getOpenStatus().equals(GameConstants.OPEN_STATUS_OPENED)){
//			List<XjSscGaTrend> list=xjSscDAO.findXjSscGaTrendAllList();
//			List<XjSscGaTrend> savelist=new ArrayList<XjSscGaTrend>();
...
//	}
	 ...
	 * @return
	 */
	public boolean openXjSscSessionOpenResultMethod(XjSscGaSession session,String result){
		String gameCode = Constants.getGameCodeOfGameType(Constants.GAME_TYPE_XY_XJSSC);
		try{
			List<Object> para = new ArrayList<Object>();
			StringBuffer hql = new StringBuffer();
			hql.append(" and ho.gameType=?");
			para.add(Constants.GAME_TYPE_XY_XJSSC);
...
//	public boolean openXjSscSessionOpenResultMethod(XjSscGaSession session,String result){
//		try{
////			XjSscGaSession session =xjSscDAO.getPreviousSessionBySessionNo(sessionNo);
//			List<Object> para = new ArrayList<Object>();
//			StringBuffer hql = new StringBuffer();
...
//						detail.setOpenResult(result);
//						if(wincash.compareTo(new BigDecimal(0)) == 1){//中奖
//							detail.setWinResult(XjSscConstants.XJ_SSC_WIN);//中奖
...
////							userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME, Constants.CASH_TYPE_CASH_BET_XJSSC, wincash, detail.getBetDetailId());			
...
//							}
//						}else if(wincash.compareTo(new BigDecimal(0)) == 0){//打和
//							detail.setWinResult(XjSscConstants.XJ_SSC_WIN_HE);//打和
...
//							totalPoint=totalPoint.add(new BigDecimal(detail.getBetMoney()));
//							detail.setWinCash(new BigDecimal(0));
//							detail.setWinResult(XjSscConstants.XJ_SSC_WIN_NOT);//未中奖
//							try {
//								xjSscDAO.updateObject(detail, null);
//							} catch (Exception e) {
//								e.printStackTrace();
//							}
//						}
...
//		xjSscDAO.saveObject(detail);
//		user=userService.saveTradeDetail(user,detail.getUserId(), Constants.TRADE_TYPE_INCOME,Constants.CASH_TYPE_CASH_PRIZE, detail.getWinCash(), detail.getBetDetailId(), Constants.GAME_TYPE_XY_XJSSC,remark);
//	}

	public XjSscGaSession getCurrentSession(){
		return xjSscDAO.getCurrentSession();
	}
	public XjSscGaSession getPreviousSessionBySessionNo(String sessionNo){
		return xjSscDAO.getPreviousSessionBySessionNo(sessionNo);
	}
	
	public User saveUserBetInfo(String room,Map<String,Integer> betMap,List<GaBetOption> list,XjSscGaSession session,User user,BigDecimal betAll){
		//彩种缓存
		GaSessionInfo gsi = CacheUtil.getGaSessionInfo(Constants.GAME_TYPE_XY_XJSSC);
		//投注与明细关联
		...
		for(Map.Entry<String, Integer> entry:betMap.entrySet()){
			...
			betDetail.setGameType(Constants.GAME_TYPE_XY_XJSSC);
		...
		Integer tradeDetailId = userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll, null, 
				Constants.GAME_TYPE_XY_XJSSC,remark,session.getSessionNo(),user.getUserType(),user.getLoginName());
		...
		xjSscDAO.updateObjectList(rlList, null);
...
//			xjSscDAO.updateObject(user, null);
//		}else{
////			user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_XJSSC, betAll.subtract(tempMoney), null);
//			StringBuilder remark = new StringBuilder();
//			remark.append("购买彩票 扣款 ")
//			    .append(betAll.subtract(tempMoney)).append("元");
//			user=userService.saveTradeDetail(user,user.getUserId(), Constants.TRADE_TYPE_PAY,Constants.CASH_TYPE_CASH_BUY_LOTO, betAll.subtract(tempMoney), null, Constants.GAME_TYPE_XY_XJSSC,remark.toString());
//
//			//更新用户实时余额  --by.cuisy.20171209
//			userService.updateUserMoney(user.getUserId());
//			userService.updateUserBanlance(user.getUserId());
//		}
////		userService.saveTradeDetail(user.getUserId(), Constants.TRADE_TYPE_PAY, Constants.CASH_TYPE_CASH_BET_XJSSC, betAll, null);
//		xjSscDAO.updateObjectList(betDetailList, null);
//		return  user;
//	}
	...
	public boolean saveOpenResult(XjSscGaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			session.setOpenResult(openResult);
			xjSscDAO.updateObject(session, null);
			flag=true;
		}
		return flag;
	...
	public boolean saveAndOpenResult(XjSscGaSession session,String openResult){
		...
		if(ParamUtils.chkString(openResult)){
			...
			buffer=buffer.substring(0, buffer.length()-1);
			session.setOpenResult(openResult);		
			boolean flag1 = openXjSscSessionOpenResultMethod(session, session.getOpenResult());
			if(flag1){
				session.setOpenTime(DateTimeUtil.getJavaUtilDateNow());
				session.setOpenStatus(GameConstants.OPEN_STATUS_OPENED);
				xjSscDAO.updateObject(session, null);
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		return xjSscDAO.findGaBetDetail(hql, para, pageNum, pageSize);
	}

	@Override
	public List<XjSscDTO> findGaBetDetailById(String hql, List<Object> para) {
		return xjSscDAO.findGaBetDetailById(hql, para);
	}
	@Override
	public boolean saveRevokePrize(XjSscGaSession session) {
		...
		para.add(session.getSessionId());
		xjSscDAO.deleteXjSscGaBet(hql.toString(),para);

		boolean result = gaService.saveXyRevokePrize(session.getSessionId(), Constants.GAME_TYPE_XY_XJSSC,session.getSessionNo());
		if(result){
