package com.xy.ssc.tjssc.dao.hibernate;
...
implements ITjSscDAO {
	...
	public TjSscGaSession getCurrentSession() {
		...
		List<TjSscGaSession> list = getSession().createQuery("from TjSscGaSession gks where gks.startTime<=? and gks.endTime>? and (gks.openStatus=? or gks.openStatus=?) order by gks.sessionId")
		.setTimestamp(0, now)
		.setTimestamp(1, now)
		.setString(2, TjSscConstants.TJ_SSC_OPEN_STATUS_INIT)
		.setString(3, TjSscConstants.TJ_SSC_OPEN_STATUS_OPENING)
		.setMaxResults(1)
		.list();
		if(list!=null && list.size()>0){
			return (TjSscGaSession)list.get(0);
		}
		return null;
	}
	public TjSscGaSession getPreviousSessionBySessionNo(String sessionNo){
		String  hql=" from TjSscGaSession gks where gks.sessionNo=? ";
	...
	public List<TjSscGaTrend> findTjSscGaTrendList(){
		String  hql=" from TjSscGaTrend gks where gks.trendCount >1 order by gks.trendCount desc";
		Query record =getSession().createQuery(hql);
		List list=record.list();
		return list;
	}
	public List<TjSscGaTrend> findTjSscGaTrendAllList(){
		String  hql=" from TjSscGaTrend gks where 1=1 ";
		Query record =getSession().createQuery(hql);
		List list=record.list();
		return list;
	}
	public PaginationSupport  findTjSscGaSessionList(String hql, List<Object> para,int pageNum,int pageSize){
		Query query = makeQuerySQL( " from TjSscGaSession ho where 1=1 "+hql,para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.sessionId) from TjSscGaSession ho where 1=1 "+hql, para);
	...
	public PaginationSupport  findTjSscGaBetList(String hql, List<Object> para,int pageNum,int pageSize){
		Query query = makeQuerySQL( " select new com.xy.ssc.tjssc.model.dto.TjSscDTO(ho.sessionNo,ho.totalPoint,ho.winCash,ho.totalPoint-ho.winCash) from TjSscGaBet ho where 1=1 "
				+hql, para);
		query.setFirstResult(pageNum);
		query.setMaxResults(pageSize);
		List queList = query.list();
		Query count = makeQuerySQL("select count(ho.betId) from  TjSscGaBet ho where 1=1" +
				hql, para);
		Integer totalCount = (Integer) count.uniqueResult();
		return new PaginationSupport(queList, totalCount.intValue());
	}
	...
	public PaginationSupport findGaBetDetail(String hql, List<Object> para,
		...
		Query query = makeQuerySQL( " select new com.xy.ssc.tjssc.model.dto.TjSscDTO(ga,u) from GaBetDetail ga,User u where 1=1 "
	...
	public List<TjSscDTO> findGaBetDetailById(String hql, List<Object> para) {
	    Query query = getSession().createQuery(" select new com.xy.ssc.tjssc.model.dto.TjSscDTO(ga,u) from GaBetDetail ga,User u where 1=1 and ga.userId = u.userId "
	    		+ hql)
	    		.setParameter(0, para.get(0));
	    List<TjSscDTO> list = query.list();
		return list;
	}
	...
	public void deleteTjSscGaBet(String hql, List<Object> para) {
		String delHql=" delete from TjSscGaBet where 1=1 ";
