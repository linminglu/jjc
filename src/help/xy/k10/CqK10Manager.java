...
public class CqK10Manager extends BaseDispatchAction {
	...
	public void previousNo(ActionMapping mapping, ActionForm form,
		...
		CqK10GaSession currentSession = cqK10Service.getCurrentSession();
		if (null == currentSession) {
			map.put("code", APIConstants.CODE_REQUEST_SUCCESS);
			map.put("msg", "未到开奖时间");
			map.put("data", data);
		} else {
			CqK10GaSession gaK10Session = cqK10Service
					.getPreviousSessionBySessionNo((Integer
							.parseInt(currentSession.getSessionNo()) - 1) + "");
			// CqK10GaSession gaK10Session=(CqK10GaSession)
			// cqK10Service.getObject(CqK10GaSession.class,
	...
	public void currentTime(ActionMapping mapping, ActionForm form,
		...
		String msg = "";
		CqK10GaSession gaK10Session=cqK10Service.getCurrentSession();
		if(gaK10Session!=null){
			JSONObject obj = new JSONObject();// 返回数据层
			...
			long betTime=0;
			if(openTime >= CqK10Constants.CQ_K10_TIME_OPENING_FREEZE){
				betTime=openTime - CqK10Constants.CQ_K10_TIME_OPENING_FREEZE;
			}
			obj.put("gameName", "重庆快乐十分");
			obj.put("room", "");
			...
			CqK10GaSession beforSession=cqK10Service.getPreviousSessionBySessionNo((Integer.parseInt(gaK10Session.getSessionNo())-1)+"");			
			if(beforSession==null){
				beforSession=(CqK10GaSession) cqK10Service.getObject(CqK10GaSession.class, gaK10Session.getSessionId()-1);
			}
...
//		String gameType = ParamUtils.getParameter(deParameter, "gameType", "5");// 游戏类型
		...
		String gameName = "cqk10Liangmianpan";
		if (playType.equals("0")) {
			gameName = "cqk10Liangmianpan";
		} else if (playType.equals("9")) {
			gameName = "cqk10Lianma";
		} else {
			gameName = "cqk10di" + playType.trim() + "qiu";
		}
	...
	public void bet(ActionMapping mapping, ActionForm form,
		...
		if (!ParamUtils.chkString(ucode) || !ParamUtils.chkString(optionArray)
			...
			CqK10GaSession gaK10Session = cqK10Service.getPreviousSessionBySessionNo(sessionNo);
			CqK10GaSession tempSession = (CqK10GaSession) cqK10Service.getObject(CqK10GaSession.class, gaK10Session.getSessionId()-1);
			Date nowDate = new Date();
			Date startTime = tempSession.getEndTime();
			Date endTime = gaK10Session.getEndTime();
			Date fengPanTime = DateTimeUtil.dateDiffSec(endTime, -CqK10Constants.CQ_K10_TIME_OPENING_FREEZE);// 封盘时间
			...
			if (sessionNo.equals(gaK10Session.getSessionNo())) {
				...
				if(ParamUtils.chkString(optionArray)){
					...
					if (user == null) {
						...
						hq.addPars(Constants.GAME_TYPE_XY_CQK10);
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							if (jsonObj.get("id") != null) {
								...
								GaBetOption gbo = (GaBetOption) cqK10Service.getObject(GaBetOption.class, optionId);
								...
								price = betPoint;
								if (betPoint > CqK10Constants.CQ_K10_MAX_BET_POINT) {
									map.put("code",APIConstants.CODE_REQUEST_ERROR);
									map.put("msg", "单注不能超过"+CqK10Constants.CQ_K10_MAX_BET_POINT+"！");
...
//						if (betAll.intValue() + betMoney > CqK10Constants.TOTAL_MAX_BET_POINT) {
//							data.put("money", ProductUtil
//									.BigFormatJud(balance));
...
//						}else 
						if (betAll.intValue() > balance.intValue()) {
							...
							for (GaBetOption option : list) {
								...
								if(!Constants.GAME_TYPE_XY_CQK10.equals(gameType)){
									map.put("code", APIConstants.CODE_REQUEST_ERROR);
							...
							user=cqK10Service.saveUserBetInfo(room, betMap, list,
	...
	public void hotRanking(ActionMapping mapping, ActionForm form,
		...
		List<CqK10GaTrend> list=cqK10Service.findCqK10GaTrendList();
		JSONArray jsonArray = new JSONArray();
		if (list != null && list.size() > 0) {
			for (CqK10GaTrend trend : list) {
				JSONObject obj = new JSONObject();// 返回数据层
	...
	public void trend(ActionMapping mapping, ActionForm form,
		...
		para.add(CqK10Constants.CQ_K10_OPEN_STATUS_OPENED);
		hqls.append(" order by  ho.sessionId desc ");// 已开奖
		PaginationSupport ps = cqK10Service.findCqK10GaSessionList(
				hqls.toString(), para, pageNum, pageSize);
		List<CqK10GaSession> list = ps.getItems();
		JSONArray jsonArray = new JSONArray();
		if (list != null && list.size() > 0) {
			if (type.equals("0")) {
				...
				items.put("八");
				jsonArray.put(items);
				for (CqK10GaSession session : list) {
					items = new JSONArray();
	...
	public void rules(ActionMapping mapping, ActionForm form,
		...
		JSONObject data = new JSONObject();// 返回数据层
		map.put("code", APIConstants.CODE_REQUEST_SUCCESS);
		data.put("link", RamConstants.getWebServer() + "/help/cqk10rule.html"
	...
	public void openList(ActionMapping mapping, ActionForm form,
		...
		if (list != null & list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				CqK10GaSession session = list.get(i);
				JSONObject it = new JSONObject();
				it.put("gameName", "重庆快乐十分");
				it.put("openTime",
	...
	public void latestLottery(ActionMapping mapping, ActionForm form,
		...
		hqls.append(" order by  ho.sessionId desc ");// 已开奖
		PaginationSupport ps = cqK10Service.findCqK10GaSessionList(hqls.toString(),
				para, 0, 10);
		List<CqK10GaSession> list = ps.getItems();
		if (list != null & list.size() > 0) {
			JSONArray items = new JSONArray();
