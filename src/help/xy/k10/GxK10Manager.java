...
public class GxK10Manager  extends BaseDispatchAction {
	...
	public void previousNo(ActionMapping mapping, ActionForm form,
		...
		GxK10GaSession currentSession = gxK10Service.getCurrentSession();
		if (null == currentSession) {
			map.put("code", APIConstants.CODE_REQUEST_SUCCESS);
			map.put("msg", "未到开奖时间");
			map.put("data", data);
		} else {
			GxK10GaSession gaK10Session = gxK10Service
					.getPreviousSessionBySessionNo((Integer
							.parseInt(currentSession.getSessionNo()) - 1) + "");
			// GxK10GaSession gaK10Session=(GxK10GaSession)
			// gxK10Service.getObject(GxK10GaSession.class,
	...
	public void currentTime(ActionMapping mapping, ActionForm form,
		...
		String msg = "";
		GxK10GaSession gaK10Session=gxK10Service.getCurrentSession();
		if(gaK10Session!=null){
			JSONObject obj = new JSONObject();// 返回数据层
			...
			long betTime=0;
			if(openTime >= GxK10Constants.GX_K10_TIME_OPENING_FREEZE){
				betTime=openTime - GxK10Constants.GX_K10_TIME_OPENING_FREEZE;
			}
			obj.put("gameName", "广西快乐十分");
			obj.put("room", "");
			...
			GxK10GaSession beforSession=gxK10Service.getPreviousSessionBySessionNo((Integer.parseInt(gaK10Session.getSessionNo())-1)+"");			
			if(beforSession==null){
				beforSession=(GxK10GaSession) gxK10Service.getObject(GxK10GaSession.class, gaK10Session.getSessionId()-1);
			}
...
//		String gameType = ParamUtils.getParameter(deParameter, "gameType", "5");// 游戏类型
		...
		String gameName = "gxk10Liangmianpan";
		if (playType.equals("0")) {
			gameName = "gxk10Liangmianpan";
		} else if (playType.equals("9")) {
			gameName = "gxk10Lianma";
		} else {
			gameName = "gxk10di" + playType.trim() + "qiu";
		}
	...
	public void bet(ActionMapping mapping, ActionForm form,
		...
		if (!ParamUtils.chkString(ucode) || !ParamUtils.chkString(optionArray)
			...
			GxK10GaSession tempSession = (GxK10GaSession) gxK10Service.getObject(GxK10GaSession.class, gaK10Session.getSessionId()-1);
			Date nowDate = new Date();
			Date startTime = tempSession.getEndTime();
			Date endTime = gaK10Session.getEndTime();
			Date fengPanTime = DateTimeUtil.dateDiffSec(endTime, -GxK10Constants.GX_K10_TIME_OPENING_FREEZE);// 封盘时间
			...
			if (sessionNo.equals(gaK10Session.getSessionNo())) {
				...
				if(ParamUtils.chkString(optionArray)){
					...
					if (user == null) {
						...
						hq.addPars(Constants.GAME_TYPE_XY_GXK10);
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							if (jsonObj.get("id") != null) {
								...
								GaBetOption gbo = (GaBetOption) gxK10Service.getObject(GaBetOption.class, optionId);
								...
								price = betPoint;
								if (betPoint > GxK10Constants.SINGLE_NOTE_MAX_BET_POINT) {
									map.put("code",
											APIConstants.CODE_REQUEST_ERROR);
									map.put("msg", "单注不能超过"+GxK10Constants.SINGLE_NOTE_MAX_BET_POINT+"！");
...
//						if (betAll.intValue() + betMoney > GxK10Constants.TOTAL_MAX_BET_POINT) {
//							data.put("money", ProductUtil
//									.BigFormatJud(balance));
...
//						}else 
						if (betAll.intValue() > balance
							...
							for (GaBetOption option : list) {
								...
								if(!Constants.GAME_TYPE_XY_GXK10.equals(gameType)){
									map.put("code", APIConstants.CODE_REQUEST_ERROR);
							...
							user=gxK10Service.saveUserBetInfo(room, betMap, list,
	...
	public void hotRanking(ActionMapping mapping, ActionForm form,
		...
		List<GxK10GaTrend> list=gxK10Service.findGxK10GaTrendList();
		JSONArray jsonArray = new JSONArray();
		if (list != null && list.size() > 0) {
			for (GxK10GaTrend trend : list) {
				JSONObject obj = new JSONObject();// 返回数据层
	...
	public void trend(ActionMapping mapping, ActionForm form,
		...
		para.add(GxK10Constants.GX_K10_OPEN_STATUS_OPENED);
		hqls.append(" order by  ho.sessionId desc ");// 已开奖
		PaginationSupport ps = gxK10Service.findGxK10GaSessionList(
				hqls.toString(), para, pageNum, pageSize);
		List<GxK10GaSession> list = ps.getItems();
		JSONArray jsonArray = new JSONArray();
		if (list != null && list.size() > 0) {
			if (type.equals("0")) {
				...
				items.put("五");
				jsonArray.put(items);
				for (GxK10GaSession session : list) {
					items = new JSONArray();
	...
	public void getSum(String results, JSONArray items) {
		...
		items.put(result + "");
		if (result > 55 && result <96) {
			items.put("大");
		} else if (result >= 15 && result < 55) {
			items.put("小");
		} else if (result == 55) {
			items.put("和");
		}
	...
	public void rules(ActionMapping mapping, ActionForm form,
		...
		JSONObject data = new JSONObject();// 返回数据层
		map.put("code", APIConstants.CODE_REQUEST_SUCCESS);
		data.put("link", RamConstants.getWebServer() + "/help/gxk10rule.html"
	...
	public void openList(ActionMapping mapping, ActionForm form,
		...
		if (list != null & list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				GxK10GaSession session = list.get(i);
				JSONObject it = new JSONObject();
				it.put("gameName", "广西快乐十分");
				it.put("openTime",
	...
	public void winRanking(ActionMapping mapping, ActionForm form,
		...
		if (message.equals("")) {
			list = gaService.findGaWinCountList(Constants.GAME_TYPE_XY_GXK10);
			if (list != null && list.size() > 0) {
