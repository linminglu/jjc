var lotteryObj={
				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-cqK10open');},timerObj.intervalAPI);
						}
						$('#openlable').html("开奖中...");
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-cqK10');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-cqK10open');},timerObj.interval);
					}
	...
	open:function(t){//开奖
		if(t!=-1){
			...
			$('.betting .innum').removeClass("fengpan");$(".fengpanbox").each(function(){
				if($(this).find(".fengpanbox").length==0){
			    	$(this).remove();
				}
			});
