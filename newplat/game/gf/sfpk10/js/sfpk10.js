var lotteryObj={
	...
	updateOpenInfo(obj){//更新界面信息
		...
		if(obj.resultItems && obj.resultItems.length==10){//上期开奖号
			$(".open-pn").html("<span class=\"open-num\">"+move.widget.fmtSfPk10OpenNo(obj.resultItems)+"</span>");
		}else{
				...
				if(code=="200"){
					...
					if(items.length>0){
						for(var i=0;i<items.length;i++){
							...
							html=html+'<li class="mui-table-view-cell"><div class="title-box"><div class="title">急速赛车</div>';
...
lotteryObj.selector = {
	...
	calcBet:function(curPlay,selArr,rate){//玩法，选中的号，倍率
		...
		if(curPlay.t==9){// 前二
			...
			/*han*/
			var betNum = betNum = lotteryObj.getBetNum(m,n);//预览下注数
			/*han*/
...
move.sfpk10_currentTime=move.server+"/api/gfSfPk10_sessionInfo";
move.sfpk10_betPanel=move.server+"/api/gfSfPk10_betPanel";
move.sfpk10_bet=move.server+"/api/gfSfPk10_bet";
move.sfpk10_trend=move.server+"/api/gfSfPk10_trend";
move.sfpk10_hotRanking=move.server+"/api/gfSfPk10_hotRanking";
move.sfpk10_openList=move.server+"/api/gfSfPk10_openList";
