var lotteryObj={
	...
	updateOpenInfo(obj){//更新界面信息
		...
		if(obj.resultItems && obj.resultItems.length==10){//上期开奖号
			$(".open-pn").html("<span class=\"open-num\">"+move.widget.fmtSfPk102OpenNo(obj.resultItems)+"</span>");
		}else{
				...
				if(code=="200"){
					...
					if(items.length>0){
						for(var i=0;i<items.length;i++){
							...
							html=html+'<li class="mui-table-view-cell"><div class="title-box"><div class="title">三分PK拾</div>';
...
move.sfpk102_currentTime=move.server+"/api/gfSfPk102_sessionInfo";
...
move.sfpk102_hotRanking=move.server+"/api/gfSfPk102_hotRanking";
