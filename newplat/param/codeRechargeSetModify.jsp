...
<body>
<div class="bread">支付宝/微信/QQ扫码充值开关设置  &raquo;  
...
<html:form action="/paramAction.do?method=saveCodeRechargeSet" onsubmit="return formSubmit();" enctype="multipart/form-data">
 	...
 	<tr class="tr-color-body" id="imgbox">
      	...
      	<td>
      		...
      		<c:if test="${paramForm.param.value ne null}">
      			<img src="..<c:out value="${paramForm.param.value}"/>" alt="<c:out value="${paramForm.param.title}"/>" title="<c:out value="${paramForm.param.title}"/>" width="300"/>
      		</c:if>
      		<p class="red">
      				建议尺寸    [宽:高]1:1
      		</p>
