...
<body>
<div class="bread"><c:if test = "${paramForm.type==1}">会员充值返水代理设置 </c:if>
<c:if test = "${paramForm.type==2}">会员打码返水代理设置</c:if>&raquo;  
...
<html:form action="/paramAction.do?method=saveSendAgentSet" onsubmit="return formSubmit();" enctype="multipart/form-data">
...
<table class="tblistadd" border="0" cellpadding="0" cellspacing="1">
    ...
    <c:if test = "${paramForm.type==2}">
 	<tr class="tr-color-body" id="imgbox">
      	...
      	<td>
      		<html:text property="time" name="paramForm" styleClass="Wdate" styleId="startTime"   onclick="WdatePicker({lang:'zh-cn',dateFmt:'HH:mm:ss'})" readonly="readonly"/>
      	</td>
    </tr>
    </c:if>
     <tr class="tr-color-body" id="imgbox">
      	...
      	<td>
      		<html:text property="value" name="paramForm" maxlength="300"/> &nbsp;%
      	</td>
    </tr>
</table>
