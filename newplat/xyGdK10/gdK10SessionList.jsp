...
function justify(id){
	...
	if(value!=null&&value!=""&&value.trim()!=""){
		 if(confirm('确实该开奖结果吗？')){		
				$.post("gdK10Action.do?method=saveOpenResult&sessionId="+id+"&openResult="+value, 
						function(data) {
							data = $.trim(data);					
							if (data == "success") {
								location.reload();
...
function open(id){
	...
	if(value!=null&&value!=""&&value.trim()!=""){
		 if(confirm('确实要开奖吗？开奖号码：'+value)){		
				$.post("gdK10Action.do?method=openResult&sessionId="+id+"&openResult="+value, 
						function(data) {
							data = $.trim(data);
							if (data == "success") {
								location.reload();
...
function drawback(id){
	 if(confirm('确实要退还本期投注吗？')){
			$.post("gdK10Action.do?method=drawback&sessionId="+id, 
					function(data) {
						data = $.trim(data);
						if (data == "success") {
							location.reload();
...
function revokePrize(id){
	layer.confirm('确实要撤回本期已经派彩的金额吗？', {icon: 3, offset: '100px', title:'提示'}, function(index){
		layer.close(index);
		$.post("gdK10Action.do?method=revokePrize&sessionId="+id, 
			function(data) {
...
<body>
<html:form action="/gdK10Action.do?method=init">
