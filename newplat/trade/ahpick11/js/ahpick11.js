...
//			var type=$(this).attr('type');
	...
	gamePlayType:function(){
		...
		$.ajax({
			type: "POST",
			url:move.ahPick11_gamePlayType,
			dataType:'json',
			data:{
				mw:mw
			},
				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-ahPick11open');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-ahPick11');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-ahPick11open');},timerObj.interval);
					}
...
//				mask.close();
	...
	openlist:function(){
		...
		$.ajax({
			type: "POST",
			url:move.ahPick11_latestLottery,
			dataType:'json',
			data:{
				mw:mw
			},
...
move.ahPick11_gamePlayType=move.server+'/api/gfAhPick11_gamePlayType';
move.ahPick11_betPanel=move.server+'/api/gfAhPick11_betPanel';
move.ahPick11_sessionInfo=move.server+'/api/gfAhPick11_sessionInfo';
move.ahPick11_openList=move.server+'/api/gfAhPick11_openList';
move.ahPick11_latestLottery=move.server+'/api/gfAhPick11_latestLottery';
move.ahPick11_bet=move.server+'/api/gfAhPick11_bet';
move.ahPick11_winRanking=move.server+'/api/gfAhPick11_winRanking';
