				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-bjk3open');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-bjk3');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-bjk3open');},timerObj.interval);
					}
...
move.bjK3_gamePlayType=move.server+'/api/gfBjK3_gamePlayType';
move.bjK3_betPanel=move.server+'/api/gfBjK3_betPanel';
move.bjK3_sessionInfo=move.server+'/api/gfBjK3_sessionInfo';
move.bjK3_openList=move.server+'/api/gfBjK3_openList';
move.bjK3_latestLottery=move.server+'/api/gfBjK3_latestLottery';
move.bjK3_bet=move.server+'/api/gfBjK3_bet';
move.bjK3_winRanking=move.server+'/api/gfBjK3_winRanking';
move.bjK3_trend=move.server+'/api/gfBjK3_trend';
