				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-jlk3open');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-jlk3');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-jlk3open');},timerObj.interval);
					}
...
move.jlK3_gamePlayType=move.server+'/api/gfJlK3_gamePlayType';
move.jlK3_betPanel=move.server+'/api/gfJlK3_betPanel';
move.jlK3_sessionInfo=move.server+'/api/gfJlK3_sessionInfo';
move.jlK3_openList=move.server+'/api/gfJlK3_openList';
move.jlK3_latestLottery=move.server+'/api/gfJlK3_latestLottery';
move.jlK3_bet=move.server+'/api/gfJlK3_bet';
move.jlK3_winRanking=move.server+'/api/gfJlK3_winRanking';
move.jlK3_trend=move.server+'/api/gfJlK3_trend';
