				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-ahk3open');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-ahk3');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-ahk3open');},timerObj.interval);
					}
...
move.ahK3_gamePlayType=move.server+'/api/gfAhK3_gamePlayType';
move.ahK3_betPanel=move.server+'/api/gfAhK3_betPanel';
move.ahK3_sessionInfo=move.server+'/api/gfAhK3_sessionInfo';
move.ahK3_openList=move.server+'/api/gfAhK3_openList';
move.ahK3_latestLottery=move.server+'/api/gfAhK3_latestLottery';
move.ahK3_bet=move.server+'/api/gfAhK3_bet';
move.ahK3_winRanking=move.server+'/api/gfAhK3_winRanking';
move.ahK3_trend=move.server+'/api/gfAhK3_trend';
