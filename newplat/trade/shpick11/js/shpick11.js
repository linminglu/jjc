...
//			var type=$(this).attr('type');
			...
			if(action=='all'){
				...
				selectArr=['06','07','08','09','10','11'];
			}else if(action=='xiao'){
				selectArr=['01','02','03','04','05'];
			}else if(action=='ji'){
				selectArr=['01','03','05','07','09','11'];
			}else if(action=='ou'){
				selectArr=['02','04','06','08','10'];
	...
	gamePlayType:function(){
		...
		$.ajax({
			type: "POST",
			url:move.shPick11_gamePlayType,
			dataType:'json',
			data:{
				mw:mw
			},
	...
	randomPlay:function(count){
		...
		for(var c = 0; c < count; ++c) {
			...
			if(group == 11) {
				play.kind = tab == '1' ? '任二复式' : '任二单式';
				play.play =  this.randomBall(2);
			}else if(group == 10) {
				play.kind = tab == '1' ? '任三复式' : '任三单式';
				play.play = this.randomBall(3);
			}else if(group == 9) {
				play.kind = tab == '1' ? '任四复式' : '任四单式';
				play.play = this.randomBall(4);
			}else if(group == 8) {
				play.kind = tab == '1' ? '任五复式' : '任五单式';
				play.play = this.randomBall(5);
			} else if(group == 7) {
				play.kind = tab == 1 ? '任六复式' : '任六单式';
				play.play = this.randomBall(6);
			} else if(group == 6) {
				play.kind = tab == 1 ? '任七复式' : '任七单式';
				play.play = this.randomBall(7);
			} else if(group == 5) {
				play.kind = tab == 1 ? '任八复式' : '任八单式';
				play.play = this.randomBall(8);
			} else if(group == 4) {
				play.kind = tab == 1 ? '前一复式' : '前一单式';
				play.play = this.randomBall(1);
			} else if(group == 3) {
				play.kind = tab == 1 ? '前二组选复式' : '前二组选单式';
				play.play = this.randomBall(2);
			} else if(group == 2) {
				play.kind = tab == 1 ? '前二直选复式' : '前二直选单式';
				play.play = this.randomBall(2,'*');
			} else if(group == 1) {
				play.kind = tab == 1 ? '前三组选复式' : '前三组选单式';
				play.play = this.randomBall(3);
			} else if(group == 0) {
				play.kind = tab == 1 ? '前三直选复式' : '前三直选单式';
				play.play = this.randomBall(3,'*');
			} else {
				return;
			}
	...
	calcZhushu2:function(type){
		...
		if(type=='11'){
			...
			var gesize=$('#play'+type+' .gebox .selected').size();
			totalNumB=getTotalNum(gesize,4);
		}else if(type=='8'){
			var gesize=$('#play'+type+' .gebox .selected').size();
			totalNumB=getTotalNum(gesize,5);
		}else if(type=='7'){
			var gesize=$('#play'+type+' .gebox .selected').size();
			totalNumB=getTotalNum(gesize,6);
		}else if(type=='6'){
			var gesize=$('#play'+type+' .gebox .selected').size();
			totalNumB=getTotalNum(gesize,7);
		}else if(type=='5'){
			var gesize=$('#play'+type+' .gebox .selected').size();
			totalNumB=getTotalNum(gesize,8);
		}else if(type=='4'){
			var gesize=$('#play'+type+' .gebox .selected').size();
			totalNumB=gesize;
		}else if(type=='3'){
			...
			for(var i=0;i<shiArr.length;i++) {
				var one=$(shiArr[i]).attr('data');
				for(var j=0;j<geArr.length;j++) {
					var tow=$(geArr[j]).attr('data');
					if(one!=tow){
						num += 1;
					}
				}
			...
			var baiArr=$('#play'+type+' .baibox .selected');
			var shiArr=$('#play'+type+' .shibox .selected');
			var geArr=$('#play'+type+' .gebox .selected');
			var num=0;
			for(var i=0;i<baiArr.length;i++) {
				var one=$(baiArr[i]).attr('data');
				for(var j=0;j<shiArr.length;j++) {
					var tow=$(shiArr[j]).attr('data');
					if(one!=tow){
						for(var k=0;k<geArr.length;k++) {
							var three=$(geArr[k]).attr('data');
							if(one != three && tow != three) {
								num += 1;
							}
						}
					}
	...
	confirmSelect:function(type){
		...
		if(totalNum>0){
			...
			var shiArr={};
			var geArr={};
			if(type=='11'||type=='10'||type=='9'||type=='8'||type=='7'||type=='6'||type=='5'||type=='4'||type=='3'||type=='1'){
				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-shPick11open');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-shPick11');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-shPick11open');},timerObj.interval);
					}
...
//				mask.close();
	...
	openlist:function(){
		...
		$.ajax({
			type: "POST",
			url:move.shPick11_latestLottery,
			dataType:'json',
			data:{
				mw:mw
			},
...
move.shPick11_gamePlayType=move.server+'/api/gfShPick11_gamePlayType';
move.shPick11_betPanel=move.server+'/api/gfShPick11_betPanel';
move.shPick11_sessionInfo=move.server+'/api/gfShPick11_sessionInfo';
move.shPick11_openList=move.server+'/api/gfShPick11_openList';
move.shPick11_latestLottery=move.server+'/api/gfShPick11_latestLottery';
move.shPick11_bet=move.server+'/api/gfShPick11_bet';
move.shPick11_winRanking=move.server+'/api/gfShPick11_winRanking';
move.shPick11_trend=move.server+'/api/gfShPick11_trend';
