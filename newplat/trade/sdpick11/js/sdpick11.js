...
//			var type=$(this).attr('type');
	...
	gamePlayType:function(){
		...
		$.ajax({
			type: "POST",
			url:move.sdPick11_gamePlayType,
			dataType:'json',
			data:{
				mw:mw
			},
				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-sdPick11open');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-sdPick11');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-sdPick11open');},timerObj.interval);
					}
...
move.sdPick11_gamePlayType=move.server+'/api/gfSdPick11_gamePlayType';
move.sdPick11_betPanel=move.server+'/api/gfSdPick11_betPanel';
move.sdPick11_sessionInfo=move.server+'/api/gfSdPick11_sessionInfo';
move.sdPick11_openList=move.server+'/api/gfSdPick11_openList';
move.sdPick11_latestLottery=move.server+'/api/gfSdPick11_latestLottery';
move.sdPick11_bet=move.server+'/api/gfSdPick11_bet';
move.sdPick11_winRanking=move.server+'/api/gfSdPick11_winRanking';
move.sdPick11_trend=move.server+'/api/gfSdPick11_trend';
