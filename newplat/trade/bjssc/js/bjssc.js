				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-bjsscopen');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-bjssc');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-bjsscopen');},timerObj.interval);
					}
...
move.bjSsc_sessionInfo=move.server+'/api/gfBjSsc_sessionInfo';
...
move.bjSsc_winRanking=move.server+'/api/gfBjSsc_winRanking';
