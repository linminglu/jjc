...
//			var type=$(this).attr('type');
	...
	gamePlayType:function(){
		...
		$.ajax({
			type: "POST",
			url:move.jxPick11_gamePlayType,
			dataType:'json',
			data:{
				mw:mw
			},
				...
				if(code=='200'){
					...
					if(length==0){
						if(timeropenresult==null){
							timeropenresult=window.setInterval(function(){timerObj.result('timer-jxPick11open');},timerObj.intervalAPI);
						}
					}else{
					...
					if(timerxiazhu==null){
						timerxiazhu=window.setInterval(function(){timerObj.init(timerObj.rest,'timer-jxPick11');},timerObj.interval); 
					}
					if(timeropen==null){
						timeropen=window.setInterval(function(){timerObj.open(timerObj.openrest,'timer-jxPick11open');},timerObj.interval);
					}
...
move.jxPick11_gamePlayType=move.server+'/api/gfJxPick11_gamePlayType';
move.jxPick11_betPanel=move.server+'/api/gfJxPick11_betPanel';
move.jxPick11_sessionInfo=move.server+'/api/gfJxPick11_sessionInfo';
move.jxPick11_openList=move.server+'/api/gfJxPick11_openList';
move.jxPick11_latestLottery=move.server+'/api/gfJxPick11_latestLottery';
move.jxPick11_bet=move.server+'/api/gfJxPick11_bet';
move.jxPick11_winRanking=move.server+'/api/gfJxPick11_winRanking';
move.jxPick11_trend=move.server+'/api/gfJxPick11_trend';
