...
function justify(id){
	...
	if(value!=null&&value!=""&&value.trim()!=""){
		 if(confirm('确实该开奖结果吗？')){		
				$.post("bjLu28Action.do?method=saveOpenResult&sessionId="+id+"&openResult="+value, 
						function(data) {
							data = $.trim(data);					
							if (data == "success") {
								location.reload();
...
function open(id){
	...
	if(value!=null&&value!=""&&value.trim()!=""){
		 if(confirm('确实要开奖吗？开奖号码：'+value)){		
				$.post("bjLu28Action.do?method=openResult&sessionId="+id+"&openResult="+value, 
						function(data) {
							data = $.trim(data);
							if (data == "success") {
								location.reload();
...
function drawback(id){
	 if(confirm('确实要退还本期投注吗？')){
			$.post("bjLu28Action.do?method=drawback&sessionId="+id, 
					function(data) {
						data = $.trim(data);
						if (data == "success") {
							location.reload();
...
function revokePrize(id){
	layer.confirm('确实要撤回本期已经派彩的金额吗？', {icon: 3, offset: '100px', title:'提示'}, function(index){
		layer.close(index);
		$.post("bjLu28Action.do?method=revokePrize&sessionId="+id, 
			function(data) {
...
<body>
<html:form action="/bjLu28Action.do?method=init">
<html:hidden property="startIndex" name="bjLu28Form"/>
<table class="tblist" cellpadding="0" cellspacing="0" border="0">
	<tr class="moon">
		<div class="bread">统计管理&raquo;各彩种统计管理</div>
	</tr>
</table>
</html:form>
<%int number=1;	
...
<table class="tblist" cellpadding="0" cellspacing="0" border="0">
	...
	<tr class="tr-color-body" onmousemove="this.style.background='#EEE'" onmouseout="this.style.background='#FFF'">
	    ...
	    <td align="center">
			<input  id="openresult<bean:write name="item" property="sessionId"/>" name="resultNo"  value="<bean:write name="item" property="openResult"/>"  size="50" /><p class="red">每个数字之间用逗号隔开,例：1,2,6,9</p>
	    </td>
		<td align="center">
